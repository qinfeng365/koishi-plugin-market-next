...
- **弱网少做无用功**：自动路由竞速命中最快源后会取消其他候选源请求，减少额外带宽占用。
- **慢在哪里能看见**：`search.logLevel: debug` 时，市场页会显示下载、缓存读取、JSON parse、索引映射、前端筛选和虚拟滚动耗时。
...
`debug` 级别会额外在市场页显示性能面板，并在后端日志输出分段耗时；默认 `warn` 不会输出这些诊断日志。
