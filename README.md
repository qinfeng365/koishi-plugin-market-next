...
`3.5.5` 是本项目的第一个正式 release。`3.5.6-alpha.5` 是弱网依赖版本刷新与依赖管理页体验的当前 alpha 测试版，默认不会替换 npm 的 `latest` 渠道。
...
```bash
npm install koishi-plugin-market-next@3.5.6-alpha.5
```
...
`3.5.6-alpha.5` 增加了依赖更新提示控制，用来减少不需要处理的更新噪音：

- 可以在插件市场设置里填写“不检测更新的依赖名”，每行或用逗号分隔一个包名；这些依赖不会进入可更新或已忽略分组。
- 依赖卡片上的“忽略此次更新”会按设置记录忽略规则，而不是只做一次临时隐藏。
- “忽略时长”可控制忽略规则到期时间，0 表示不按时间过期。
...
`3.5.6-alpha.5` 主要用于验证弱网环境下的依赖版本刷新性能、依赖管理页可读性，以及依赖更新提示策略。
