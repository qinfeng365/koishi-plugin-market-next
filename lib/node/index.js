...
var NOT_FOUND_CACHE_TTL = import_koishi4.Time.minute * 5;
var RegistryMetadata = class {
  ...
  async getRegistry(name2, serial = this.currentSerial) {
    ...
    if (probe?.serial === serial && probe.name === name2 && probe.endpoint === this.metadataEndpoint) {
      ...
      logger3.debug(`reuse npm registry route probe payload for ${name2}: endpoint=${probe.endpoint}, probeElapsed=${probe.elapsed}ms`);
      return probe.registry;
    }
    for (let retry = 0; retry <= maxRetry; retry++) {
      const endpoints = this.getRegistryEndpoints();
      logger3.debug(`registry metadata candidates for ${name2}: endpoints=${endpoints.join(", ")}, retry=${retry + 1}/${maxRetry + 1}, concurrency=${this.config.concurrency ?? 4}`);
      try {
        const result = await this.fetchRegistryByRoute(name2, endpoints, serial, (endpoint) => {
          attempts++;
        ...
        if (result.endpoint !== this.metadataEndpoint) {
          logger3.debug(`routed npm registry endpoint for ${name2}: ${result.endpoint}`);
          logger3.info(`npm registry route selected for ${name2}: endpoint=${result.endpoint}, previous=${this.metadataEndpoint}, reason=${result.fallbackReason ?? "same-priority"}, elapsed=${result.elapsed}ms`);
        ...
        logger3.debug(`loaded registry metadata for ${name2} from ${result.endpoint} in ${result.elapsed}ms, attempts=${attempts}, versions=${Object.keys(result.registry.versions).length}`);
        return result.registry;
      } catch (error) {
        lastError = error;
        const detail2 = this.formatError(error);
        failureReasons.push(...getRegistryAttemptReasons(error, detail2.reason));
        logger3.debug(`failed routed registry metadata for ${name2}, attempt=${retry + 1}/${maxRetry + 1}, endpoint=${lastEndpoint}, attempts=${attempts}: ${detail2.error}`);
    ...
    logger3.warn(`failed to fetch registry metadata for ${name2}: ${detail.error}`);
  ...
  getPackage(name2) {
    ...
    if (!this.pkgTasks[name2]) {
      const task = this.fetchPackage(name2, this.currentSerial);
  ...
  getPreferredMetadataEndpoint() {
    ...
    if (stats.failures >= 2 && selectedScore + 1 < primaryScore) {
      logger3.debug(`demote npm metadata endpoint: selected=${endpoint}, selectedScore=${selectedScore.toFixed(1)}, primary=${this.endpoint}, primaryScore=${primaryScore.toFixed(1)}, failures=${stats.failures}, lastFailure=${stats.lastFailureReason ?? "-"}`);
      return this.endpoint;
    }
    return endpoint;
  }
  getRegistryEndpointCandidates() {
    return [this.endpoint, ...this.config.autoRoute === false ? [] : REGISTRY_FALLBACK_ENDPOINTS].filter((endpoint, index, array) => !!endpoint && array.indexOf(endpoint) === index);
  }
  ...
  getRouteScores() {
    ...
    return this.getRegistryEndpointCandidates().map((endpoint) => ({
      endpoint,
      score: this.getRouteScore(endpoint, now),
      fallbackDelay: endpoint === this.endpoint ? this.getFallbackDelay(endpoint) : void 0,
      ...this.routeStats[endpoint]
    }));
  }
  ...
  setRegistryStatus(name2, status, serial = this.currentSerial) {
    if (this.isStale(serial)) return;
    const value = {
  ...
  async fetchPackage(name2, serial = this.currentSerial) {
    try {
      ...
      logger3.warn(error instanceof Error ? error.message : error);
      throw error;
    }
  }
...
var DEFAULT_INSTALL_LOG_RETENTION = import_koishi5.Time.day * 3;
...
var InstallHistoryStore = class {
      ...
      try {
        ...
        await Promise.all(entries.filter((entry) => entry.isFile() && (entry.name.endsWith(".log") || entry.name.endsWith(".log.json"))).map(async (entry) => {
          ...
          try {
            ...
            logger4.debug(`failed to cleanup install log ${path}: ${error instanceof Error ? error.message : error}`);
          }
        }));
      } catch (error) {
        if (error?.code !== "ENOENT") {
          logger4.debug(`failed to cleanup install logs: ${error instanceof Error ? error.message : error}`);
        }
      }
    })().finally(() => {
  ...
  async start(deps, forced, options = {}, changes = []) {
    ...
    await this.writeMetadata().catch((error) => {
      logger4.debug(`failed to write install log metadata ${this.metadataFile}: ${error instanceof Error ? error.message : error}`);
    });
    logger4.info(`dependency install log started: ${file}`);
  }
  emit(type, line) {
  ...
  async finish(result) {
    ...
    if (this.metadata) {
      ...
      await this.writeMetadata().catch((error) => {
        logger4.debug(`failed to finish install log metadata ${this.metadataFile}: ${error instanceof Error ? error.message : error}`);
      });
    }
    logger4.info(`dependency install log saved: ${this.logFile}`);
    this.resetCurrent();
  }
  ...
  getRetention() {
    ...
    if (Number.isFinite(hours) && hours > 0) return Math.max(1, hours) * import_koishi5.Time.hour;
    const legacyRetention = Number(this.config.installLogRetention);
    return Number.isFinite(legacyRetention) && legacyRetention > 0 ? Math.max(import_koishi5.Time.hour, legacyRetention) : DEFAULT_INSTALL_LOG_RETENTION;
  }
  async writeMetadata() {
...
`;
    this.writeTask = this.writeTask.then(() => import_fs7.promises.appendFile(file, text)).catch((error) => {
      logger4.debug(`failed to write install log ${file}: ${error instanceof Error ? error.message : error}`);
    });
  }
  getPath(id) {
  ...
  async readMetadata(id) {
    ...
    try {
      ...
      if (error?.code !== "ENOENT" && !(error instanceof SyntaxError)) {
        logger4.debug(`failed to read install log metadata ${id}: ${error instanceof Error ? error.message : error}`);
      }
    }
  }
...
var Installer = class extends import_koishi6.Service {
  constructor(ctx, config = {}) {
    super(ctx, "installer");
    this.ctx = ctx;
    this.config = config;
    ...
    this.environmentSnapshots = new EnvironmentSnapshotStore(
      ...
      (message) => logger5.warn(message)
    );
    this.localPackageUploads = new LocalPackageUploadStore(ctx.baseDir, (message) => logger5.warn(message));
    ctx.setInterval(() => {
      void this.localPackageUploads.pruneExpired();
    }, import_koishi6.Time.minute * 5);
    ctx.effect(() => () => {
  ...
  async start() {
    ...
    logger5.debug(`registry endpoint initialized: ${this.endpoint}, timeout=${this.config.timeout ?? "default"}, autoRoute=${this.config.autoRoute !== false}`);
    logger5.info(`npm registry endpoint initialized: ${this.endpoint}, timeout=${this.config.timeout ?? "default"}, autoRoute=${this.config.autoRoute !== false}`);
    await this.recordCurrentEnvironmentSnapshot("startup").catch((error) => {
      logger5.warn(`failed to record startup environment snapshot: ${error instanceof Error ? error.message : error}`);
    });
    const dependencies = this.getDeps({ background: false });
    logger5.info(`dependency startup metadata probe scheduled: deps=${Object.keys(dependencies).length}`);
  ...
  markRegistryNotFoundDependency(name2, dependency = this.depCache[name2]) {
    ...
    logger5.info(`dependency classified as unbound local plugin: ${name2}@${dependency.resolved}`);
    return true;
  }
  getLocalDepsSnapshot() {
    const start = Date.now();
    const result = (0, import_koishi6.valueMap)(this.manifest.dependencies, (request) => {
    ...
    for (const name2 of names) {
      try {
        ...
        logger5.debug(`local dependency resolved: ${name2}@${meta.version}, workspace=${!!meta.$workspace}, request=${result[name2].request}`);
      } catch {
        logger5.debug(`local dependency not found before metadata fetch: ${name2}, request=${result[name2].request}`);
      }
      ...
      if (!result[name2].local && !(0, import_semver8.valid)(result[name2].request)) {
        result[name2].invalid = true;
        logger5.debug(`dependency request is not exact semver: ${name2}, request=${result[name2].request}`);
      }
    ...
    logger5.info(`dependency local snapshot ready: total=${names.length}, installed=${installed}, invalid=${invalid}, elapsed=${Date.now() - start}ms`);
    return result;
  }
  async _refreshDependencyMetadata(result = this.depCache, serial = this.metadata.serial) {
    ...
    logger5.debug(`refresh dependency metadata started: total=${names.length}, targets=${targets.length}, concurrency=${this.config.concurrency ?? 4}, registry=${this.endpoint}, autoRoute=${this.config.autoRoute !== false}`);
    const probeName = pickMetadataProbe(targets);
    if (probeName) await this.metadata.ensureEndpoint(probeName, this.metadata.serial);
    logger5.debug(`refresh dependency metadata route ready: probe=${probeName ?? "-"}, selected=${this.metadata.selectedEndpoint}, configured=${this.endpoint}, probed=${this.metadata.hasRouteProbeResult}`);
    await (0, import_p_map.default)(targets, async (name2) => {
      ...
      try {
        ...
        if (versions) {
          ...
          logger5.debug(`dependency latest resolved: ${name2}, resolved=${result[name2].resolved ?? "-"}, latest=${result[name2].latest}, versions=${Object.keys(versions).length}`);
        } else if (!versions && this.metadata.hasRecentNotFound(name2) && this.markRegistryNotFoundDependency(name2, result[name2])) {
          logger5.debug(`dependency npm not-found result reused from cache: ${name2}`);
        } else {
          logger5.debug(`dependency latest unresolved: ${name2}, resolved=${result[name2].resolved ?? "-"}, request=${result[name2].request}`);
        }
      } catch (error) {
        if (this.isStale(serial)) return;
        ...
        if (detail.reason === "not-found" && this.markRegistryNotFoundDependency(name2, result[name2])) {
          ...
          logger5.debug(`dependency metadata refresh skipped after error: ${name2}, reason=${detail.reason}, error=${detail.error}`);
        }
      }
    }, { concurrency: this.config.concurrency ?? 4 });
    logger5.info(`dependency metadata refresh completed: total=${names.length}, targets=${targets.length}, registry=${this.metadata.selectedEndpoint}, elapsed=${Date.now() - start}ms`);
  ...
  refreshDependencyMetadata(wait = false) {
    ...
    if (!this.depTask) {
      ...
      task.then(() => {
        ...
        if (this.depTask === task) this.depTask = void 0;
        logger5.warn(`dependency metadata refresh failed: ${error instanceof Error ? error.message : error}`);
      });
    }
  ...
  async probeDependenciesInBackground(reason = "background") {
    const start = Date.now();
    if (this.depTask) {
      logger5.debug(`reuse running dependency metadata task for ${reason} probe`);
      await this.depTask;
      await this.refreshData();
      logger5.info(`dependency ${reason} probe reused running metadata task: elapsed=${Date.now() - start}ms`);
      return;
    }
    ...
    logger5.info(`dependency ${reason} probe started: deps=${Object.keys(this.manifest.dependencies ?? {}).length}`);
    await this.refreshDependencyMetadata(true);
    await this.refreshData();
    logger5.info(`dependency ${reason} probe completed: deps=${Object.keys(this.manifest.dependencies ?? {}).length}, elapsed=${Date.now() - start}ms`);
  }
  getDeps(options = {}) {
  ...
  async refresh(refresh = false, waitMetadata = false) {
    ...
    logger5.info(`dependency refresh requested by console: deps=${Object.keys(this.manifest.dependencies ?? {}).length}, waitMetadata=${waitMetadata}, elapsed=${Date.now() - start}ms`);
  }
  ...
  async exec(args) {
    ...
    const start = Date.now();
    logger5.info(`run package manager: agent=${name2}${this.agent?.version ? "@" + this.agent.version : ""}, args=${args.join(" ") || "(none)"}, cwd=${this.cwd}, json=${useJson}`);
    return new Promise((resolve11) => {
      if (useJson) args.push("--json");
      const child = (0, import_execa2.default)(name2, args, { cwd: this.cwd });
      this.emitInstallLog("stdout", `package manager started: agent=${name2}${this.agent?.version ? "@" + this.agent.version : ""}`);
      let stderr = "";
      let stdout = "";
      let settled = false;
      const emitStdoutLine = (line) => {
        ...
        try {
          ...
          logger5[levelMap[type] ?? "info"](data);
      ...
      child.on("exit", (code, signal) => {
        logger5.info(`package manager exited: code=${code}, signal=${signal ?? "-"}, elapsed=${Date.now() - start}ms`);
        if (code == null) {
      ...
      child.on("error", (error) => {
        logger5.warn(`package manager failed to start: ${error instanceof Error ? error.message : String(error)}`);
  ...
  async override(deps) {
    ...
    logger5.debug(`override package dependencies: file=${filename}, changes=${formatDeps(deps)}`);
    ...
    logger5.info(`package dependencies updated: changes=${formatDeps(deps)}, total=${Object.keys(this.manifest.dependencies).length}`);
  }
  ...
  async restorePackageManifest(snapshot, deps, reason) {
    ...
    this.depCache = this.getLocalDepsSnapshot();
    this.depMetadataFresh = false;
    logger5.warn(`package dependencies rolled back: reason=${reason}, changes=${formatDeps(deps)}, total=${Object.keys(this.manifest.dependencies ?? {}).length}`);
  }
  _install(options = {}) {
    options ||= {};
    const args = [];
  ...
  _getLocalDeps(override) {
    return (0, import_koishi6.valueMap)(override, (request, name2) => {
      const dep = { request };
      try {
        const meta = loadManifest(name2, this.cwd);
  ...
  async captureCurrentEnvironmentSnapshot(source, operationId) {
    ...
    for (const [name2, request] of Object.entries(manifest.dependencies)) {
      ...
      dependencies[name2] = {
        ...
        invalid: !local[name2]?.local && !(0, import_semver8.valid)(normalizedRequest)
      };
    }
  ...
  async _installLocked(deps, forced, beforeReload, options = {}) {
    ...
    await this.recordCurrentEnvironmentSnapshot("external").catch((error) => {
      logger5.warn(`failed to record pre-operation environment snapshot: ${error instanceof Error ? error.message : error}`);
    });
    await this.startInstallLog(deps, forced, options, changes).catch((error) => {
      this.installHistory.resetCurrent();
      logger5.warn(`failed to start dependency install log: ${error instanceof Error ? error.message : error}`);
    });
    logger5.info(`dependency install requested: deps=${formatDeps(deps)}, forced=${!!forced}, installEndpoint=${options.installEndpoint || "(default)"}`);
    try {
      ...
      logger5.debug(`dependency install local state: ${formatLocalDeps(localDeps)}`);
      ...
      if (needsPackageManager) {
        ...
        if (unresolved.length) {
          logger5.info(`resolve possible local plugin sources before package manager: ${unresolved.join(", ")}`);
      ...
      for (const name2 in localDeps) {
        ...
        shouldReload = true;
        logger5.debug(`dependency changed may require full reload: ${name2}, previous=${localDeps[name2]?.resolved ?? "-"}, current=${newDeps[name2]?.resolved ?? "-"}`);
      }
      if (beforeReload) {
        logger5.debug("run pre-reload dependency hook");
        await beforeReload();
      }
      await this.refreshData();
      await this.recordCurrentEnvironmentSnapshot("operation", this.installHistory.currentId).catch((error) => {
        logger5.warn(`failed to record dependency environment snapshot: ${error instanceof Error ? error.message : error}`);
      });
      logger5.info(`dependency install completed: deps=${formatDeps(deps)}, forced=${!!needsPackageManager}, fullReload=${shouldReload}, elapsed=${Date.now() - start}ms`);
      if (shouldReload) {
        this.emitInstallLog("stdout", `full reload scheduled in ${FULL_RELOAD_DELAY}ms`);
        logger5.info(`dependency install triggers full reload after ${FULL_RELOAD_DELAY}ms`);
        setTimeout(() => {
      ...
      await this.finishInstallLog(logResult).catch((error) => {
        logger5.warn(`failed to finish dependency install log: ${error instanceof Error ? error.message : error}`);
      });
    }
  }
  async withInstallLock(description2, callback) {
    ...
    if (this.installActive) logger5.info(`dependency install queued: ${description2}`);
    await previous;
  ...
  async applyEnvironmentSnapshot(id, options = {}) {
    ...
    return this.withInstallLock(`environmentSnapshot=${id}`, async () => {
      ...
      logger5.info(`environment snapshot restore requested: target=${id}, changes=${formatDeps(changes)}`);
...
((Installer2) => {
  Installer2.Config = import_koishi6.Schema.object({
    endpoint: import_koishi6.Schema.string().role("link"),
    timeout: import_koishi6.Schema.number().role("time").default(import_koishi6.Time.second * 5),
    autoRoute: import_koishi6.Schema.boolean().default(true),
    retry: import_koishi6.Schema.number().min(0).max(5).step(1).default(1),
    concurrency: import_koishi6.Schema.number().min(1).max(16).step(1).default(4),
    installLogRetentionHours: import_koishi6.Schema.number().min(1).max(24 * 365).step(1).default(72)
  });
})(Installer || (Installer = {}));
...
var FIRST_PAYLOAD_TIMEOUT = import_koishi7.Time.second * 1.5;
var FAST_ROUTE_THRESHOLD = import_koishi7.Time.second * 0.5;
var MAX_CACHE_ENTRIES = 3;
var CACHE_ENTRY_TTL = import_koishi7.Time.day * 30;
...
function formatAge(age) {
  ...
  if (age < import_koishi7.Time.second) return `${Math.max(0, Math.round(age))}ms`;
  if (age < import_koishi7.Time.minute) return `${Math.round(age / import_koishi7.Time.second)}s`;
  if (age < import_koishi7.Time.hour) return `${Math.round(age / import_koishi7.Time.minute)}m`;
  if (age < import_koishi7.Time.day) return `${Math.round(age / import_koishi7.Time.hour)}h`;
  return `${Math.round(age / import_koishi7.Time.day)}d`;
}
...
function getRouteCooldown(failures = 0) {
  ...
  if (failures === 1) return import_koishi7.Time.minute;
  if (failures === 2) return import_koishi7.Time.minute * 5;
  if (failures === 3) return import_koishi7.Time.minute * 30;
  if (failures === 4) return import_koishi7.Time.hour * 4;
  return import_koishi7.Time.hour * 12;
}
...
var MarketDiskCache = class {
  ...
  async load(serial) {
    ...
    try {
      ...
      if (staleEndpoints.length) {
        this.options.log("debug", `market disk cache has ${staleEndpoints.length} stale entries (>${CACHE_ENTRY_TTL / import_koishi8.Time.day}d), will prune on next write: ${staleEndpoints.join(", ")}`);
      }
  ...
  getScore(cache2) {
    ...
    if (age <= import_koishi8.Time.hour * 12) score += 3;
    else if (age <= import_koishi8.Time.day * 3) score += 1;
    else score -= 1;
...
var MarketRouter = class {
  ...
  getScore(endpoint, now = Date.now()) {
    ...
    if (cached) {
      ...
      score += age <= import_koishi9.Time.day ? 1.5 : 0.5;
    }
    return scoreRouteHealth(stats, {
      baseScore: score,
  ...
  restoreStats(persisted) {
    for (const [endpoint, stats] of Object.entries(persisted)) {
      if (!stats) continue;
      const hasRecentSuccess = stats.lastSuccess && Date.now() - stats.lastSuccess < import_koishi9.Time.day;
      this.stats[endpoint] = {
...
var MarketProvider2 = class extends MarketProvider {
  ...
  log(level, message) {
    ...
    const logger6 = this.ctx.logger("market");
    if (level === "debug") {
      logger6.info(`[debug] ${message}`);
    } else {
      logger6[level](message);
    }
  }
};
((MarketProvider3) => {
  MarketProvider3.Config = import_koishi10.Schema.object({
    endpoint: import_koishi10.Schema.string().role("link").default(DEFAULT_ENDPOINT),
    timeout: import_koishi10.Schema.number().role("time").default(import_koishi10.Time.second * 30),
    proxyAgent: import_koishi10.Schema.string().role("link"),
    autoRoute: import_koishi10.Schema.boolean().default(true),
    logLevel: import_koishi10.Schema.union(logLevels.map((level) => import_koishi10.Schema.const(level))).default("warn")
  });
...
var CACHE_TTL = import_koishi11.Time.minute * 10;
var DAY = import_koishi11.Time.day;
...
function applyChatLunaTool(ctx, config = {}) {
  ...
  const logger6 = ctx.logger("market");
  logger6.debug("ChatLuna market search tool is enabled; waiting for chatluna service");
...
async function fetchIndex(ctx, config) {
  ...
  const http = ctx.http.extend({
    endpoint,
    timeout: config.search?.timeout ?? import_koishi11.Time.second * 30,
...
function setupIdleProbe(ctx, config) {
  ...
  const getDelay = () => Math.max(0, config.idleProbeDelay ?? import_koishi12.Time.minute * 5);
  const getBootDelay = () => Math.max(0, config.idleProbeBootDelay ?? import_koishi12.Time.minute);
  const getInterval = () => Math.max(0, config.idleProbeInterval ?? import_koishi12.Time.hour * 6);
  const runProbe = async () => {
    clearIdleTimer();
    if (!ctx.scope.isActive) return;
    if (getClientCount()) return;
    if (ctx.installer.isInstalling) {
      logger6.debug("skip idle background probe because dependency install is active");
      schedule(getDelay());
      return;
    }
    ...
    const retryWait = lastFailure ? Math.min(import_koishi12.Time.minute * 5, getInterval()) - (Date.now() - lastFailure) : 0;
    if (!lastProbe && retryWait > 0) {
      logger6.debug(`skip idle background probe because retry gate is active: remaining=${retryWait}ms`);
      schedule(retryWait);
      return;
    }
    const intervalWait = lastProbe ? getInterval() - (Date.now() - lastProbe) : 0;
    if (intervalWait > 0) {
      logger6.debug(`skip idle background probe because interval gate is active: remaining=${intervalWait}ms`);
    ...
    logger6.info(`idle background probe started: clients=0, delay=${getDelay()}ms, interval=${getInterval()}ms`);
    try {
      ...
      if (succeeded) {
        lastProbe = Date.now();
        lastFailure = 0;
        logger6.info(`idle background probe completed: elapsed=${Date.now() - probeStartedAt}ms`);
      } else {
        ...
        logger6.warn(`idle background probe failed: ${reason instanceof Error ? reason.message : reason}`);
      }
    } catch (error) {
      lastFailure = Date.now();
      logger6.warn(`idle background probe failed: ${error instanceof Error ? error.message : error}`);
    } finally {
      running = false;
      if (!getClientCount()) schedule(lastProbe ? getInterval() : Math.min(import_koishi12.Time.minute * 5, getInterval()));
    }
  };
  const schedule = (delay = getDelay()) => {
    ...
    logger6.debug(`idle background probe scheduled: delay=${Math.max(0, delay)}ms`);
  };
  ctx.on("console/connection", () => {
    if (getClientCount()) {
      clearIdleTimer();
      logger6.debug(`idle background probe cancelled: clients=${getClientCount()}`);
    } else {
      schedule();
    }
  });
...
async function assertNoDirectBundleCycles(ctx, packageName, members) {
  ...
  for (const member of members) {
    try {
      ...
      const version = (0, import_semver9.maxSatisfying)(versions, member.version, { includePrerelease: true });
      if (!version) continue;
...
var AVATAR_CACHE_TTL = import_koishi14.Time.day * 7;
var AVATAR_CACHE_SWEEP_INTERVAL = import_koishi14.Time.hour;
...
var MarketSilentRuleType = import_koishi15.Schema.union([
  import_koishi15.Schema.const("preview").description("\u72B6\u6001\uFF1A\u9884\u89C8\u7248\u63D2\u4EF6"),
  import_koishi15.Schema.const("insecure").description("\u72B6\u6001\uFF1A\u4E0D\u5B89\u5168\u63D2\u4EF6"),
  import_koishi15.Schema.const("bundle").description("\u72B6\u6001\uFF1A\u63D2\u4EF6\u5305"),
  import_koishi15.Schema.const("created-before").description("\u521B\u5EFA\u65F6\u95F4\uFF1A\u65E9\u4E8E\u6307\u5B9A\u65E5\u671F"),
  import_koishi15.Schema.const("created-after").description("\u521B\u5EFA\u65F6\u95F4\uFF1A\u665A\u4E8E\u6307\u5B9A\u65E5\u671F"),
  import_koishi15.Schema.const("updated-before").description("\u66F4\u65B0\u65F6\u95F4\uFF1A\u65E9\u4E8E\u6307\u5B9A\u65E5\u671F"),
  import_koishi15.Schema.const("updated-after").description("\u66F4\u65B0\u65F6\u95F4\uFF1A\u665A\u4E8E\u6307\u5B9A\u65E5\u671F"),
  import_koishi15.Schema.const("created-within").description("\u521B\u5EFA\u65F6\u95F4\uFF1A\u6700\u8FD1 N \u5929\u5185"),
  import_koishi15.Schema.const("updated-within").description("\u66F4\u65B0\u65F6\u95F4\uFF1A\u6700\u8FD1 N \u5929\u5185"),
  import_koishi15.Schema.const("custom").description("\u81EA\u5B9A\u4E49\u9AD8\u7EA7\u6761\u4EF6")
]);
var MarketSilentRules = import_koishi15.Schema.array(import_koishi15.Schema.object({
  type: MarketSilentRuleType.default("preview").description("\u89C4\u5219\u7C7B\u578B"),
  value: import_koishi15.Schema.string().default("").description("\u89C4\u5219\u503C\u3002\u72B6\u6001\u7C7B\u7559\u7A7A\uFF1B\u65E5\u671F\u7C7B\u586B\u5199 YYYY-MM-DD\uFF0C\u4F8B\u5982 2024-01-01\uFF1B\u6700\u8FD1 N \u5929\u586B\u5199\u6570\u5B57\uFF0C\u4F8B\u5982 30\uFF1B\u81EA\u5B9A\u4E49\u89C4\u5219\u586B\u5199\u641C\u7D22\u6761\u4EF6\uFF0C\u4F8B\u5982 category:adapter\u3002"),
  note: import_koishi15.Schema.string().default("").description("\u5907\u6CE8"),
  enabled: import_koishi15.Schema.boolean().default(true).description("\u662F\u5426\u542F\u7528")
...
var Config = import_koishi15.Schema.object({
  frontendMode: import_koishi15.Schema.union([
    import_koishi15.Schema.const("performance").description("\u6027\u80FD\u6A21\u5F0F"),
    import_koishi15.Schema.const("polished").description("\u7CBE\u81F4\u6A21\u5F0F")
  ]).role("radio").default("performance").description("Frontend display mode."),
  depsLayout: import_koishi15.Schema.union([
    import_koishi15.Schema.const("grid").description("\u7F51\u683C"),
    import_koishi15.Schema.const("list").description("\u5217\u8868")
  ]).role("radio").default("grid").description("Dependencies page layout."),
  idleProbe: import_koishi15.Schema.boolean().default(true).description("Run dependency and market metadata probes while Console is idle."),
  idleProbeDelay: import_koishi15.Schema.number().role("time").default(import_koishi15.Time.minute * 5).description("How long Console must stay idle before the background probe starts."),
  idleProbeBootDelay: import_koishi15.Schema.number().role("time").default(import_koishi15.Time.minute).description("Minimum delay after startup before idle probing is allowed."),
  idleProbeInterval: import_koishi15.Schema.number().role("time").default(import_koishi15.Time.hour * 6).description("Minimum interval between idle background probes."),
  bulkMode: import_koishi15.Schema.boolean().default(false).hidden().description("Batch operation mode for dependency changes."),
  removeConfig: import_koishi15.Schema.union([
    import_koishi15.Schema.const(void 0).description("Ask every time"),
    import_koishi15.Schema.const(true).description("Always remove plugin config"),
    import_koishi15.Schema.const(false).description("Never remove plugin config")
  ]).hidden().description("Whether to remove existing plugin config when uninstalling a plugin."),
  updateIgnoredPackages: import_koishi15.Schema.string().role("textarea").hidden().description("Dependency package names that should not be checked for updates. One package per line, or separated by commas."),
  updateIgnoreDuration: import_koishi15.Schema.number().role("time").default(0).hidden().description("Default duration for ignoring one update. 0 means no time-based expiry."),
  updateIgnoreVersions: import_koishi15.Schema.number().min(1).max(20).step(1).default(1).hidden().description("How many consecutive newer versions should be ignored after ignoring one update."),
  updateIgnorePrerelease: import_koishi15.Schema.boolean().default(false).hidden().description("Ignore alpha, beta, rc and other prerelease versions when checking updates."),
  collapsedGroups: import_koishi15.Schema.dict(Boolean).hidden(),
  registry: installer_default.Config,
  search: market_default.Config,
  chatlunaTool: import_koishi15.Schema.boolean().default(false).description("Enable ChatLuna plugin market query tool."),
  marketSilentFilters: import_koishi15.Schema.string().role("textarea").hidden().description("Legacy permanent silent filters."),
  marketSilentStatusRules: import_koishi15.Schema.array(import_koishi15.Schema.any()).hidden(),
  marketSilentDateRules: import_koishi15.Schema.array(import_koishi15.Schema.any()).hidden(),
  marketSilentRecentRules: import_koishi15.Schema.array(import_koishi15.Schema.any()).hidden(),
  marketSilentCustomRules: import_koishi15.Schema.array(import_koishi15.Schema.any()).hidden(),
...
function apply(ctx, config = {}) {
  ...
  ctx.inject(["installer"], (ctx2) => {
    ...
    ctx2.i18n.define("en-US", require_message_en_US());
    ctx2.command("plugin.install <name>", { authority: 4 }).alias(".i").action(async ({ session }, name2) => {
      ...
      ctx2.loader.envData.message = {
        ...(0, import_koishi16.pick)(session, ["sid", "channelId", "guildId", "isDirect"]),
    ...
    ctx2.command("plugin.upgrade [name...]", { authority: 4 }).alias(".update", ".up").option("self", "-s, --koishi").option("force", "-f, --force").action(async ({ session, options }, ...names) => {
      await ctx2.installer.refresh(true, true);
      const deps = await ctx2.installer.getDeps({ background: false });
      const requested = names.length ? names.map((name2) => {
        const candidates = ctx2.installer.resolveName(name2);
      ...
      const runtimeData = activeDataStore ? await activeDataStore.get() : await readMarketDataStore(ctx2);
      const policy = {
      ...
      const updates = Array.from(new Set(requested)).flatMap((name2) => {
        ...
        const versions = Object.keys(ctx2.installer.fullCache[name2] ?? {});
      ...
      try {
        const code = await ctx2.installer.install(installDeps, void 0, () => ensurePluginConfigs(ctx2, installNames));
    ...
    ctx2.command("plugin.clear-avatar-cache", { authority: 4 }).action(async ({ session }) => {
      const { memory, disk } = await clearAvatarCacheStorage(ctx2);
  ...
  ctx.inject(["console", "installer", "server"], (ctx2) => {
    ...
    ctx2.console.addListener("market/install", async (deps, forced, options) => {
      options ||= {};
      const installNames = Object.entries(deps).filter(([, version]) => version).map(([name2]) => name2).filter((name2) => name2 !== SELF_PACKAGE3);
      const code = await ctx2.installer.install(deps, forced, installNames.length ? () => ensurePluginConfigs(ctx2, installNames) : void 0, options);
      if (!code) {
        await ensurePluginConfigs(ctx2, installNames);
      }
      await Promise.all([
    ...
    ctx2.console.addListener("market/install-bundle", async (request, forced, options) => {
      options ||= {};
      return installBundle(ctx2, dataStore, request, forced, options);
    }, { authority: 4 });
    ctx2.console.addListener("market/install-fallback-candidate", async (failedEndpoint) => {
      return ctx2.installer.getInstallFallbackCandidate(failedEndpoint);
    }, { authority: 4 });
    ctx2.console.addListener("market/install-history", async (limit) => {
      return ctx2.installer.getInstallHistory(limit);
    }, { authority: 4 });
    ctx2.console.addListener("market/install-history-detail", async (id) => {
      return ctx2.installer.getInstallLogDetail(id);
    }, { authority: 4 });
    ctx2.console.addListener("market/local-package-upload-start", async (request) => {
      return ctx2.installer.startLocalPackageUpload(request);
    }, { authority: 4 });
    ctx2.console.addListener("market/local-package-upload-chunk", async (request) => {
      return ctx2.installer.appendLocalPackageUpload(request);
    }, { authority: 4 });
    ctx2.console.addListener("market/local-package-upload-finish", async (request) => {
      return ctx2.installer.finishLocalPackageUpload(request);
    }, { authority: 4 });
    ctx2.console.addListener("market/local-package-upload-commit", async (uploadId) => {
      return ctx2.installer.commitLocalPackageUpload(uploadId);
    }, { authority: 4 });
    ctx2.console.addListener("market/local-package-upload-cancel", async (uploadId) => {
      return ctx2.installer.cancelLocalPackageUpload(uploadId);
    }, { authority: 4 });
    ctx2.console.addListener("market/prepare-local-binding", async (name2) => {
      return ctx2.installer.prepareLocalBinding(name2);
    }, { authority: 4 });
    ctx2.console.addListener("market/environment-snapshots", async () => {
      return ctx2.installer.getEnvironmentSnapshots();
    }, { authority: 4 });
    ctx2.console.addListener("market/environment-snapshot-preview", async (id) => {
      return ctx2.installer.getEnvironmentSnapshotPreview(id);
    }, { authority: 4 });
    ctx2.console.addListener("market/environment-snapshot-apply", async (id, options) => {
      const code = await ctx2.installer.applyEnvironmentSnapshot(id, options);
      await Promise.all([
        ctx2.get("console")?.refresh("dependencies"),
        ctx2.get("console")?.refresh("registry"),
        ctx2.get("console")?.refresh("packages")
    ...
    ctx2.console.addListener("market/remove-bundle-configs", async (request) => {
      return removeBundleConfigs(ctx2, request);
    }, { authority: 4 });
    ctx2.console.addListener("market/update-config", async (patch) => {
      return updateMarketNextConfig(ctx2, config, patch);
    }, { authority: 4 });
    ctx2.console.addListener("market/update-data", async (patch) => {
    ...
    ctx2.console.addListener("market/index", async (request) => {
      const snapshot = await ctx2.console.services.market?.getSnapshot?.();
      if (!snapshot || request?.transport !== "http-gzip") return snapshot;
      return marketSnapshotTransport.create(snapshot);
    }, { authority: 4 });
    ctx2.console.addListener("market/lookup", async (request) => {
      return lookupMarket(ctx2.console.services.market, request);
    }, { authority: 4 });
    ctx2.console.addListener("market/registry", async (names) => {
      const entries = await (0, import_p_map2.default)(names, async (name2) => {
        try {
          ...
          ctx2.logger("market").debug(`skip registry metadata for ${name2}: ${error instanceof Error ? error.message : error}`);
        }
    ...
    ctx2.console.addListener("market/avatar", async (key, url) => {
      try {
        return await fetchAvatar(ctx2, key, url);
      } catch (error) {
        ctx2.logger("market").debug(`avatar fetch failed: ${error instanceof Error ? error.message : error}`);
      }
    }, { authority: 4 });
    ctx2.on("ready", () => {
