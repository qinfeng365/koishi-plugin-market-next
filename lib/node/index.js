...
var require_schema_zh_CN = __commonJS({
  "src/node/locales/schema.zh-CN.yml"(exports2, module2) {
    module2.exports = { registry: { $description: "\u63D2\u4EF6\u6E90\u8BBE\u7F6E", endpoint: "\u63D2\u4EF6\u7684\u4E0B\u8F7D\u6E90\u3002\u9ED8\u8BA4\u8DDF\u968F\u5F53\u524D\u9879\u76EE\u7684 npm config\u3002", timeout: "\u83B7\u53D6\u63D2\u4EF6\u6570\u636E\u7684\u8D85\u65F6\u65F6\u95F4\u3002" }, search: { $description: "\u641C\u7D22\u8BBE\u7F6E", endpoint: "\u7528\u4E8E\u641C\u7D22\u63D2\u4EF6\u5E02\u573A\u7684\u7F51\u5740\u3002\u9ED8\u8BA4\u4F7F\u7528 t4wefan \u955C\u50CF\u3002", timeout: "\u641C\u7D22\u63D2\u4EF6\u5E02\u573A\u7684\u8D85\u65F6\u65F6\u95F4\u3002", proxyAgent: "\u7528\u4E8E\u641C\u7D22\u63D2\u4EF6\u5E02\u573A\u7684\u4EE3\u7406\u3002", autoRoute: "\u5F53\u524D\u5E02\u573A\u6E90\u5931\u8D25\u65F6\u81EA\u52A8\u5C1D\u8BD5\u5907\u7528\u5E02\u573A\u6E90\u3002", logLevel: "\u63D2\u4EF6\u5E02\u573A\u8C03\u8BD5\u65E5\u5FD7\u7EA7\u522B\u3002silent \u5173\u95ED\u65E5\u5FD7\uFF0Cdebug \u8F93\u51FA\u6700\u8BE6\u7EC6\u3002" }, chatlunaTool: "\u542F\u7528 ChatLuna \u63D2\u4EF6\u5E02\u573A\u67E5\u8BE2\u5DE5\u5177\u3002\u542F\u7528\u540E\uFF0C\u82E5\u5F53\u524D Koishi \u540C\u65F6\u5B89\u88C5\u4E86 ChatLuna\uFF0C\u4F1A\u6CE8\u518C\u53EA\u8BFB\u5DE5\u5177 koishi_plugin_market_search\u3002" };
  }
});
...
var MarketProvider2 = class extends MarketProvider {
  ...
  async fetchEndpoint(endpoint, index, total, serial, warnFailure = true) {
  ...
  async applyDiskCache(serial) {
    try {
      const cache2 = JSON.parse(await import_fs2.promises.readFile(this.cacheFile, "utf8"));
      ...
      this.cacheMeta = { endpoint: cache2.endpoint, fetchedAt: cache2.fetchedAt };
      this.log("info", `loaded market index from disk cache: ${this.scanner.total}/${cache2.result.objects.length} objects, endpoint=${cache2.endpoint}, cachedAt=${new Date(cache2.fetchedAt).toISOString()}`);
      return true;
    } catch (error) {
  ...
  async writeDiskCache(cache2) {
    ...
    try {
      ...
      this.log("debug", `wrote market disk cache: endpoint=${cache2.endpoint}, objects=${cache2.result.objects.length}`);
    } catch (error) {
