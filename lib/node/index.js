...
var Installer = class extends import_koishi7.Service {
  constructor(ctx, config = {}) {
    super(ctx, "installer");
    this.ctx = ctx;
    this.config = config;
    ...
    this.environmentSnapshots = new EnvironmentSnapshotStore(
      ...
      (message) => logger6.warn(message)
    );
    this.localPackageUploads = new LocalPackageUploadStore(ctx.baseDir, (message) => logger6.warn(message));
    ctx.setInterval(() => {
      void this.localPackageUploads.pruneExpired();
    }, import_koishi7.Time.minute * 5);
    ctx.effect(() => () => {
  ...
  async start() {
    ...
    logger6.debug(`registry endpoint initialized: ${this.endpoint}, timeout=${this.config.timeout ?? "default"}, autoRoute=${this.config.autoRoute !== false}`);
    logger6.info(`npm registry endpoint initialized: ${this.endpoint}, timeout=${this.config.timeout ?? "default"}, autoRoute=${this.config.autoRoute !== false}`);
    await this.recordCurrentEnvironmentSnapshot("startup").catch((error) => {
      logger6.warn(`failed to record startup environment snapshot: ${error instanceof Error ? error.message : error}`);
    });
    const dependencies = this.getDeps({ background: false });
    logger6.info(`dependency startup metadata probe scheduled: deps=${Object.keys(dependencies).length}`);
  ...
  markRegistryNotFoundDependency(name2, dependency = this.depCache[name2]) {
    ...
    logger6.info(`dependency classified as unbound local plugin: ${name2}@${dependency.resolved}`);
    return true;
  }
  getLocalDepsSnapshot() {
    const start = Date.now();
    const result = (0, import_koishi7.valueMap)(this.manifest.dependencies, (request) => {
    ...
    for (const name2 of names) {
      try {
        ...
        logger6.debug(`local dependency resolved: ${name2}@${meta.version}, workspace=${!!meta.$workspace}, request=${result[name2].request}`);
      } catch {
        logger6.debug(`local dependency not found before metadata fetch: ${name2}, request=${result[name2].request}`);
      }
      ...
      if (!result[name2].local && !(0, import_semver8.valid)(result[name2].request)) {
        ...
        logger6.debug(`dependency request is not exact semver: ${name2}, request=${result[name2].request}`);
      }
    ...
    logger6.info(`dependency local snapshot ready: total=${names.length}, installed=${installed}, invalid=${invalid}, elapsed=${Date.now() - start}ms`);
    return result;
  }
  async _refreshDependencyMetadata(result = this.depCache, serial = this.metadata.serial) {
    ...
    logger6.debug(`refresh dependency metadata started: total=${names.length}, targets=${targets.length}, concurrency=${this.config.concurrency ?? 4}, registry=${this.endpoint}, autoRoute=${this.config.autoRoute !== false}`);
    const probeName = pickMetadataProbe(targets);
    if (probeName) await this.metadata.ensureEndpoint(probeName, this.metadata.serial);
    logger6.debug(`refresh dependency metadata route ready: probe=${probeName ?? "-"}, selected=${this.metadata.selectedEndpoint}, configured=${this.endpoint}, probed=${this.metadata.hasRouteProbeResult}`);
    await (0, import_p_map.default)(targets, async (name2) => {
      ...
      try {
        ...
        if (versions) {
          ...
          logger6.debug(`dependency latest resolved: ${name2}, resolved=${result[name2].resolved ?? "-"}, latest=${result[name2].latest}, versions=${Object.keys(versions).length}`);
        } else if (!versions && this.metadata.hasRecentNotFound(name2) && this.markRegistryNotFoundDependency(name2, result[name2])) {
          logger6.debug(`dependency npm not-found result reused from cache: ${name2}`);
        } else {
          logger6.debug(`dependency latest unresolved: ${name2}, resolved=${result[name2].resolved ?? "-"}, request=${result[name2].request}`);
        }
      } catch (error) {
        if (this.isStale(serial)) return;
        ...
        if (detail.reason === "not-found" && this.markRegistryNotFoundDependency(name2, result[name2])) {
          ...
          logger6.debug(`dependency metadata refresh skipped after error: ${name2}, reason=${detail.reason}, error=${detail.error}`);
        }
      }
    }, { concurrency: this.config.concurrency ?? 4 });
    logger6.info(`dependency metadata refresh completed: total=${names.length}, targets=${targets.length}, registry=${this.metadata.selectedEndpoint}, elapsed=${Date.now() - start}ms`);
  ...
  refreshDependencyMetadata(wait = false) {
    ...
    if (!this.depTask) {
      ...
      task.then(() => {
        ...
        if (this.depTask === task) this.depTask = void 0;
        logger6.warn(`dependency metadata refresh failed: ${error instanceof Error ? error.message : error}`);
      });
    }
  ...
  async probeDependenciesInBackground(reason = "background") {
    const start = Date.now();
    if (this.depTask) {
      logger6.debug(`reuse running dependency metadata task for ${reason} probe`);
      await this.depTask;
      await this.refreshData();
      logger6.info(`dependency ${reason} probe reused running metadata task: elapsed=${Date.now() - start}ms`);
      return;
    }
    ...
    logger6.info(`dependency ${reason} probe started: deps=${Object.keys(this.manifest.dependencies ?? {}).length}`);
    await this.refreshDependencyMetadata(true);
    await this.refreshData();
    logger6.info(`dependency ${reason} probe completed: deps=${Object.keys(this.manifest.dependencies ?? {}).length}, elapsed=${Date.now() - start}ms`);
  }
  getDeps(options = {}) {
  ...
  async refresh(refresh = false, waitMetadata = false) {
    ...
    logger6.info(`dependency refresh requested by console: deps=${Object.keys(this.manifest.dependencies ?? {}).length}, waitMetadata=${waitMetadata}, elapsed=${Date.now() - start}ms`);
  }
  ...
  async override(deps) {
    ...
    logger6.debug(`override package dependencies: file=${filename}, changes=${formatDeps(deps)}`);
    ...
    logger6.info(`package dependencies updated: changes=${formatDeps(deps)}, total=${Object.keys(this.manifest.dependencies).length}`);
  }
  ...
  async restorePackageManifest(snapshot, deps, reason) {
    ...
    this.depCache = this.getLocalDepsSnapshot();
    this.depMetadataFresh = false;
    logger6.warn(`package dependencies rolled back: reason=${reason}, changes=${formatDeps(deps)}, total=${Object.keys(this.manifest.dependencies ?? {}).length}`);
  }
  _install(options = {}) {
    options ||= {};
    const args = [];
  ...
  _getLocalDeps(override) {
    return (0, import_koishi7.valueMap)(override, (request, name2) => {
      const dep = { request };
      try {
        const meta = loadManifest(name2, this.cwd);
  ...
  async _installLocked(deps, forced, beforeReload, options = {}) {
    ...
    await this.recordCurrentEnvironmentSnapshot("external").catch((error) => {
      logger6.warn(`failed to record pre-operation environment snapshot: ${error instanceof Error ? error.message : error}`);
    });
    await this.startInstallLog(deps, forced, options, changes).catch((error) => {
      this.installHistory.resetCurrent();
      logger6.warn(`failed to start dependency install log: ${error instanceof Error ? error.message : error}`);
    });
    logger6.info(`dependency install requested: deps=${formatDeps(deps)}, forced=${!!forced}, installEndpoint=${options.installEndpoint || "(default)"}`);
    try {
      ...
      logger6.debug(`dependency install local state: ${formatLocalDeps(localDeps)}`);
      ...
      if (needsPackageManager) {
        ...
        if (unresolved.length) {
          logger6.info(`resolve possible local plugin sources before package manager: ${unresolved.join(", ")}`);
      ...
      for (const name2 in localDeps) {
        ...
        shouldReload = true;
        logger6.debug(`dependency changed may require full reload: ${name2}, previous=${localDeps[name2]?.resolved ?? "-"}, current=${newDeps[name2]?.resolved ?? "-"}`);
      }
      if (beforeReload) {
        logger6.debug("run pre-reload dependency hook");
        await beforeReload();
      }
      await this.refreshData();
      await this.recordCurrentEnvironmentSnapshot("operation", this.installHistory.currentId).catch((error) => {
        logger6.warn(`failed to record dependency environment snapshot: ${error instanceof Error ? error.message : error}`);
      });
      logger6.info(`dependency install completed: deps=${formatDeps(deps)}, forced=${!!needsPackageManager}, fullReload=${shouldReload}, elapsed=${Date.now() - start}ms`);
      if (shouldReload) {
        this.emitInstallLog("stdout", `full reload scheduled in ${FULL_RELOAD_DELAY}ms`);
        logger6.info(`dependency install triggers full reload after ${FULL_RELOAD_DELAY}ms`);
        setTimeout(() => {
      ...
      await this.finishInstallLog(logResult).catch((error) => {
        logger6.warn(`failed to finish dependency install log: ${error instanceof Error ? error.message : error}`);
      });
    }
  }
  async withInstallLock(description2, callback) {
    ...
    if (this.installActive) logger6.info(`dependency install queued: ${description2}`);
    await previous;
  ...
  async applyEnvironmentSnapshot(id, options = {}) {
    ...
    return this.withInstallLock(`environmentSnapshot=${id}`, async () => {
      ...
      logger6.info(`environment snapshot restore requested: target=${id}, changes=${formatDeps(changes)}`);
...
((Installer2) => {
  Installer2.Config = import_koishi7.Schema.object({
    endpoint: import_koishi7.Schema.string().role("link"),
    timeout: import_koishi7.Schema.number().role("time").default(import_koishi7.Time.second * 5),
    autoRoute: import_koishi7.Schema.boolean().default(true),
    retry: import_koishi7.Schema.number().min(0).max(5).step(1).default(1),
    concurrency: import_koishi7.Schema.number().min(1).max(16).step(1).default(4),
    installLogRetentionHours: import_koishi7.Schema.number().min(1).max(24 * 365).step(1).default(72)
  });
})(Installer || (Installer = {}));
...
var FIRST_PAYLOAD_TIMEOUT = import_koishi8.Time.second * 1.5;
var FAST_ROUTE_THRESHOLD = import_koishi8.Time.second * 0.5;
var MAX_CACHE_ENTRIES = 3;
var CACHE_ENTRY_TTL = import_koishi8.Time.day * 30;
...
function formatAge(age) {
  ...
  if (age < import_koishi8.Time.second) return `${Math.max(0, Math.round(age))}ms`;
  if (age < import_koishi8.Time.minute) return `${Math.round(age / import_koishi8.Time.second)}s`;
  if (age < import_koishi8.Time.hour) return `${Math.round(age / import_koishi8.Time.minute)}m`;
  if (age < import_koishi8.Time.day) return `${Math.round(age / import_koishi8.Time.hour)}h`;
  return `${Math.round(age / import_koishi8.Time.day)}d`;
}
...
function getRouteCooldown(failures = 0) {
  ...
  if (failures === 1) return import_koishi8.Time.minute;
  if (failures === 2) return import_koishi8.Time.minute * 5;
  if (failures === 3) return import_koishi8.Time.minute * 30;
  if (failures === 4) return import_koishi8.Time.hour * 4;
  return import_koishi8.Time.hour * 12;
}
...
var MarketDiskCache = class {
  ...
  async load(serial) {
    ...
    try {
      ...
      if (staleEndpoints.length) {
        this.options.log("debug", `market disk cache has ${staleEndpoints.length} stale entries (>${CACHE_ENTRY_TTL / import_koishi9.Time.day}d), will prune on next write: ${staleEndpoints.join(", ")}`);
      }
  ...
  getScore(cache2) {
    ...
    if (age <= import_koishi9.Time.hour * 12) score += 3;
    else if (age <= import_koishi9.Time.day * 3) score += 1;
    else score -= 1;
...
var MarketRouter = class {
  ...
  getScore(endpoint, now = Date.now()) {
    ...
    if (cached) {
      ...
      score += age <= import_koishi10.Time.day ? 1.5 : 0.5;
    }
    return scoreRouteHealth(stats, {
      baseScore: score,
  ...
  restoreStats(persisted) {
    for (const [endpoint, stats] of Object.entries(persisted)) {
      if (!stats) continue;
      const hasRecentSuccess = stats.lastSuccess && Date.now() - stats.lastSuccess < import_koishi10.Time.day;
      this.stats[endpoint] = {
...
var MarketProvider2 = class extends MarketProvider {
  ...
  log(level, message) {
    ...
    const logger7 = this.ctx.logger("market");
    if (level === "debug") {
      logger7.info(`[debug] ${message}`);
    } else {
      logger7[level](message);
    }
  }
};
((MarketProvider3) => {
  MarketProvider3.Config = import_koishi11.Schema.object({
    endpoint: import_koishi11.Schema.string().role("link").default(DEFAULT_ENDPOINT),
    timeout: import_koishi11.Schema.number().role("time").default(import_koishi11.Time.second * 30),
    proxyAgent: import_koishi11.Schema.string().role("link"),
    autoRoute: import_koishi11.Schema.boolean().default(true),
    logLevel: import_koishi11.Schema.union(logLevels.map((level) => import_koishi11.Schema.const(level))).default("warn")
  });
...
var DAY = import_koishi12.Time.day;
...
// src/node/chatluna.ts
var CACHE_TTL = import_koishi13.Time.minute * 10;
var cache = {};
var pending = {};
...
function applyChatLunaTool(ctx, config = {}) {
  ...
  const logger7 = ctx.logger("market");
  logger7.debug("ChatLuna market search tool is enabled; waiting for chatluna service");
...
async function fetchIndex(ctx, config) {
  ...
  const http = ctx.http.extend({
    endpoint,
    timeout: config.search?.timeout ?? import_koishi13.Time.second * 30,
...
function setupIdleProbe(ctx, config) {
  ...
  const getDelay = () => Math.max(0, config.idleProbeDelay ?? import_koishi17.Time.minute * 5);
  const getBootDelay = () => Math.max(0, config.idleProbeBootDelay ?? import_koishi17.Time.minute);
  const getInterval = () => Math.max(0, config.idleProbeInterval ?? import_koishi17.Time.hour * 6);
  const runProbe = async () => {
    clearIdleTimer();
    if (!ctx.scope.isActive) return;
    if (getClientCount()) return;
    if (ctx.installer.isInstalling) {
      logger7.debug("skip idle background probe because dependency install is active");
      schedule(getDelay());
      return;
    }
    ...
    const retryWait = lastFailure ? Math.min(import_koishi17.Time.minute * 5, getInterval()) - (Date.now() - lastFailure) : 0;
    if (!lastProbe && retryWait > 0) {
      logger7.debug(`skip idle background probe because retry gate is active: remaining=${retryWait}ms`);
      schedule(retryWait);
      return;
    }
    const intervalWait = lastProbe ? getInterval() - (Date.now() - lastProbe) : 0;
    if (intervalWait > 0) {
      logger7.debug(`skip idle background probe because interval gate is active: remaining=${intervalWait}ms`);
    ...
    logger7.info(`idle background probe started: clients=0, delay=${getDelay()}ms, interval=${getInterval()}ms`);
    try {
      ...
      if (succeeded) {
        lastProbe = Date.now();
        lastFailure = 0;
        logger7.info(`idle background probe completed: elapsed=${Date.now() - probeStartedAt}ms`);
      } else {
        ...
        logger7.warn(`idle background probe failed: ${reason instanceof Error ? reason.message : reason}`);
      }
    } catch (error) {
      lastFailure = Date.now();
      logger7.warn(`idle background probe failed: ${error instanceof Error ? error.message : error}`);
    } finally {
      running = false;
      if (!getClientCount()) schedule(lastProbe ? getInterval() : Math.min(import_koishi17.Time.minute * 5, getInterval()));
    }
  };
  const schedule = (delay = getDelay()) => {
    ...
    logger7.debug(`idle background probe scheduled: delay=${Math.max(0, delay)}ms`);
  };
  ctx.on("console/connection", () => {
    if (getClientCount()) {
      clearIdleTimer();
      logger7.debug(`idle background probe cancelled: clients=${getClientCount()}`);
    } else {
      schedule();
    }
  });
...
var MarketSilentRuleType = import_koishi18.Schema.union([
  import_koishi18.Schema.const("preview").description("\u72B6\u6001\uFF1A\u9884\u89C8\u7248\u63D2\u4EF6"),
  import_koishi18.Schema.const("insecure").description("\u72B6\u6001\uFF1A\u4E0D\u5B89\u5168\u63D2\u4EF6"),
  import_koishi18.Schema.const("bundle").description("\u72B6\u6001\uFF1A\u63D2\u4EF6\u5305"),
  import_koishi18.Schema.const("created-before").description("\u521B\u5EFA\u65F6\u95F4\uFF1A\u65E9\u4E8E\u6307\u5B9A\u65E5\u671F"),
  import_koishi18.Schema.const("created-after").description("\u521B\u5EFA\u65F6\u95F4\uFF1A\u665A\u4E8E\u6307\u5B9A\u65E5\u671F"),
  import_koishi18.Schema.const("updated-before").description("\u66F4\u65B0\u65F6\u95F4\uFF1A\u65E9\u4E8E\u6307\u5B9A\u65E5\u671F"),
  import_koishi18.Schema.const("updated-after").description("\u66F4\u65B0\u65F6\u95F4\uFF1A\u665A\u4E8E\u6307\u5B9A\u65E5\u671F"),
  import_koishi18.Schema.const("created-within").description("\u521B\u5EFA\u65F6\u95F4\uFF1A\u6700\u8FD1 N \u5929\u5185"),
  import_koishi18.Schema.const("updated-within").description("\u66F4\u65B0\u65F6\u95F4\uFF1A\u6700\u8FD1 N \u5929\u5185"),
  import_koishi18.Schema.const("custom").description("\u81EA\u5B9A\u4E49\u9AD8\u7EA7\u6761\u4EF6")
]);
var MarketSilentRules = import_koishi18.Schema.array(import_koishi18.Schema.object({
  type: MarketSilentRuleType.default("preview").description("\u89C4\u5219\u7C7B\u578B"),
  value: import_koishi18.Schema.string().default("").description("\u89C4\u5219\u503C\u3002\u72B6\u6001\u7C7B\u7559\u7A7A\uFF1B\u65E5\u671F\u7C7B\u586B\u5199 YYYY-MM-DD\uFF0C\u4F8B\u5982 2024-01-01\uFF1B\u6700\u8FD1 N \u5929\u586B\u5199\u6570\u5B57\uFF0C\u4F8B\u5982 30\uFF1B\u81EA\u5B9A\u4E49\u89C4\u5219\u586B\u5199\u641C\u7D22\u6761\u4EF6\uFF0C\u4F8B\u5982 category:adapter\u3002"),
  note: import_koishi18.Schema.string().default("").description("\u5907\u6CE8"),
  enabled: import_koishi18.Schema.boolean().default(true).description("\u662F\u5426\u542F\u7528")
...
var Config = import_koishi18.Schema.object({
  frontendMode: import_koishi18.Schema.union([
    import_koishi18.Schema.const("performance").description("\u6027\u80FD\u6A21\u5F0F"),
    import_koishi18.Schema.const("polished").description("\u7CBE\u81F4\u6A21\u5F0F")
  ]).role("radio").default("performance").description("Frontend display mode."),
  depsLayout: import_koishi18.Schema.union([
    import_koishi18.Schema.const("grid").description("\u7F51\u683C"),
    import_koishi18.Schema.const("list").description("\u5217\u8868")
  ]).role("radio").default("grid").description("Dependencies page layout."),
  idleProbe: import_koishi18.Schema.boolean().default(true).description("Run dependency and market metadata probes while Console is idle."),
  idleProbeDelay: import_koishi18.Schema.number().role("time").default(import_koishi18.Time.minute * 5).description("How long Console must stay idle before the background probe starts."),
  idleProbeBootDelay: import_koishi18.Schema.number().role("time").default(import_koishi18.Time.minute).description("Minimum delay after startup before idle probing is allowed."),
  idleProbeInterval: import_koishi18.Schema.number().role("time").default(import_koishi18.Time.hour * 6).description("Minimum interval between idle background probes."),
  bulkMode: import_koishi18.Schema.boolean().default(false).hidden().description("Batch operation mode for dependency changes."),
  removeConfig: import_koishi18.Schema.union([
    import_koishi18.Schema.const(void 0).description("Ask every time"),
    import_koishi18.Schema.const(true).description("Always remove plugin config"),
    import_koishi18.Schema.const(false).description("Never remove plugin config")
  ]).hidden().description("Whether to remove existing plugin config when uninstalling a plugin."),
  updateIgnoredPackages: import_koishi18.Schema.string().role("textarea").hidden().description("Dependency package names that should not be checked for updates. One package per line, or separated by commas."),
  updateIgnoreDuration: import_koishi18.Schema.number().role("time").default(0).hidden().description("Default duration for ignoring one update. 0 means no time-based expiry."),
  updateIgnoreVersions: import_koishi18.Schema.number().min(1).max(20).step(1).default(1).hidden().description("How many consecutive newer versions should be ignored after ignoring one update."),
  updateIgnorePrerelease: import_koishi18.Schema.boolean().default(false).hidden().description("Ignore alpha, beta, rc and other prerelease versions when checking updates."),
  collapsedGroups: import_koishi18.Schema.dict(Boolean).hidden(),
  registry: installer_default.Config,
  search: market_default.Config,
  chatlunaTool: import_koishi18.Schema.boolean().default(false).description("Enable ChatLuna plugin market query tool."),
  marketSilentFilters: import_koishi18.Schema.string().role("textarea").hidden().description("Legacy permanent silent filters."),
  marketSilentStatusRules: import_koishi18.Schema.array(import_koishi18.Schema.any()).hidden(),
  marketSilentDateRules: import_koishi18.Schema.array(import_koishi18.Schema.any()).hidden(),
  marketSilentRecentRules: import_koishi18.Schema.array(import_koishi18.Schema.any()).hidden(),
  marketSilentCustomRules: import_koishi18.Schema.array(import_koishi18.Schema.any()).hidden(),
