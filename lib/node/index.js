...
var PackageManagerRunner = class {
  ...
  async exec(args) {
    ...
    const start = Date.now();
    logger6.info(`run package manager: agent=${name2}${this.agent?.version ? "@" + this.agent.version : ""}, args=${args.join(" ") || "(none)"}, cwd=${this.cwd}, json=${useJson}`);
    return new Promise((resolve11) => {
      if (useJson) args.push("--json");
      const child = this.spawnProcess(name2, args, { cwd: this.cwd });
...
var Config = import_koishi19.Schema.object({
  ...
  chatlunaTool: import_koishi19.Schema.boolean().default(false).description("Enable ChatLuna plugin market query tool."),
