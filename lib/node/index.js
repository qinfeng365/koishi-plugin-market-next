...
var Installer = class extends import_koishi.Service {
  ...
  async fetchRegistryEndpoint(name2, endpoint, serial, signal) {
    ...
    try {
      ...
      logger.debug(`probe npm registry endpoint succeeded: probe=${name2}, endpoint=${endpoint}, elapsed=${elapsed}ms, versions=${Object.keys(registry.versions).length}`);
    ...
    for (const endpoint of endpoints) {
      ...
      for (let retry = 0; retry <= maxRetry; retry++) {
        ...
        try {
          ...
          this.recordRegistryRouteFailure(endpoint, detail2.reason);
