...
var MarketProvider2 = class extends MarketProvider {
  ...
  pickDiskCache() {
    const endpoints = this.getEndpointCandidates();
    const candidates = endpoints.map((endpoint) => this.cacheEntries[endpoint]).filter((cache2) => !!cache2 && Array.isArray(cache2.result?.objects));
...
async function requestPluginRuntime(ctx, name2) {
  await ctx.console.listeners["config/request-runtime"]?.callback.call(null, name2);
}
async function ensurePluginConfig(ctx, name2) {
  if (!import_registry3.default.isPlugin(name2)) return false;
  const configWriter = ctx.console.services.config;
  ...
  ctx.logger("market").info("created default config entry for %c", name2);
  return true;
}
...
function apply(ctx, config = {}) {
  ...
  ctx.inject(["console", "installer"], (ctx2) => {
    ...
    ctx2.console.addListener("market/install", async (deps, forced) => {
      ...
      if (!code) {
        await ensurePluginConfigs(ctx2, Object.entries(deps).filter(([, version]) => version).map(([name2]) => name2));
      }
      await Promise.all([
