...
var RegistryMetadata = class {
  ...
  getRouteScore(endpoint) {
    ...
    if (stats.lastSuccess && Date.now() - stats.lastSuccess <= import_koishi4.Time.minute * 10) score += 1.5;
...
var MarketRouter = class {
  ...
  getScore(endpoint) {
    ...
    if (stats.lastSuccess && Date.now() - stats.lastSuccess <= import_koishi9.Time.minute * 10) score += 1.5;
    score -= Math.min(5, (stats.consecutiveFailures ?? 0) * 1.5);
    return score;
  }
  ...
  async fetchEndpoint(endpoint, index, total, serial, warnFailure = true, signal) {
    ...
    try {
      ...
      if (response.status === 304) {
        ...
        this.options.log("debug", `market index not modified from ${endpoint} in ${elapsed2}ms, reuse cache hash=${shortHash(cache2.hash) || "unknown"}`);
        this.options.log("info", `market index http-304: endpoint=${endpoint}, elapsed=${elapsed2}ms, request=${requestElapsed}ms, cachedAt=${formatTime(cache2.fetchedAt)}, hash=${shortHash(cache2.hash) || "unknown"}`);
        return {
          endpoint,
          ...
          size: cache2.size,
          wireSize: headerWireSize ?? cache2.wireSize,
          contentEncoding: contentEncoding ?? cache2.contentEncoding,
          hash: cache2.hash,
          etag: etag || cache2.etag,
          lastModified: lastModified || cache2.lastModified,
          cachedAt: cache2.fetchedAt,
          validatedAt
        };
      }
      ...
      const hashCache = cached && cached.hash === hash ? await this.cache.loadEntry(cached) : void 0;
      if (hashCache) {
        const elapsed2 = Date.now() - start;
        const validatedAt = Date.now();
        this.options.log("debug", `market index hash unchanged from ${endpoint} in ${elapsed2}ms, size=${size}, hash=${shortHash(hash)}`);
        this.options.log("info", `market index hash-cache: endpoint=${endpoint}, elapsed=${elapsed2}ms, request=${requestElapsed}ms, hash=${shortHash(hash)}, size=${formatBytes2(size)}, wireSize=${formatBytes2(wireSize)}, encoding=${contentEncoding ?? "identity"}`);
        return {
          endpoint,
      ...
      const parseStart = Date.now();
      this.options.log("debug", `market json parse started: endpoint=${endpoint}, decodedSize=${formatBytes2(size)}`);
      ...
      this.options.log("info", `market index fetched: endpoint=${endpoint}, elapsed=${elapsed}ms, request=${requestElapsed}ms, hash=${hashElapsed}ms, json=${parseElapsed}ms, objects=${result.objects.length}, size=${formatBytes2(size)}, wireSize=${formatBytes2(wireSize)}, encoding=${contentEncoding ?? "identity"}, hash=${shortHash(hash) || "unknown"}, version=${result.version ?? "legacy"}`);
      return {
        endpoint,
        result,
        elapsed,
