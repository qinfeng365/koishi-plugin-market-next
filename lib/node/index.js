...
// src/node/chatluna.ts
var CACHE_TTL = import_koishi12.Time.minute * 10;
var cache = {};
var pending = {};
...
async function fetchIndex(ctx, config) {
  ...
  const http = ctx.http.extend({
    endpoint,
    timeout: config.search?.timeout ?? import_koishi12.Time.second * 30,
...
function setupIdleProbe(ctx, config) {
  ...
  const getDelay = () => Math.max(0, config.idleProbeDelay ?? import_koishi16.Time.minute * 5);
  const getBootDelay = () => Math.max(0, config.idleProbeBootDelay ?? import_koishi16.Time.minute);
  const getInterval = () => Math.max(0, config.idleProbeInterval ?? import_koishi16.Time.hour * 6);
  const runProbe = async () => {
    ...
    const retryWait = lastFailure ? Math.min(import_koishi16.Time.minute * 5, getInterval()) - (Date.now() - lastFailure) : 0;
    ...
    try {
      ...
      running = false;
      if (!getClientCount()) schedule(lastProbe ? getInterval() : Math.min(import_koishi16.Time.minute * 5, getInterval()));
    }
  };
...
var MarketSilentRuleType = import_koishi17.Schema.union([
  import_koishi17.Schema.const("preview").description("\u72B6\u6001\uFF1A\u9884\u89C8\u7248\u63D2\u4EF6"),
  import_koishi17.Schema.const("insecure").description("\u72B6\u6001\uFF1A\u4E0D\u5B89\u5168\u63D2\u4EF6"),
  import_koishi17.Schema.const("bundle").description("\u72B6\u6001\uFF1A\u63D2\u4EF6\u5305"),
  import_koishi17.Schema.const("created-before").description("\u521B\u5EFA\u65F6\u95F4\uFF1A\u65E9\u4E8E\u6307\u5B9A\u65E5\u671F"),
  import_koishi17.Schema.const("created-after").description("\u521B\u5EFA\u65F6\u95F4\uFF1A\u665A\u4E8E\u6307\u5B9A\u65E5\u671F"),
  import_koishi17.Schema.const("updated-before").description("\u66F4\u65B0\u65F6\u95F4\uFF1A\u65E9\u4E8E\u6307\u5B9A\u65E5\u671F"),
  import_koishi17.Schema.const("updated-after").description("\u66F4\u65B0\u65F6\u95F4\uFF1A\u665A\u4E8E\u6307\u5B9A\u65E5\u671F"),
  import_koishi17.Schema.const("created-within").description("\u521B\u5EFA\u65F6\u95F4\uFF1A\u6700\u8FD1 N \u5929\u5185"),
  import_koishi17.Schema.const("updated-within").description("\u66F4\u65B0\u65F6\u95F4\uFF1A\u6700\u8FD1 N \u5929\u5185"),
  import_koishi17.Schema.const("custom").description("\u81EA\u5B9A\u4E49\u9AD8\u7EA7\u6761\u4EF6")
]);
var MarketSilentRules = import_koishi17.Schema.array(import_koishi17.Schema.object({
  type: MarketSilentRuleType.default("preview").description("\u89C4\u5219\u7C7B\u578B"),
  value: import_koishi17.Schema.string().default("").description("\u89C4\u5219\u503C\u3002\u72B6\u6001\u7C7B\u7559\u7A7A\uFF1B\u65E5\u671F\u7C7B\u586B\u5199 YYYY-MM-DD\uFF0C\u4F8B\u5982 2024-01-01\uFF1B\u6700\u8FD1 N \u5929\u586B\u5199\u6570\u5B57\uFF0C\u4F8B\u5982 30\uFF1B\u81EA\u5B9A\u4E49\u89C4\u5219\u586B\u5199\u641C\u7D22\u6761\u4EF6\uFF0C\u4F8B\u5982 category:adapter\u3002"),
  note: import_koishi17.Schema.string().default("").description("\u5907\u6CE8"),
  enabled: import_koishi17.Schema.boolean().default(true).description("\u662F\u5426\u542F\u7528")
})).role("table").default([]).description("\u63D2\u4EF6\u5E02\u573A\u6C38\u4E45\u9759\u9ED8\u8FC7\u6EE4\u3002\u6DFB\u52A0\u89C4\u5219\u540E\uFF0C\u547D\u4E2D\u7684\u63D2\u4EF6\u4F1A\u76F4\u63A5\u4ECE\u5E02\u573A\u9875\u9690\u85CF\uFF0C\u4E0D\u4F1A\u663E\u793A\u5728\u641C\u7D22\u6846\u4E2D\u3002\u72B6\u6001\u7C7B\u4E0D\u9700\u8981\u586B\u5199\u503C\uFF1B\u65E5\u671F\u7C7B\u586B\u5199 YYYY-MM-DD\uFF1B\u6700\u8FD1 N \u5929\u586B\u5199\u6570\u5B57\u3002");
var Config = import_koishi17.Schema.object({
  frontendMode: import_koishi17.Schema.union([
    import_koishi17.Schema.const("performance").description("\u6027\u80FD\u6A21\u5F0F"),
    import_koishi17.Schema.const("polished").description("\u7CBE\u81F4\u6A21\u5F0F")
  ]).role("radio").default("performance").description("Frontend display mode."),
  depsLayout: import_koishi17.Schema.union([
    import_koishi17.Schema.const("grid").description("\u7F51\u683C"),
    import_koishi17.Schema.const("list").description("\u5217\u8868")
  ]).role("radio").default("grid").description("Dependencies page layout."),
  idleProbe: import_koishi17.Schema.boolean().default(true).description("Run dependency and market metadata probes while Console is idle."),
  idleProbeDelay: import_koishi17.Schema.number().role("time").default(import_koishi17.Time.minute * 5).description("How long Console must stay idle before the background probe starts."),
  idleProbeBootDelay: import_koishi17.Schema.number().role("time").default(import_koishi17.Time.minute).description("Minimum delay after startup before idle probing is allowed."),
  idleProbeInterval: import_koishi17.Schema.number().role("time").default(import_koishi17.Time.hour * 6).description("Minimum interval between idle background probes."),
  bulkMode: import_koishi17.Schema.boolean().default(false).hidden().description("Batch operation mode for dependency changes."),
  removeConfig: import_koishi17.Schema.union([
    import_koishi17.Schema.const(void 0).description("Ask every time"),
    import_koishi17.Schema.const(true).description("Always remove plugin config"),
    import_koishi17.Schema.const(false).description("Never remove plugin config")
  ]).hidden().description("Whether to remove existing plugin config when uninstalling a plugin."),
  updateIgnoredPackages: import_koishi17.Schema.string().role("textarea").hidden().description("Dependency package names that should not be checked for updates. One package per line, or separated by commas."),
  updateIgnoreDuration: import_koishi17.Schema.number().role("time").default(0).hidden().description("Default duration for ignoring one update. 0 means no time-based expiry."),
  updateIgnoreVersions: import_koishi17.Schema.number().min(1).max(20).step(1).default(1).hidden().description("How many consecutive newer versions should be ignored after ignoring one update."),
  updateIgnorePrerelease: import_koishi17.Schema.boolean().default(false).hidden().description("Ignore alpha, beta, rc and other prerelease versions when checking updates."),
  collapsedGroups: import_koishi17.Schema.dict(Boolean).hidden(),
  registry: installer_default.Config,
  search: market_default.Config,
  chatlunaTool: import_koishi17.Schema.boolean().default(false).description("Enable ChatLuna plugin market query tool."),
  marketSilentFilters: import_koishi17.Schema.string().role("textarea").hidden().description("Legacy permanent silent filters."),
  marketSilentStatusRules: import_koishi17.Schema.array(import_koishi17.Schema.any()).hidden(),
  marketSilentDateRules: import_koishi17.Schema.array(import_koishi17.Schema.any()).hidden(),
  marketSilentRecentRules: import_koishi17.Schema.array(import_koishi17.Schema.any()).hidden(),
  marketSilentCustomRules: import_koishi17.Schema.array(import_koishi17.Schema.any()).hidden(),
