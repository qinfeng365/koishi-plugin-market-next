...
var Installer = class extends import_koishi.Service {
  ...
  async _installLocked(deps, forced, beforeReload) {
    const start = Date.now();
...
async function installBundle(ctx, dataStore, request, forced) {
  const start = Date.now();
...
function apply(ctx, config = {}) {
  ...
  ctx.inject(["console", "installer"], (ctx2) => {
    ...
    ctx2.console.addListener("market/install", async (deps, forced) => {
      ...
      const code = await ctx2.installer.install(deps, forced, installNames.length ? () => ensurePluginConfigs(ctx2, installNames) : void 0);
      if (!code) {
    ...
    ctx2.console.addListener("market/install-bundle", async (request, forced) => {
      return installBundle(ctx2, dataStore, request, forced);
    }, { authority: 4 });
