...
var MarketProvider2 = class extends MarketProvider {
  ...
  async get() {
    ...
    if (!this.scanner) {
      ...
      return this.payload ? { ...this.payload, serverNow: Date.now(), debug: this.getDebugInfo() } : { data: {}, failed: 0, total: 0, progress: 0, serverNow: Date.now(), debug: this.getDebugInfo() };
    }
    if (this._error) {
...
async function assertNoDirectBundleCycles(ctx, packageName, members) {
  ...
  for (const member of members) {
    try {
      ...
      const version = (0, import_semver4.maxSatisfying)(versions, member.version, { includePrerelease: true });
      if (!version) continue;
  ...
  ctx.inject(["installer"], (ctx2) => {
    ...
    ctx2.command("plugin.upgrade [name...]", { authority: 4 }).alias(".update", ".up").option("self", "-s, --koishi").action(async ({ session, options }, ...names) => {
      ...
      names = names.filter((name2) => {
        ...
        try {
          return !invalid && (0, import_semver4.gt)(latest, resolved);
        } catch {
        }
      });
