...
var MarketProvider2 = class extends MarketProvider {
  constructor(ctx, config = {}) {
    ...
    this.cacheFile = (0, import_path6.resolve)(ctx.baseDir, "cache", "market-next-index.json");
    this.cacheDir = (0, import_path6.resolve)(ctx.baseDir, "cache", "market-next-index");
  ...
  async fetchIndex(serial) {
    ...
    try {
      ...
      if (!rescueEndpoints.length || this.isStale(serial) || this.isInternalAbort(error)) throw error;
      this.log("warn", `market active endpoints failed; retry cooled endpoints as rescue: active=${endpoints.join(", ")}, rescue=${rescueEndpoints.join(", ")}, error=${formatError(error)}`);
  ...
  async fetchIndexFromEndpoints(serial, endpoints, options = {}) {
    ...
    this.log("debug", `market endpoint candidates (${routeMode}): ${endpoints.join(", ")}`);
    this.log("debug", `market route scores before fetch: ${formatRouteScores2(this.getRouteScores(endpoints))}`);
    this.log("info", `market endpoint candidates: mode=${routeMode}, primary=${endpoints[0]}, fallbacks=${Math.max(0, endpoints.length - 1)}, autoRoute=${this.config.autoRoute !== false}`);
    if (endpoints.length === 1 || this.config.autoRoute === false) {
      const controller = this.trackController(new AbortController());
      try {
        ...
        if (!this.isStale(serial) && !this.isInternalAbort(error)) this.recordRouteFailure(endpoints[0], { rescue: options.rescue });
        throw error;
      } finally {
    ...
    return new Promise((resolve7, reject) => {
      ...
      const settle = (data, index) => {
        ...
        if (options.rescue) {
          ...
          this.log("debug", `rescue market endpoint selected: endpoint=${data.endpoint}, elapsed=${data.elapsed}ms, configured=${this.config.endpoint}`);
          this.log("info", `market rescue endpoint selected: endpoint=${data.endpoint}, elapsed=${data.elapsed}ms, configured=${this.config.endpoint}`);
      ...
      const fail = (endpoint, index, error) => {
        ...
        this.recordRouteFailure(endpoint, { rescue: options.rescue });
        lastError = error;
        failed++;
        if (index === 0) startFallback("primary-failed");
        if (failed < endpoints.length) return;
  ...
  getRouteScore(endpoint) {
    ...
    if (stats.lastSuccess && Date.now() - stats.lastSuccess <= import_koishi3.Time.minute * 10) score += 1.5;
    score -= Math.min(5, (stats.consecutiveFailures ?? 0) * 1.5);
    return score;
  }
  ...
  getRouteScores(endpoints = this.getEndpointCandidates()) {
  ...
  async fetchEndpoint(endpoint, index, total, serial, warnFailure = true, signal) {
    ...
    try {
      ...
      this.log("debug", `market response headers: endpoint=${endpoint}, status=${response.status}, request=${requestElapsed}ms, etag=${etag ?? "-"}, lastModified=${lastModified ?? "-"}, encoding=${contentEncoding ?? "identity"}, contentLength=${formatBytes2(headerWireSize)}`);
      const cached = this.cacheEntries[endpoint];
      if (response.status === 304) {
        const cache2 = cached && await this.loadCacheEntryResult(cached);
        if (!cache2) {
          throw new Error(`market index from ${endpoint} returned 304 without cache`);
        }
        const elapsed2 = Date.now() - start;
        const validatedAt = Date.now();
        this.log("debug", `market index not modified from ${endpoint} in ${elapsed2}ms, reuse cache hash=${shortHash(cache2.hash) || "unknown"}`);
        this.log("info", `market index http-304: endpoint=${endpoint}, elapsed=${elapsed2}ms, request=${requestElapsed}ms, cachedAt=${formatTime(cache2.fetchedAt)}, hash=${shortHash(cache2.hash) || "unknown"}`);
        return {
          endpoint,
      ...
      this.log("debug", `market response body decoded: endpoint=${endpoint}, chars=${text.length}, decodedSize=${formatBytes2(size)}, wireSize=${formatBytes2(wireSize)}, cachedHash=${shortHash(cached?.hash) ?? "-"}, cachedAt=${cached?.fetchedAt ? formatTime(cached.fetchedAt) : "-"}`);
      const hashStart = Date.now();
      const hash = (0, import_crypto4.createHash)("sha256").update(text).digest("hex");
      ...
      const hashCache = cached && cached.hash === hash ? await this.loadCacheEntryResult(cached) : void 0;
      if (hashCache) {
        const elapsed2 = Date.now() - start;
        const validatedAt = Date.now();
        this.log("debug", `market index hash unchanged from ${endpoint} in ${elapsed2}ms, size=${size}, hash=${shortHash(hash)}`);
        this.log("info", `market index hash-cache: endpoint=${endpoint}, elapsed=${elapsed2}ms, request=${requestElapsed}ms, hash=${shortHash(hash)}, size=${formatBytes2(size)}, wireSize=${formatBytes2(wireSize)}, encoding=${contentEncoding ?? "identity"}`);
        return {
          endpoint,
      ...
      if (cached && cached.hash === hash) {
        this.log("debug", `market cache hash matched but cached result is unavailable, parse network body instead: endpoint=${endpoint}, hash=${shortHash(hash)}`);
      }
      const parseStart = Date.now();
      this.log("debug", `market json parse started: endpoint=${endpoint}, decodedSize=${formatBytes2(size)}`);
  ...
  async loadDiskCache(serial) {
    ...
    try {
      ...
      const content = await import_fs5.promises.readFile(this.cacheFile, "utf8");
      ...
      if (store.routeStats) {
        for (const [endpoint, stats] of Object.entries(store.routeStats)) {
          if (!stats) continue;
          const hasRecentSuccess = stats.lastSuccess && Date.now() - stats.lastSuccess < import_koishi3.Time.day;
      ...
      this.log("debug", `market disk cache store parsed: bytes=${formatBytes2(Buffer.byteLength(content))}, entries=${Object.keys(store.entries).length}, lastUsed=${store.lastUsed ?? "-"}, parse=${parseElapsed}ms, endpoints=${formatCacheEntries(store.entries)}`);
      const staleEndpoints = Object.values(store.entries).filter((e) => !!e && Date.now() - e.fetchedAt > CACHE_ENTRY_TTL).map((e) => e.endpoint);
      if (staleEndpoints.length) {
        this.log("debug", `market disk cache has ${staleEndpoints.length} stale entries (>${CACHE_ENTRY_TTL / import_koishi3.Time.day}d), will prune on next write: ${staleEndpoints.join(", ")}`);
      ...
      const cache2 = await this.pickDiskCache();
      if (!cache2) {
      ...
      const applyStart = Date.now();
      this.applyIndex(cache2.result, cache2.endpoint, cache2.hash);
      ...
      this.log("info", `market disk cache loaded: endpoint=${cache2.endpoint}, objects=${this.scanner.total}, cachedAt=${formatTime(cache2.fetchedAt)}, age=${formatAge(Date.now() - cache2.fetchedAt)}, entries=${Object.keys(this.cacheEntries).length}, elapsed=${Date.now() - start}ms, size=${formatBytes2(cache2.size)}, wireSize=${formatBytes2(cache2.wireSize)}, encoding=${cache2.contentEncoding ?? "identity"}`);
      if (shouldMigrate) {
        ...
        setTimeout(() => {
          ...
          void this.writeDiskCache({
            version: 3,
            entries: this.pruneCacheEntries(cache2.endpoint),
            lastUsed: cache2.endpoint,
            routeStats: this.serializeRouteStats()
          });
        }, 1e3);
      }
  ...
  async pickDiskCache() {
    ...
    const primaryCache = primary ? await this.loadCacheEntryResult(primary) : void 0;
    if (primaryCache) {
    ...
    const candidates = endpoints.filter((endpoint) => endpoint !== this.config.endpoint).map((endpoint) => this.cacheEntries[endpoint]).filter((cache2) => !!cache2 && hasCacheResultReference(cache2)).sort((a, b) => {
      const delta = this.getCacheScore(b) - this.getCacheScore(a);
      if (delta) return delta;
      return b.fetchedAt - a.fetchedAt;
    });
    for (const entry of candidates) {
      const cache2 = await this.loadCacheEntryResult(entry);
      if (!cache2) continue;
      this.log("debug", `pick market disk cache fallback hit: endpoint=${cache2.endpoint}, candidates=${candidates.map((item) => `${item.endpoint} score=${this.getCacheScore(item).toFixed(2)} age=${formatAge(Date.now() - item.fetchedAt)} objects=${item.result?.objects?.length ?? item.objects ?? "-"}`).join(" | ")}`);
      return cache2;
    }
  }
  async loadCacheEntryResult(entry) {
    if (Array.isArray(entry.result?.objects)) return entry;
    if (!entry.file) return;
    try {
      const content = await import_fs5.promises.readFile((0, import_path6.resolve)(this.cacheDir, entry.file), "utf8");
      const result = JSON.parse(content);
      if (!Array.isArray(result?.objects)) return;
      const cache2 = { ...entry, result };
      this.cacheEntries[entry.endpoint] = cache2;
      return cache2;
    } catch (error) {
  ...
  async writeCacheEntryFile(entry) {
    await import_fs5.promises.mkdir(this.cacheDir, { recursive: true });
    const file = (0, import_path6.resolve)(this.cacheDir, this.getCacheEntryFilename(entry.endpoint));
  ...
  async pruneSplitCacheFiles(entries) {
    try {
      const keep = new Set(Object.values(entries).map((entry) => entry?.file).filter(Boolean));
      const files = await import_fs5.promises.readdir(this.cacheDir).catch(() => []);
      await Promise.all(files.filter((file) => file.endsWith(".json") && !keep.has(file)).map((file) => import_fs5.promises.unlink((0, import_path6.resolve)(this.cacheDir, file)).catch(() => {
      })));
    } catch (error) {
  ...
  scheduleDiskCacheWrite(result, meta = this.conditionMeta) {
    ...
    this.log("debug", `schedule market disk cache write: endpoint=${entry.endpoint}, objects=${result.objects.length}, entries=${Object.keys(cache2.entries).length}, file=${this.cacheFile}, hash=${shortHash(entry.hash) ?? "-"}, size=${formatBytes2(entry.size)}, wireSize=${formatBytes2(entry.wireSize)}, encoding=${entry.contentEncoding ?? "identity"}`);
    this.cacheWriteTimer = setTimeout(() => {
      this.cacheWriteTimer = void 0;
  ...
  pruneCacheEntries(lastUsed) {
    const entries = Object.values(this.cacheEntries).filter((cache2) => !!cache2 && hasCacheResultReference(cache2) && Date.now() - cache2.fetchedAt <= CACHE_ENTRY_TTL).sort((a, b) => {
  ...
  async writeDiskCache(cache2) {
    if (this.disposed || !this.ctx.scope.isActive) return;
    try {
      await import_fs5.promises.mkdir((0, import_path6.dirname)(this.cacheFile), { recursive: true });
      const entries = {};
      for (const [endpoint, entry] of Object.entries(cache2.entries)) {
        if (!entry) continue;
        if (Array.isArray(entry.result?.objects)) {
          await this.writeCacheEntryFile(entry);
          entries[endpoint] = this.createSplitCacheEntry(entry);
        } else if (entry.file) {
      ...
      await import_fs5.promises.rename(tempFile, this.cacheFile);
...
var AVATAR_CACHE_TTL = import_koishi5.Time.day * 7;
var AVATAR_CACHE_SWEEP_INTERVAL = import_koishi5.Time.hour;
...
function normalizeAvatarCacheKey(key) {
  return key.replace(/[^0-9A-Za-z:@._-]/g, "-").slice(0, 128) || `url:${(0, import_crypto5.createHash)("sha1").update(key).digest("hex")}`;
}
function getAvatarCacheFile(ctx, key) {
  return (0, import_path7.resolve)(getAvatarCacheDir(ctx), `${(0, import_crypto5.createHash)("sha1").update(normalizeAvatarCacheKey(key)).digest("hex")}.json`);
}
...
var MarketSilentRuleType = import_koishi6.Schema.union([
  import_koishi6.Schema.const("preview").description("\u72B6\u6001\uFF1A\u9884\u89C8\u7248\u63D2\u4EF6"),
  import_koishi6.Schema.const("insecure").description("\u72B6\u6001\uFF1A\u4E0D\u5B89\u5168\u63D2\u4EF6"),
  import_koishi6.Schema.const("bundle").description("\u72B6\u6001\uFF1A\u63D2\u4EF6\u5305"),
  import_koishi6.Schema.const("created-before").description("\u521B\u5EFA\u65F6\u95F4\uFF1A\u65E9\u4E8E\u6307\u5B9A\u65E5\u671F"),
  import_koishi6.Schema.const("created-after").description("\u521B\u5EFA\u65F6\u95F4\uFF1A\u665A\u4E8E\u6307\u5B9A\u65E5\u671F"),
  import_koishi6.Schema.const("updated-before").description("\u66F4\u65B0\u65F6\u95F4\uFF1A\u65E9\u4E8E\u6307\u5B9A\u65E5\u671F"),
  import_koishi6.Schema.const("updated-after").description("\u66F4\u65B0\u65F6\u95F4\uFF1A\u665A\u4E8E\u6307\u5B9A\u65E5\u671F"),
  import_koishi6.Schema.const("created-within").description("\u521B\u5EFA\u65F6\u95F4\uFF1A\u6700\u8FD1 N \u5929\u5185"),
  import_koishi6.Schema.const("updated-within").description("\u66F4\u65B0\u65F6\u95F4\uFF1A\u6700\u8FD1 N \u5929\u5185"),
  import_koishi6.Schema.const("custom").description("\u81EA\u5B9A\u4E49\u9AD8\u7EA7\u6761\u4EF6")
]);
var MarketSilentRules = import_koishi6.Schema.array(import_koishi6.Schema.object({
  type: MarketSilentRuleType.default("preview").description("\u89C4\u5219\u7C7B\u578B"),
  value: import_koishi6.Schema.string().default("").description("\u89C4\u5219\u503C\u3002\u72B6\u6001\u7C7B\u7559\u7A7A\uFF1B\u65E5\u671F\u7C7B\u586B\u5199 YYYY-MM-DD\uFF0C\u4F8B\u5982 2024-01-01\uFF1B\u6700\u8FD1 N \u5929\u586B\u5199\u6570\u5B57\uFF0C\u4F8B\u5982 30\uFF1B\u81EA\u5B9A\u4E49\u89C4\u5219\u586B\u5199\u641C\u7D22\u6761\u4EF6\uFF0C\u4F8B\u5982 category:adapter\u3002"),
  note: import_koishi6.Schema.string().default("").description("\u5907\u6CE8"),
  enabled: import_koishi6.Schema.boolean().default(true).description("\u662F\u5426\u542F\u7528")
...
var Config = import_koishi6.Schema.object({
  frontendMode: import_koishi6.Schema.union([
    import_koishi6.Schema.const("performance").description("\u6027\u80FD\u6A21\u5F0F"),
    import_koishi6.Schema.const("polished").description("\u7CBE\u81F4\u6A21\u5F0F")
  ]).role("radio").default("performance").description("Frontend display mode."),
  depsLayout: import_koishi6.Schema.union([
    import_koishi6.Schema.const("grid").description("\u7F51\u683C"),
    import_koishi6.Schema.const("list").description("\u5217\u8868")
  ]).role("radio").default("grid").description("Dependencies page layout."),
  idleProbe: import_koishi6.Schema.boolean().default(true).description("Run dependency and market metadata probes while Console is idle."),
  idleProbeDelay: import_koishi6.Schema.number().role("time").default(import_koishi6.Time.minute * 5).description("How long Console must stay idle before the background probe starts."),
  idleProbeBootDelay: import_koishi6.Schema.number().role("time").default(import_koishi6.Time.minute).description("Minimum delay after startup before idle probing is allowed."),
  idleProbeInterval: import_koishi6.Schema.number().role("time").default(import_koishi6.Time.hour * 6).description("Minimum interval between idle background probes."),
  bulkMode: import_koishi6.Schema.boolean().default(false).hidden().description("Batch operation mode for dependency changes."),
  removeConfig: import_koishi6.Schema.union([
    import_koishi6.Schema.const(void 0).description("Ask every time"),
    import_koishi6.Schema.const(true).description("Always remove plugin config"),
    import_koishi6.Schema.const(false).description("Never remove plugin config")
  ]).hidden().description("Whether to remove existing plugin config when uninstalling a plugin."),
  updateIgnoredPackages: import_koishi6.Schema.string().role("textarea").hidden().description("Dependency package names that should not be checked for updates. One package per line, or separated by commas."),
  updateIgnoreDuration: import_koishi6.Schema.number().role("time").default(0).hidden().description("Default duration for ignoring one update. 0 means no time-based expiry."),
  updateIgnoreVersions: import_koishi6.Schema.number().min(1).max(20).step(1).default(1).hidden().description("How many consecutive newer versions should be ignored after ignoring one update."),
  updateIgnorePrerelease: import_koishi6.Schema.boolean().default(false).hidden().description("Ignore alpha, beta, rc and other prerelease versions when checking updates."),
  collapsedGroups: import_koishi6.Schema.dict(Boolean).hidden(),
  registry: installer_default.Config,
  search: market_default.Config,
  chatlunaTool: import_koishi6.Schema.boolean().default(false).description("Enable ChatLuna plugin market query tool."),
  marketSilentFilters: import_koishi6.Schema.string().role("textarea").hidden().description("Legacy permanent silent filters."),
  marketSilentStatusRules: import_koishi6.Schema.array(import_koishi6.Schema.any()).hidden(),
  marketSilentDateRules: import_koishi6.Schema.array(import_koishi6.Schema.any()).hidden(),
  marketSilentRecentRules: import_koishi6.Schema.array(import_koishi6.Schema.any()).hidden(),
  marketSilentCustomRules: import_koishi6.Schema.array(import_koishi6.Schema.any()).hidden(),
...
var MarketSnapshotTransport = class {
  ...
  async create(snapshot) {
    ...
    const id = (0, import_crypto6.createHash)("sha256").update(json).digest("hex");
...
function setupIdleProbe(ctx, config) {
  ...
  const getDelay = () => Math.max(0, config.idleProbeDelay ?? import_koishi7.Time.minute * 5);
  const getBootDelay = () => Math.max(0, config.idleProbeBootDelay ?? import_koishi7.Time.minute);
  const getInterval = () => Math.max(0, config.idleProbeInterval ?? import_koishi7.Time.hour * 6);
  const runProbe = async () => {
    ...
    const retryWait = lastFailure ? Math.min(import_koishi7.Time.minute * 5, getInterval()) - (Date.now() - lastFailure) : 0;
    ...
    try {
      ...
      running = false;
      if (!getClientCount()) schedule(lastProbe ? getInterval() : Math.min(import_koishi7.Time.minute * 5, getInterval()));
    }
  };
  ...
  ctx.inject(["installer"], (ctx2) => {
    ...
    ctx2.command("plugin.install <name>", { authority: 4 }).alias(".i").action(async ({ session }, name2) => {
      ...
      ctx2.loader.envData.message = {
        ...(0, import_koishi7.pick)(session, ["sid", "channelId", "guildId", "isDirect"]),
