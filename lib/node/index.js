...
function validatePackFilename(value) {
  if (typeof value !== "string" || (0, import_path2.basename)(value) !== value || !/^[a-z0-9@._+-]+\.tgz$/i.test(value)) {
...
var LocalPackageUploadStore = class {
  constructor(baseDir, warn) {
    this.warn = warn;
    this.root = (0, import_path3.resolve)(baseDir, ".yarn", "local");
    this.temporaryRoot = (0, import_path3.resolve)(this.root, ".market-next-upload");
  }
  warn;
  root;
  temporaryRoot;
  ...
  async start(request) {
    ...
    await import_fs2.promises.mkdir(this.temporaryRoot, { recursive: true });
    const uploadId = (0, import_crypto.randomUUID)();
    const path = (0, import_path3.resolve)(this.temporaryRoot, `${uploadId}.part`);
    ...
    this.sessions.set(uploadId, {
      ...
      touchedAt: Date.now(),
      handle,
      hash: (0, import_crypto.createHash)("sha256")
    });
  ...
  async commit(uploadId) {
    ...
    await this.closeHandle(session);
    await import_fs2.promises.mkdir(this.root, { recursive: true });
    const target = (0, import_path3.resolve)(this.root, session.validated.targetFilename);
    ...
    if (!existing) {
      try {
        await import_fs2.promises.rename(session.path, target);
      } catch (error) {
        const concurrent = await readFileHash(target);
        if (concurrent !== session.validated.hash) throw error;
        await import_fs2.promises.rm(session.path, { force: true });
      }
    } else {
      await import_fs2.promises.rm(session.path, { force: true });
    }
  ...
  async pruneExpired(now = Date.now()) {
    ...
    const entries = await import_fs2.promises.readdir(this.temporaryRoot, { withFileTypes: true }).catch((error) => {
      if (error.code === "ENOENT") return [];
      throw error;
    });
    await Promise.all(entries.map(async (entry) => {
      if (!entry.isFile() || !entry.name.endsWith(".part")) return;
      const path = (0, import_path3.resolve)(this.temporaryRoot, entry.name);
      if (activePaths.has(path)) return;
      const stat = await import_fs2.promises.stat(path);
      if (now - stat.mtimeMs <= LOCAL_UPLOAD_TTL) return;
      await import_fs2.promises.rm(path, { force: true });
    }));
  }
  async dispose() {
...
function getLocalPackageOperation(currentRequest, currentVersion, targetVersion) {
  ...
  if (!currentVersion || !(0, import_semver3.valid)(currentVersion) || !(0, import_semver3.valid)(targetVersion)) return "replace";
  const result = (0, import_semver3.compare)(targetVersion, currentVersion);
...
async function inspectPackageArchive(path) {
  ...
  if (typeof manifest.name !== "string" || !import_registry.default.isPlugin(manifest.name)) {
    throw new Error("\u5F52\u6863\u4E2D\u7684\u5305\u540D\u4E0D\u662F\u6709\u6548\u7684 Koishi \u63D2\u4EF6\u540D\u79F0\u3002");
  }
  if (typeof manifest.version !== "string" || !(0, import_semver3.valid)(manifest.version)) {
...
function validateUploadFilename(value) {
  if (typeof value !== "string" || (0, import_path3.basename)(value) !== value || !value.toLowerCase().endsWith(".tgz")) {
...
async function readFileHash(path) {
  try {
    ...
    return (0, import_crypto.createHash)("sha256").update(await import_fs2.promises.readFile(path)).digest("hex");
  } catch (error) {
...
function assertInside(root, target) {
  if ((0, import_path3.dirname)(target) !== root || (0, import_path3.relative)(root, target).startsWith("..")) {
...
function getEnvironmentSnapshotId(dependencies) {
  return `env-${(0, import_crypto2.createHash)("sha256").update(canonicalDependencies(dependencies)).digest("hex").slice(0, 20)}`;
}
...
function getEnvironmentDiff(current, target) {
  ...
  return [...names].sort((a, b) => a.localeCompare(b)).map((name2) => {
    ...
    if (currentVersion && targetVersion && (0, import_semver4.valid)(currentVersion) && (0, import_semver4.valid)(targetVersion)) {
      const direction = (0, import_semver4.compare)(targetVersion, currentVersion);
...
var EnvironmentSnapshotStore = class {
  ...
  async load() {
    if (!this.loaded) {
      this.loaded = (async () => {
        try {
          const parsed = JSON.parse(await import_fs3.promises.readFile(this.filename, "utf8"));
  ...
  async persist() {
    await import_fs3.promises.mkdir((0, import_path4.dirname)(this.filename), { recursive: true });
    const temporary = `${this.filename}.${process.pid}.${Date.now()}.tmp`;
    await import_fs3.promises.writeFile(temporary, JSON.stringify(this.value, null, 2) + "\n");
    try {
      await import_fs3.promises.rename(temporary, this.filename);
    } catch (error) {
      if (!["EEXIST", "EPERM"].includes(error?.code)) throw error;
      await import_fs3.promises.rm(this.filename, { force: true });
      await import_fs3.promises.rename(temporary, this.filename);
    }
  }
...
var REGISTRY_FAST_ROUTE_THRESHOLD = import_koishi2.Time.second * 0.8;
var REGISTRY_STATS_TTL = import_koishi2.Time.day * 30;
var NOT_FOUND_CACHE_TTL = import_koishi2.Time.minute * 5;
var FULL_RELOAD_DELAY = import_koishi2.Time.second;
var SELF_PACKAGE = "koishi-plugin-market-next";
var DEFAULT_INSTALL_LOG_RETENTION = import_koishi2.Time.day * 3;
...
function loadManifest(name2, baseDir) {
  const resolver = baseDir ? (0, import_module.createRequire)((0, import_path5.resolve)(baseDir, "package.json")) : require;
  const filename = resolver.resolve(name2 + "/package.json");
  const meta = JSON.parse((0, import_fs4.readFileSync)(filename, "utf8"));
  meta.dependencies ||= {};
...
function resolvePackageManifest(name2, baseDir) {
  const resolver = (0, import_module.createRequire)((0, import_path5.resolve)(baseDir, "package.json"));
  return resolver.resolve(name2 + "/package.json");
}
function getVersions(versions) {
  return Object.fromEntries(versions.map((item) => [item.version, (0, import_koishi2.pick)(item, ["peerDependencies", "peerDependenciesMeta", "deprecated"])]).sort(([a], [b]) => (0, import_semver5.compare)(b, a)));
}
var Installer = class extends import_koishi2.Service {
  constructor(ctx, config = {}) {
    super(ctx, "installer");
    this.ctx = ctx;
    this.config = config;
    this.manifest = loadManifest(this.cwd);
    this.statsFile = (0, import_path5.resolve)(ctx.baseDir, "cache", "market-next-registry-stats.json");
    this.environmentSnapshots = new EnvironmentSnapshotStore(
      (0, import_path5.resolve)(ctx.baseDir, "data", "market-next-environment-snapshots.json"),
      (message) => logger2.warn(message)
    );
    this.localPackageUploads = new LocalPackageUploadStore(ctx.baseDir, (message) => logger2.warn(message));
    ctx.setInterval(() => {
      void this.localPackageUploads.pruneExpired();
    }, import_koishi2.Time.minute * 5);
    ctx.effect(() => () => {
  ...
  installActive = false;
  installLogFile;
  installLogMetadataFile;
  installLogMetadata;
  installLogWriteTask = Promise.resolve();
  installLogCleanupTask;
  ...
  async start() {
    ...
    logger2.debug(`registry endpoint initialized: ${this.endpoint}, timeout=${this.config.timeout ?? "default"}, autoRoute=${this.config.autoRoute !== false}`);
    logger2.info(`npm registry endpoint initialized: ${this.endpoint}, timeout=${this.config.timeout ?? "default"}, autoRoute=${this.config.autoRoute !== false}`);
    await this.recordCurrentEnvironmentSnapshot("startup").catch((error) => {
      logger2.warn(`failed to record startup environment snapshot: ${error instanceof Error ? error.message : error}`);
    });
    const dependencies = this.getDeps({ background: false });
    logger2.info(`dependency startup metadata probe scheduled: deps=${Object.keys(dependencies).length}`);
  ...
  async loadRouteStats() {
    try {
      const content = await import_fs4.promises.readFile(this.statsFile, "utf8");
      ...
      for (const [endpoint, stats] of Object.entries(store.stats)) {
        ...
        const hasRecentSuccess = Number(stats.lastSuccess) && Date.now() - Number(stats.lastSuccess) < import_koishi2.Time.day;
      ...
      logger2.debug(`npm registry route stats restored from disk: ${Object.keys(store.stats).join(", ")}`);
    } catch (error) {
      if (error?.code !== "ENOENT") logger2.debug(`failed to load registry route stats: ${error instanceof Error ? error.message : error}`);
    }
  }
  scheduleStatsWrite() {
    clearTimeout(this.statsWriteTimer);
    this.statsWriteTimer = setTimeout(async () => {
      ...
      try {
        await import_fs4.promises.mkdir((0, import_path5.resolve)(this.statsFile, ".."), { recursive: true });
        await import_fs4.promises.writeFile(this.statsFile, JSON.stringify({ version: 1, stats, savedAt: Date.now() }));
      } catch (error) {
        logger2.debug(`failed to write registry route stats: ${error instanceof Error ? error.message : error}`);
      }
    }, 2e3);
  }
  async resetEndpoint() {
    ...
    if (previous && previous !== endpoint) {
      ...
      logger2.info(`npm registry endpoint changed: previous=${previous}, current=${endpoint}, routeStats=reset`);
    }
  }
  resolveName(name2) {
  ...
  getPreferredMetadataEndpoint() {
    ...
    if (stats.failures >= 2 && selectedScore + 1 < primaryScore) {
      logger2.debug(`demote npm metadata endpoint: selected=${endpoint}, selectedScore=${selectedScore.toFixed(1)}, primary=${this.endpoint}, primaryScore=${primaryScore.toFixed(1)}, failures=${stats.failures}, lastFailure=${stats.lastFailureReason ?? "-"}`);
  ...
  raceEndpoints(name2, endpoints, serial, onAttempt) {
    ...
    if (endpoints.length === 1 || this.config.autoRoute === false) {
      ...
      return this.fetchRegistryEndpoint(name2, endpoints[0], serial, controller.signal).then((r) => {
        this.recordRegistryRouteSuccess(r);
        return r;
      }).catch((e) => {
    ...
    return new Promise((resolve7, reject) => {
      let settled = false, failed = 0, lastError, fallbackStarted = false;
      ...
      const settle = (result, index) => {
        ...
        controllers.forEach((c, i) => {
          if (i !== index) c.abort(new Error("race settled"));
        });
      ...
      const startFallback = (reason) => {
        ...
        logger2.info(`npm registry fallback race started: probe=${name2}, reason=${reason}, count=${endpoints.length - 1}`);
  ...
  async probeMetadataEndpoint(name2, endpoints, serial) {
    ...
    logger2.info(`npm registry route probe started: probe=${name2}, primary=${endpoints[0]}, fallbackCount=${Math.max(0, endpoints.length - 1)}, slowThreshold=${this.getFallbackDelay(endpoints[0])}ms`);
    try {
      ...
      if (this.isStale(serial)) return;
      logger2.warn(`npm registry route probe failed: probe=${name2}, candidates=${endpoints.length}, elapsed=${Date.now() - start}ms`);
    }
  }
  async fetchRegistryEndpoint(name2, endpoint, serial, signal) {
    const attemptStart = Date.now();
    try {
      logger2.debug(`fetch npm registry endpoint: package=${name2}, endpoint=${endpoint}`);
      ...
      logger2.debug(`fetch npm registry endpoint succeeded: package=${name2}, endpoint=${endpoint}, elapsed=${elapsed}ms, versions=${Object.keys(registry.versions).length}`);
      return { endpoint, registry, elapsed };
    } catch (error) {
      const detail = this.formatRegistryError(error);
      logger2.debug(`fetch npm registry endpoint failed: package=${name2}, endpoint=${endpoint}, elapsed=${Date.now() - attemptStart}ms, reason=${detail.reason}, error=${detail.error}`);
      throw error;
    }
  }
  applyRouteProbeResult(name2, result, serial, start) {
    ...
    if (result.endpoint === this.endpoint) {
      logger2.info(`npm registry primary selected: probe=${name2}, endpoint=${result.endpoint}, elapsed=${result.elapsed}ms, total=${Date.now() - start}ms`);
    } else {
      logger2.info(`npm registry fallback selected: probe=${name2}, endpoint=${result.endpoint}, previous=${previous}, reason=${result.fallbackReason ?? "unknown"}, elapsed=${result.elapsed}ms, total=${Date.now() - start}ms`);
    }
    logger2.debug(`npm registry route scores after probe: ${formatRouteScores(this.getRegistryRouteScores())}`);
  }
  waitRouteTurn(index, signal) {
    if (!index) return Promise.resolve();
    return new Promise((resolve7, reject) => {
      if (signal?.aborted) return reject(signal.reason);
      const timer = setTimeout(resolve7, index * REGISTRY_ROUTE_STAGGER);
  ...
  getRegistryRouteScore(endpoint) {
    const stats = this.registryRouteStats[endpoint];
    ...
    if (stats.lastSuccess && Date.now() - stats.lastSuccess <= import_koishi2.Time.minute * 10) score += 1.5;
    score -= Math.min(5, (stats.consecutiveFailures ?? 0) * 1.5);
    return score;
  }
  recordRegistryRouteSuccess(result) {
  ...
  recordRegistryRouteFailure(endpoint, reason) {
  ...
  getFallbackDelay(endpoint) {
    ...
    const recentSuccess = stats.lastSuccess && Date.now() - stats.lastSuccess <= import_koishi2.Time.minute * 10;
  ...
  async getRegistry(name2, serial = this.serial) {
    ...
    if (probe?.serial === serial && probe.name === name2 && probe.endpoint === this.metadataEndpoint) {
      ...
      logger2.debug(`reuse npm registry route probe payload for ${name2}: endpoint=${probe.endpoint}, probeElapsed=${probe.elapsed}ms`);
      return probe.registry;
    }
    for (let retry = 0; retry <= maxRetry; retry++) {
      const endpoints = this.getRegistryEndpoints();
      logger2.debug(`registry metadata candidates for ${name2}: endpoints=${endpoints.join(", ")}, retry=${retry + 1}/${maxRetry + 1}, concurrency=${this.config.concurrency ?? 4}`);
      try {
        ...
        if (result.endpoint !== this.metadataEndpoint) {
          logger2.debug(`routed npm registry endpoint for ${name2}: ${result.endpoint}`);
          logger2.info(`npm registry route selected for ${name2}: endpoint=${result.endpoint}, previous=${this.metadataEndpoint}, reason=${result.fallbackReason ?? "same-priority"}, elapsed=${result.elapsed}ms`);
          this.metadataEndpoint = result.endpoint;
        }
        this.setRegistryStatus(name2, {
          loading: false,
        ...
        logger2.debug(`loaded registry metadata for ${name2} from ${result.endpoint} in ${result.elapsed}ms, attempts=${attempts}, versions=${Object.keys(result.registry.versions).length}`);
        return result.registry;
      } catch (error) {
        lastError = error;
        const detail2 = this.formatRegistryError(error);
        failureReasons.push(...getRegistryAttemptReasons(error, detail2.reason));
        logger2.debug(`failed routed registry metadata for ${name2}, attempt=${retry + 1}/${maxRetry + 1}, endpoint=${lastEndpoint}, attempts=${attempts}: ${detail2.error}`);
    ...
    logger2.warn(`failed to fetch registry metadata for ${name2}: ${detail.error}`);
  ...
  async _getPackage(name2, serial = this.serial) {
    try {
      ...
      this.fullCache[name2] = this.tempCache[name2] = getVersions(Object.values(registry.versions).filter((remote) => {
        if (name2 === "koishi") return (0, import_semver5.satisfies)(remote.version, "4");
        return !import_registry2.default.isPlugin(name2) || import_registry2.default.isCompatible("4", remote);
      }));
      this.flushData();
      return this.fullCache[name2];
    } catch (e) {
      logger2.warn(e instanceof Error ? e.message : e);
      throw e;
    }
  }
  ...
  markRegistryNotFoundDependency(name2, dependency = this.depCache[name2]) {
    const source = classifyRegistryNotFoundDependency(dependency, import_registry2.default.isPlugin(name2));
    if (!source || !dependency) return false;
    Object.assign(dependency, source);
    dependency.invalid = false;
    delete dependency.latest;
    logger2.info(`dependency classified as unbound local plugin: ${name2}@${dependency.resolved}`);
    return true;
  }
  getLocalDepsSnapshot() {
    ...
    for (const name2 of names) {
      try {
        ...
        logger2.debug(`local dependency resolved: ${name2}@${meta.version}, workspace=${!!meta.$workspace}, request=${result[name2].request}`);
      } catch {
        logger2.debug(`local dependency not found before metadata fetch: ${name2}, request=${result[name2].request}`);
      }
      ...
      if (!result[name2].local && !(0, import_semver5.valid)(result[name2].request)) {
        result[name2].invalid = true;
        logger2.debug(`dependency request is not exact semver: ${name2}, request=${result[name2].request}`);
      }
    ...
    logger2.info(`dependency local snapshot ready: total=${names.length}, installed=${installed}, invalid=${invalid}, elapsed=${Date.now() - start}ms`);
    return result;
  }
  async _refreshDependencyMetadata(result = this.depCache, serial = this.serial) {
    const start = Date.now();
    const names = Object.keys(result);
    const targets = names.filter((name2) => !result[name2].local && !result[name2].invalid);
    logger2.debug(`refresh dependency metadata started: total=${names.length}, targets=${targets.length}, concurrency=${this.config.concurrency ?? 4}, registry=${this.endpoint}, autoRoute=${this.config.autoRoute !== false}`);
    const probeName = pickMetadataProbe(targets);
    if (probeName) await this.ensureMetadataEndpoint(probeName, this.serial);
    logger2.debug(`refresh dependency metadata route ready: probe=${probeName ?? "-"}, selected=${this.metadataEndpoint}, configured=${this.endpoint}, probed=${!!this.routeProbeResult}`);
    await (0, import_p_map.default)(targets, async (name2) => {
      ...
      try {
        ...
        if (versions) {
          ...
          logger2.debug(`dependency latest resolved: ${name2}, resolved=${result[name2].resolved ?? "-"}, latest=${result[name2].latest}, versions=${Object.keys(versions).length}`);
        } else if (!versions && this.notFoundCache[name2] && this.markRegistryNotFoundDependency(name2, result[name2])) {
          logger2.debug(`dependency npm not-found result reused from cache: ${name2}`);
        } else {
          logger2.debug(`dependency latest unresolved: ${name2}, resolved=${result[name2].resolved ?? "-"}, request=${result[name2].request}`);
        }
      } catch (error) {
        if (this.isStale(serial)) return;
        const detail = this.formatRegistryError(error);
        if (detail.reason === "not-found" && this.markRegistryNotFoundDependency(name2, result[name2])) {
          ...
          logger2.debug(`dependency metadata refresh skipped after error: ${name2}, reason=${detail.reason}, error=${detail.error}`);
        }
      }
    }, { concurrency: this.config.concurrency ?? 4 });
    logger2.info(`dependency metadata refresh completed: total=${names.length}, targets=${targets.length}, registry=${this.metadataEndpoint}, elapsed=${Date.now() - start}ms`);
  ...
  refreshDependencyMetadata(wait = false) {
    ...
    if (!this.depTask) {
      ...
      task.then(() => {
        ...
        if (this.depTask === task) this.depTask = void 0;
        logger2.warn(`dependency metadata refresh failed: ${error instanceof Error ? error.message : error}`);
      });
    }
  ...
  async probeDependenciesInBackground(reason = "background") {
    const start = Date.now();
    if (this.depTask) {
      logger2.debug(`reuse running dependency metadata task for ${reason} probe`);
      await this.depTask;
      await this.refreshData();
      logger2.info(`dependency ${reason} probe reused running metadata task: elapsed=${Date.now() - start}ms`);
      return;
    }
    this.serial++;
    ...
    logger2.info(`dependency ${reason} probe started: deps=${Object.keys(this.manifest.dependencies ?? {}).length}`);
    await this.refreshDependencyMetadata(true);
    await this.refreshData();
    logger2.info(`dependency ${reason} probe completed: deps=${Object.keys(this.manifest.dependencies ?? {}).length}, elapsed=${Date.now() - start}ms`);
  }
  getDeps(options = {}) {
  ...
  async refresh(refresh = false, waitMetadata = false) {
    ...
    logger2.info(`dependency refresh requested by console: deps=${Object.keys(this.manifest.dependencies ?? {}).length}, waitMetadata=${waitMetadata}, elapsed=${Date.now() - start}ms`);
  }
  getInstallLogDir() {
    return (0, import_path5.resolve)(this.cwd, "data", INSTALL_LOG_DIR);
  }
  getInstallLogRetention() {
    const hours = Number(this.config.installLogRetentionHours);
    if (Number.isFinite(hours) && hours > 0) return Math.max(1, hours) * import_koishi2.Time.hour;
    const legacyRetention = Number(this.config.installLogRetention);
    return Number.isFinite(legacyRetention) && legacyRetention > 0 ? Math.max(import_koishi2.Time.hour, legacyRetention) : DEFAULT_INSTALL_LOG_RETENTION;
  }
  async cleanupInstallLogs() {
    ...
    this.installLogCleanupTask = (async () => {
      ...
      try {
        ...
        await Promise.all(entries.filter((entry) => entry.isFile() && (entry.name.endsWith(".log") || entry.name.endsWith(".log.json"))).map(async (entry) => {
          const path = (0, import_path5.resolve)(dir, entry.name);
          ...
          try {
            ...
            logger2.debug(`failed to cleanup install log ${path}: ${error instanceof Error ? error.message : error}`);
          }
        }));
      } catch (error) {
        if (error?.code !== "ENOENT") {
          logger2.debug(`failed to cleanup install logs: ${error instanceof Error ? error.message : error}`);
        }
      }
    })().finally(() => {
      this.installLogCleanupTask = void 0;
    });
    return this.installLogCleanupTask;
  }
  async writeInstallLogMetadata() {
    if (!this.installLogMetadataFile || !this.installLogMetadata) return;
    await import_fs4.promises.writeFile(this.installLogMetadataFile, JSON.stringify(this.installLogMetadata, null, 2) + "\n");
  }
  async startInstallLog(deps, forced, options = {}, changes = []) {
    await this.cleanupInstallLogs();
    const dir = this.getInstallLogDir();
    await import_fs4.promises.mkdir(dir, { recursive: true });
    const now = Date.now();
    const suffix = sanitizeLogSegment(formatDeps(deps) || "noop");
    const file = (0, import_path5.resolve)(dir, `${formatLogTimestamp(now)}-${suffix}.log`);
    const id = (0, import_path5.basename)(file);
    await import_fs4.promises.writeFile(file, [
    ...
    await this.writeInstallLogMetadata().catch((error) => {
      logger2.debug(`failed to write install log metadata ${this.installLogMetadataFile}: ${error instanceof Error ? error.message : error}`);
    });
    logger2.info(`dependency install log started: ${file}`);
  }
  ...
  writeInstallLog(type, line) {
    const file = this.installLogFile;
    if (!file) return;
    const text = `[${(/* @__PURE__ */ new Date()).toISOString()}] [${type}] ${line}
`;
    this.installLogWriteTask = this.installLogWriteTask.then(() => import_fs4.promises.appendFile(file, text)).catch((error) => {
      logger2.debug(`failed to write install log ${file}: ${error instanceof Error ? error.message : error}`);
    });
  }
  async finishInstallLog(result) {
    ...
    if (this.installLogMetadata) {
      ...
      await this.writeInstallLogMetadata().catch((error) => {
        logger2.debug(`failed to finish install log metadata ${this.installLogMetadataFile}: ${error instanceof Error ? error.message : error}`);
      });
    }
    logger2.info(`dependency install log saved: ${this.installLogFile}`);
    this.installLogFile = void 0;
    this.installLogMetadataFile = void 0;
    this.installLogMetadata = void 0;
    this.installLogWriteTask = Promise.resolve();
  }
  getInstallLogPath(id) {
    if (!id || (0, import_path5.basename)(id) !== id || !id.endsWith(".log")) return;
    return (0, import_path5.resolve)(this.getInstallLogDir(), id);
  }
  async readInstallLogMetadata(id) {
    const file = this.getInstallLogPath(id);
    if (!file) return;
    try {
      const metadata = JSON.parse(await import_fs4.promises.readFile(file + ".json", "utf8"));
      if (metadata?.version !== 1 || metadata.id !== id || !Array.isArray(metadata.changes)) return;
      return metadata;
    } catch (error) {
      if (error?.code !== "ENOENT" && !(error instanceof SyntaxError)) {
        logger2.debug(`failed to read install log metadata ${id}: ${error instanceof Error ? error.message : error}`);
      }
    }
  }
  async readInstallLog(file, limit, headLimit, tailLimit) {
    const stat = await import_fs4.promises.stat(file);
    if (stat.size <= limit) {
      return {
        content: await import_fs4.promises.readFile(file, "utf8"),
        truncated: false,
        size: stat.size
      };
    }
    const handle = await import_fs4.promises.open(file, "r");
    try {
...
${tail.toString("utf8")}`,
  ...
  parseLegacyInstallLog(id, content, size) {
    ...
    const active = (0, import_path5.basename)(this.installLogFile || "") === id;
  ...
  createInstallHistoryEntry(metadata, size) {
    const status = metadata.status === "running" && (0, import_path5.basename)(this.installLogFile || "") !== metadata.id ? "unknown" : metadata.status;
    return {
      id: metadata.id,
  ...
  async getInstallHistoryEntry(id) {
    const file = this.getInstallLogPath(id);
    if (!file) return;
    if (file === this.installLogFile) await this.installLogWriteTask;
    let stat;
    try {
      stat = await import_fs4.promises.stat(file);
    } catch (error) {
  ...
  async getInstallHistory(limit = 20) {
    ...
    try {
      ...
      files = (await Promise.all(entries.filter((entry) => entry.isFile() && entry.name.endsWith(".log")).map(async (entry) => ({
        id: entry.name,
        mtime: (await import_fs4.promises.stat((0, import_path5.resolve)(dir, entry.name))).mtimeMs
  ...
  async exec(args) {
    ...
    const start = Date.now();
    logger2.info(`run package manager: agent=${name2}${this.agent?.version ? "@" + this.agent.version : ""}, args=${args.join(" ") || "(none)"}, cwd=${this.cwd}, json=${useJson}`);
    return new Promise((resolve7) => {
      ...
      const emitStdoutLine = (line) => {
        ...
        try {
          ...
          logger2[levelMap[type] ?? "info"](data);
      ...
      child.on("exit", (code, signal) => {
        logger2.info(`package manager exited: code=${code}, signal=${signal ?? "-"}, elapsed=${Date.now() - start}ms`);
        if (code == null) {
      ...
      child.on("error", (error) => {
        logger2.warn(`package manager failed to start: ${error instanceof Error ? error.message : String(error)}`);
  ...
  async override(deps) {
    const filename = (0, import_path5.resolve)(this.cwd, "package.json");
    logger2.debug(`override package dependencies: file=${filename}, changes=${formatDeps(deps)}`);
    ...
    await import_fs4.promises.writeFile(filename, JSON.stringify(this.manifest, null, 2) + "\n");
    logger2.info(`package dependencies updated: changes=${formatDeps(deps)}, total=${Object.keys(this.manifest.dependencies).length}`);
  }
  async snapshotPackageManifest() {
    const filename = (0, import_path5.resolve)(this.cwd, "package.json");
    const content = await import_fs4.promises.readFile(filename, "utf8");
  ...
  async restorePackageManifest(snapshot, deps, reason) {
    ...
    let manifest;
    try {
      manifest = JSON.parse(await import_fs4.promises.readFile(filename, "utf8"));
    } catch {
    ...
    await import_fs4.promises.writeFile(filename, JSON.stringify(manifest, null, 2) + "\n");
    this.manifest = manifest;
    this.depCache = this.getLocalDepsSnapshot();
    this.depMetadataFresh = false;
    logger2.warn(`package dependencies rolled back: reason=${reason}, changes=${formatDeps(deps)}, total=${Object.keys(this.manifest.dependencies ?? {}).length}`);
  }
  _install(options = {}) {
    options ||= {};
    const args = [];
  ...
  _getLocalDeps(override) {
    return (0, import_koishi2.valueMap)(override, (request, name2) => {
      const dep = { request };
      try {
        const meta = loadManifest(name2, this.cwd);
  ...
  requiresPackageManager(deps, localDeps, forced) {
    ...
    for (const name2 in deps) {
      ...
      if (local || resolved && (0, import_semver5.satisfies)(resolved, nextRequest, { includePrerelease: true })) continue;
      return true;
    }
    return false;
  }
  async captureCurrentEnvironmentSnapshot(source, operationId) {
    ...
    for (const [name2, request] of Object.entries(manifest.dependencies)) {
      ...
      dependencies[name2] = {
        ...
        invalid: !local[name2]?.local && !(0, import_semver5.valid)(normalizedRequest)
      };
    }
  ...
  async _installLocked(deps, forced, beforeReload, options = {}) {
    ...
    await this.recordCurrentEnvironmentSnapshot("external").catch((error) => {
      logger2.warn(`failed to record pre-operation environment snapshot: ${error instanceof Error ? error.message : error}`);
    });
    await this.startInstallLog(deps, forced, options, changes).catch((error) => {
      ...
      this.installLogMetadata = void 0;
      this.installLogWriteTask = Promise.resolve();
      logger2.warn(`failed to start dependency install log: ${error instanceof Error ? error.message : error}`);
    });
    logger2.info(`dependency install requested: deps=${formatDeps(deps)}, forced=${!!forced}, installEndpoint=${options.installEndpoint || "(default)"}`);
    try {
      ...
      logger2.debug(`dependency install local state: ${formatLocalDeps(localDeps)}`);
      ...
      if (needsPackageManager) {
        ...
        if (unresolved.length) {
          logger2.info(`resolve possible local plugin sources before package manager: ${unresolved.join(", ")}`);
          const unresolvedErrors = await Promise.all(unresolved.map(async (name2) => {
            try {
              const versions = await this.getPackage(name2);
              if (versions) return;
              if (this.notFoundCache[name2]) {
      ...
      for (const name2 in localDeps) {
        ...
        shouldReload = true;
        logger2.debug(`dependency changed may require full reload: ${name2}, previous=${resolved ?? "-"}, current=${newDeps[name2]?.resolved ?? "-"}`);
      }
      if (beforeReload) {
        logger2.debug("run pre-reload dependency hook");
        await beforeReload();
      }
      await this.refreshData();
      await this.recordCurrentEnvironmentSnapshot("operation", this.installLogMetadata?.id).catch((error) => {
        logger2.warn(`failed to record dependency environment snapshot: ${error instanceof Error ? error.message : error}`);
      });
      logger2.info(`dependency install completed: deps=${formatDeps(deps)}, forced=${!!needsPackageManager}, fullReload=${shouldReload}, elapsed=${Date.now() - start}ms`);
      if (shouldReload) {
        this.emitInstallLog("stdout", `full reload scheduled in ${FULL_RELOAD_DELAY}ms`);
        logger2.info(`dependency install triggers full reload after ${FULL_RELOAD_DELAY}ms`);
        setTimeout(() => {
      ...
      await this.finishInstallLog(logResult).catch((error) => {
        logger2.warn(`failed to finish dependency install log: ${error instanceof Error ? error.message : error}`);
      });
    }
  }
  async withInstallLock(description2, callback) {
    ...
    let release;
    this.installTask = new Promise((resolve7) => {
      release = resolve7;
    });
    if (this.installActive) logger2.info(`dependency install queued: ${description2}`);
    await previous;
  ...
  async prepareLocalBinding(name2) {
    ...
    if (!import_registry2.default.isPlugin(name2) || !Object.prototype.hasOwnProperty.call(packageSnapshot.dependencies, name2)) {
      throw new Error("\u53EA\u80FD\u7ED1\u5B9A\u5F53\u524D package.json \u4E2D\u7684 Koishi \u63D2\u4EF6\u4F9D\u8D56\u3002");
    }
    const dependency = this.depCache[name2];
    if (!dependency?.resolved || dependency.source !== "unbound") {
      throw new Error("\u8BE5\u63D2\u4EF6\u4E0D\u662F\u6765\u6E90\u672A\u7ED1\u5B9A\u7684\u672C\u5730\u63D2\u4EF6\u3002");
    }
    const currentRequest = packageSnapshot.dependencies[name2].replace(/^[~^]/, "");
    if (dependency.request !== currentRequest) {
      throw new Error("package.json \u5DF2\u53D1\u751F\u53D8\u5316\uFF0C\u8BF7\u5237\u65B0\u4F9D\u8D56\u540E\u91CD\u8BD5\u3002");
    }
    let manifestFile;
    try {
      manifestFile = resolvePackageManifest(name2, this.cwd);
    } catch {
      throw new Error("\u65E0\u6CD5\u5B9A\u4F4D\u672C\u5730\u63D2\u4EF6\u76EE\u5F55\uFF1B\u8BF7\u786E\u8BA4\u63D2\u4EF6\u4ECD\u53EF\u88AB\u5F53\u524D Koishi \u5B9E\u4F8B\u52A0\u8F7D\u3002");
    }
    const sourceDir = (0, import_path5.dirname)(manifestFile);
    const manifest = JSON.parse(await import_fs4.promises.readFile(manifestFile, "utf8"));
    if (manifest.name !== name2 || manifest.version !== dependency.resolved) {
      throw new Error("\u672C\u5730\u63D2\u4EF6\u6E05\u5355\u4E0E\u5F53\u524D\u4F9D\u8D56\u72B6\u6001\u4E0D\u4E00\u81F4\uFF0C\u8BF7\u5237\u65B0\u4F9D\u8D56\u540E\u91CD\u8BD5\u3002");
    }
    const destination = (0, import_path5.resolve)(this.cwd, ".yarn", "local");
    await import_fs4.promises.mkdir(destination, { recursive: true });
    const temporary = await import_fs4.promises.mkdtemp((0, import_path5.resolve)(destination, ".market-next-pack-"));
    try {
      const child = await (0, import_execa.default)("npm", [
        ...
        cwd: this.cwd,
        timeout: Math.max(import_koishi2.Time.minute, this.config.timeout ?? 0)
      });
      ...
      const packedFile = (0, import_path5.resolve)(temporary, pack.filename);
      if ((0, import_path5.dirname)(packedFile) !== temporary || (0, import_path5.relative)(temporary, packedFile).startsWith("..")) {
        throw new Error("\u672C\u5730\u63D2\u4EF6\u6253\u5305\u8DEF\u5F84\u65E0\u6548\u3002");
      }
      const stat = await import_fs4.promises.stat(packedFile);
      if (!stat.isFile() || stat.size <= 0 || stat.size > MAX_LOCAL_BINDING_PACK_SIZE || stat.size !== pack.size) {
        throw new Error("\u672C\u5730\u63D2\u4EF6\u6253\u5305\u6587\u4EF6\u65E0\u6548\u6216\u8FC7\u5927\u3002");
      }
      const content = await import_fs4.promises.readFile(packedFile);
      const hash = (0, import_crypto3.createHash)("sha256").update(content).digest("hex");
      const filename = createHashedLocalBindingFilename(pack.filename, hash.slice(0, 12));
      const target = (0, import_path5.resolve)(destination, filename);
      if ((0, import_path5.dirname)(target) !== destination || (0, import_path5.relative)(destination, target).startsWith("..")) {
        throw new Error("\u672C\u5730\u63D2\u4EF6\u76EE\u6807\u8DEF\u5F84\u65E0\u6548\u3002");
      }
      const validateExistingTarget = async () => {
        let targetStat;
        try {
          targetStat = await import_fs4.promises.stat(target);
        } catch (error) {
        ...
        const targetHash = (0, import_crypto3.createHash)("sha256").update(await import_fs4.promises.readFile(target)).digest("hex");
        if (targetHash !== hash) {
          throw new Error("\u540C\u540D\u672C\u5730\u63D2\u4EF6\u5F52\u6863\u5DF2\u5B58\u5728\uFF0C\u4F46\u6587\u4EF6\u5185\u5BB9\u4E0D\u4E00\u81F4\u3002");
        }
        return true;
      ...
      if (!await validateExistingTarget()) {
        try {
          await import_fs4.promises.rename(packedFile, target);
        } catch (error) {
      ...
      await import_fs4.promises.rm(temporary, { recursive: true, force: true }).catch((error) => {
  ...
  async applyEnvironmentSnapshot(id, options = {}) {
    ...
    return this.withInstallLock(`environmentSnapshot=${id}`, async () => {
      ...
      logger2.info(`environment snapshot restore requested: target=${id}, changes=${formatDeps(changes)}`);
...
((Installer2) => {
  Installer2.Config = import_koishi2.Schema.object({
    ...
    timeout: import_koishi2.Schema.number().role("time").default(import_koishi2.Time.second * 5),
    autoRoute: import_koishi2.Schema.boolean().default(true),
    retry: import_koishi2.Schema.number().min(0).max(5).step(1).default(1),
    concurrency: import_koishi2.Schema.number().min(1).max(16).step(1).default(4),
    installLogRetentionHours: import_koishi2.Schema.number().min(1).max(24 * 365).step(1).default(72)
  });
})(Installer || (Installer = {}));
function sleep(ms) {
...
function pickMetadataProbe(names) {
  return names.find((name2) => name2 === "koishi") || names.find((name2) => name2 === "@koishijs/plugin-console") || names.find((name2) => import_registry2.default.isPlugin(name2)) || names[0];
}
...
var FAST_ROUTE_THRESHOLD = import_koishi3.Time.second * 0.5;
var MAX_CACHE_ENTRIES = 3;
var CACHE_ENTRY_TTL = import_koishi3.Time.day * 30;
...
function getRouteCooldown(failures = 0) {
  ...
  if (failures === 1) return import_koishi3.Time.minute;
  if (failures === 2) return import_koishi3.Time.minute * 5;
  if (failures === 3) return import_koishi3.Time.minute * 30;
  if (failures === 4) return import_koishi3.Time.hour * 4;
  return import_koishi3.Time.hour * 12;
}
  ...
  try {
    return await Promise.race([
      ...
      new Promise((resolve7) => {
        timer = setTimeout(() => resolve7(false), timeout);
      })
    ]);
  } finally {
    clearTimeout(timer);
...
var MarketDiskCache = class {
  constructor(ctx, config, options) {
    ...
    this.file = (0, import_path6.resolve)(ctx.baseDir, "cache", "market-next-index.json");
    this.directory = (0, import_path6.resolve)(ctx.baseDir, "cache", "market-next-index");
  }
  ctx;
  config;
  options;
  ...
  async load(serial) {
    ...
    try {
      ...
      const content = await import_fs5.promises.readFile(this.file, "utf8");
      ...
      if (staleEndpoints.length) {
        this.options.log("debug", `market disk cache has ${staleEndpoints.length} stale entries (>${CACHE_ENTRY_TTL / import_koishi4.Time.day}d), will prune on next write: ${staleEndpoints.join(", ")}`);
      }
  ...
  async loadEntry(entry) {
    ...
    try {
      const content = await import_fs5.promises.readFile((0, import_path6.resolve)(this.directory, entry.file), "utf8");
  ...
  getScore(cache2) {
    ...
    if (age <= import_koishi4.Time.hour * 12) score += 3;
    else if (age <= import_koishi4.Time.day * 3) score += 1;
    else score -= 1;
  ...
  async writeEntryFile(entry) {
    await import_fs5.promises.mkdir(this.directory, { recursive: true });
    const file = (0, import_path6.resolve)(this.directory, this.getEntryFilename(entry.endpoint));
    const tempFile = `${file}.${process.pid}.${Date.now()}.tmp`;
    await import_fs5.promises.writeFile(tempFile, JSON.stringify(entry.result));
    await import_fs5.promises.rename(tempFile, file);
  }
  async pruneSplitFiles(entries) {
    try {
      const keep = new Set(Object.values(entries).map((entry) => entry?.file).filter(Boolean));
      const files = await import_fs5.promises.readdir(this.directory).catch(() => []);
      await Promise.all(files.filter((file) => file.endsWith(".json") && !keep.has(file)).map((file) => import_fs5.promises.unlink((0, import_path6.resolve)(this.directory, file)).catch(() => {
      })));
    } catch (error) {
  ...
  async writeStore(cache2) {
    if (!this.options.isActive()) return;
    try {
      await import_fs5.promises.mkdir((0, import_path6.dirname)(this.file), { recursive: true });
      const entries = {};
      ...
      const tempFile = `${this.file}.${process.pid}.${Date.now()}.tmp`;
      await import_fs5.promises.writeFile(tempFile, nextStore);
      await import_fs5.promises.rename(tempFile, this.file);
...
var MarketRouter = class {
  ...
  getScore(endpoint) {
    ...
    if (cached) {
      ...
      score += age <= import_koishi5.Time.day ? 1.5 : 0.5;
    }
    if (!stats) return score;
    const total = stats.successes + stats.failures;
    if (total) {
    ...
    if (stats.lastSuccess && Date.now() - stats.lastSuccess <= import_koishi5.Time.minute * 10) score += 1.5;
    score -= Math.min(5, (stats.consecutiveFailures ?? 0) * 1.5);
    return score;
  }
  ...
  restoreStats(persisted) {
    for (const [endpoint, stats] of Object.entries(persisted)) {
      if (!stats) continue;
      const hasRecentSuccess = stats.lastSuccess && Date.now() - stats.lastSuccess < import_koishi5.Time.day;
      this.stats[endpoint] = {
        score: hasRecentSuccess ? clamp2(stats.score, -1, 3) : clamp2(stats.score, -4, 3),
        successes: 0,
        failures: 0,
  ...
  waitRouteTurn(index, signal) {
    ...
    return new Promise((resolve7, reject) => {
      if (signal?.aborted) return reject(signal.reason);
      const timer = setTimeout(resolve7, index * ROUTE_STAGGER);
      signal?.addEventListener("abort", () => {
        clearTimeout(timer);
        reject(signal.reason);
      }, { once: true });
  ...
  recordSuccess(result) {
    ...
    stats.cooldownUntil = void 0;
    stats.failures = Math.max(0, Math.floor(stats.failures * 0.6));
    stats.score = clamp2(stats.score + (result.elapsed <= FAST_ROUTE_THRESHOLD ? 0.4 : 0.1), -6, 3);
    stats.lastSuccess = Date.now();
  ...
  recordFailure(endpoint, options = {}) {
    ...
    stats.failures++;
    if (options.rescue) {
      stats.score = clamp2(stats.score - 0.25, -10, 3);
      ...
      stats.score = clamp2(stats.score - 1.2, -10, 3);
...
var MarketProvider2 = class extends MarketProvider {
  ...
  createScanner() {
    ...
    this.scanner = new import_registry3.default((url, config) => registry.get(url, config));
    return registry;
  }
...
((MarketProvider3) => {
  MarketProvider3.Config = import_koishi6.Schema.object({
    endpoint: import_koishi6.Schema.string().role("link").default(DEFAULT_ENDPOINT),
    timeout: import_koishi6.Schema.number().role("time").default(import_koishi6.Time.second * 30),
    proxyAgent: import_koishi6.Schema.string().role("link"),
    autoRoute: import_koishi6.Schema.boolean().default(true),
    logLevel: import_koishi6.Schema.union(logLevels.map((level) => import_koishi6.Schema.const(level))).default("warn")
  });
...
var CACHE_TTL = import_koishi7.Time.minute * 10;
var DAY = import_koishi7.Time.day;
...
async function fetchIndex(ctx, config) {
  ...
  const http = ctx.http.extend({
    endpoint,
    timeout: config.search?.timeout ?? import_koishi7.Time.second * 30,
...
function normalizeInput(input) {
  ...
  return {
    ...
    limit: clamp3(input.limit ?? 10, 1, 50),
...
var AVATAR_CACHE_TTL = import_koishi8.Time.day * 7;
var AVATAR_CACHE_SWEEP_INTERVAL = import_koishi8.Time.hour;
...
function getAvatarCacheDir(ctx) {
  return (0, import_path7.resolve)(ctx.baseDir, "cache", "market-next-avatars");
}
...
function getAvatarCacheFile(ctx, key) {
  return (0, import_path7.resolve)(getAvatarCacheDir(ctx), `${(0, import_crypto6.createHash)("sha1").update(normalizeAvatarCacheKey(key)).digest("hex")}.json`);
}
...
async function readAvatarDiskCache(ctx, key) {
  try {
    ...
    const entry = normalizeAvatarDiskCache(JSON.parse(await import_fs6.promises.readFile(file, "utf8")), key);
    if (!entry) {
      void import_fs6.promises.unlink(file).catch(() => {
      });
      return;
    }
...
async function writeAvatarDiskCache(ctx, key, url, result) {
  try {
    await import_fs6.promises.mkdir(getAvatarCacheDir(ctx), { recursive: true });
    const file = getAvatarCacheFile(ctx, key);
    const tempFile = `${file}.${process.pid}.${Date.now()}.tmp`;
    const entry = {
      key,
    ...
    await import_fs6.promises.writeFile(tempFile, JSON.stringify(entry));
    await import_fs6.promises.rename(tempFile, file);
  } catch (error) {
...
async function cleanupAvatarDiskCache(ctx) {
  ...
  avatarDiskCleanupTask = (async () => {
    try {
      ...
      const files = await import_fs6.promises.readdir(dir).catch(() => []);
      const entries = await Promise.all(files.filter((file) => file.endsWith(".json")).map(async (file) => {
        const path = (0, import_path7.resolve)(dir, file);
        try {
          const stat = await import_fs6.promises.stat(path);
          let cachedAt = stat.mtimeMs;
          try {
            const value = JSON.parse(await import_fs6.promises.readFile(path, "utf8"));
            cachedAt = Number(value.cachedAt) || cachedAt;
            if (!value?.key || !value?.url || typeof value?.data !== "string" || typeof value?.type !== "string") {
              await import_fs6.promises.unlink(path).catch(() => {
              });
              return;
            }
      ...
      await Promise.all(alive.slice(AVATAR_MAX_ENTRIES).map((entry) => import_fs6.promises.unlink(entry.path).catch(() => {
      })));
    } finally {
...
async function clearAvatarCacheStorage(ctx) {
  ...
  const dir = getAvatarCacheDir(ctx);
  const files = await import_fs6.promises.readdir(dir).catch((error) => {
    if (error?.code === "ENOENT") return [];
    throw error;
  });
  const disk = files.filter((file) => file.endsWith(".json")).length;
  await import_fs6.promises.rm(dir, { recursive: true, force: true });
  return { memory, disk };
}
...
var MarketSilentRuleType = import_koishi9.Schema.union([
  import_koishi9.Schema.const("preview").description("\u72B6\u6001\uFF1A\u9884\u89C8\u7248\u63D2\u4EF6"),
  import_koishi9.Schema.const("insecure").description("\u72B6\u6001\uFF1A\u4E0D\u5B89\u5168\u63D2\u4EF6"),
  import_koishi9.Schema.const("bundle").description("\u72B6\u6001\uFF1A\u63D2\u4EF6\u5305"),
  import_koishi9.Schema.const("created-before").description("\u521B\u5EFA\u65F6\u95F4\uFF1A\u65E9\u4E8E\u6307\u5B9A\u65E5\u671F"),
  import_koishi9.Schema.const("created-after").description("\u521B\u5EFA\u65F6\u95F4\uFF1A\u665A\u4E8E\u6307\u5B9A\u65E5\u671F"),
  import_koishi9.Schema.const("updated-before").description("\u66F4\u65B0\u65F6\u95F4\uFF1A\u65E9\u4E8E\u6307\u5B9A\u65E5\u671F"),
  import_koishi9.Schema.const("updated-after").description("\u66F4\u65B0\u65F6\u95F4\uFF1A\u665A\u4E8E\u6307\u5B9A\u65E5\u671F"),
  import_koishi9.Schema.const("created-within").description("\u521B\u5EFA\u65F6\u95F4\uFF1A\u6700\u8FD1 N \u5929\u5185"),
  import_koishi9.Schema.const("updated-within").description("\u66F4\u65B0\u65F6\u95F4\uFF1A\u6700\u8FD1 N \u5929\u5185"),
  import_koishi9.Schema.const("custom").description("\u81EA\u5B9A\u4E49\u9AD8\u7EA7\u6761\u4EF6")
]);
var MarketSilentRules = import_koishi9.Schema.array(import_koishi9.Schema.object({
  type: MarketSilentRuleType.default("preview").description("\u89C4\u5219\u7C7B\u578B"),
  value: import_koishi9.Schema.string().default("").description("\u89C4\u5219\u503C\u3002\u72B6\u6001\u7C7B\u7559\u7A7A\uFF1B\u65E5\u671F\u7C7B\u586B\u5199 YYYY-MM-DD\uFF0C\u4F8B\u5982 2024-01-01\uFF1B\u6700\u8FD1 N \u5929\u586B\u5199\u6570\u5B57\uFF0C\u4F8B\u5982 30\uFF1B\u81EA\u5B9A\u4E49\u89C4\u5219\u586B\u5199\u641C\u7D22\u6761\u4EF6\uFF0C\u4F8B\u5982 category:adapter\u3002"),
  note: import_koishi9.Schema.string().default("").description("\u5907\u6CE8"),
  enabled: import_koishi9.Schema.boolean().default(true).description("\u662F\u5426\u542F\u7528")
...
var Config = import_koishi9.Schema.object({
  frontendMode: import_koishi9.Schema.union([
    import_koishi9.Schema.const("performance").description("\u6027\u80FD\u6A21\u5F0F"),
    import_koishi9.Schema.const("polished").description("\u7CBE\u81F4\u6A21\u5F0F")
  ]).role("radio").default("performance").description("Frontend display mode."),
  depsLayout: import_koishi9.Schema.union([
    import_koishi9.Schema.const("grid").description("\u7F51\u683C"),
    import_koishi9.Schema.const("list").description("\u5217\u8868")
  ]).role("radio").default("grid").description("Dependencies page layout."),
  idleProbe: import_koishi9.Schema.boolean().default(true).description("Run dependency and market metadata probes while Console is idle."),
  idleProbeDelay: import_koishi9.Schema.number().role("time").default(import_koishi9.Time.minute * 5).description("How long Console must stay idle before the background probe starts."),
  idleProbeBootDelay: import_koishi9.Schema.number().role("time").default(import_koishi9.Time.minute).description("Minimum delay after startup before idle probing is allowed."),
  idleProbeInterval: import_koishi9.Schema.number().role("time").default(import_koishi9.Time.hour * 6).description("Minimum interval between idle background probes."),
  bulkMode: import_koishi9.Schema.boolean().default(false).hidden().description("Batch operation mode for dependency changes."),
  removeConfig: import_koishi9.Schema.union([
    import_koishi9.Schema.const(void 0).description("Ask every time"),
    import_koishi9.Schema.const(true).description("Always remove plugin config"),
    import_koishi9.Schema.const(false).description("Never remove plugin config")
  ]).hidden().description("Whether to remove existing plugin config when uninstalling a plugin."),
  updateIgnoredPackages: import_koishi9.Schema.string().role("textarea").hidden().description("Dependency package names that should not be checked for updates. One package per line, or separated by commas."),
  updateIgnoreDuration: import_koishi9.Schema.number().role("time").default(0).hidden().description("Default duration for ignoring one update. 0 means no time-based expiry."),
  updateIgnoreVersions: import_koishi9.Schema.number().min(1).max(20).step(1).default(1).hidden().description("How many consecutive newer versions should be ignored after ignoring one update."),
  updateIgnorePrerelease: import_koishi9.Schema.boolean().default(false).hidden().description("Ignore alpha, beta, rc and other prerelease versions when checking updates."),
  collapsedGroups: import_koishi9.Schema.dict(Boolean).hidden(),
  registry: installer_default.Config,
  search: market_default.Config,
  chatlunaTool: import_koishi9.Schema.boolean().default(false).description("Enable ChatLuna plugin market query tool."),
  marketSilentFilters: import_koishi9.Schema.string().role("textarea").hidden().description("Legacy permanent silent filters."),
  marketSilentStatusRules: import_koishi9.Schema.array(import_koishi9.Schema.any()).hidden(),
  marketSilentDateRules: import_koishi9.Schema.array(import_koishi9.Schema.any()).hidden(),
  marketSilentRecentRules: import_koishi9.Schema.array(import_koishi9.Schema.any()).hidden(),
  marketSilentCustomRules: import_koishi9.Schema.array(import_koishi9.Schema.any()).hidden(),
...
async function ensurePluginConfig(ctx, name2, write = true) {
  if (!import_registry4.default.isPlugin(name2)) return false;
...
async function ensurePluginConfigs(ctx, names) {
  ...
  let changed = false;
  let checked = 0;
  for (const name2 of names.filter((name3) => import_registry4.default.isPlugin(name3))) {
...
async function ensureInstalledPluginConfigs(ctx) {
  ...
  const names = Object.keys(manifest.dependencies ?? {}).filter((name2) => import_registry4.default.isPlugin(name2)).filter((name2) => !isPluginBundleDependency(name2));
...
async function assertNoDirectBundleCycles(ctx, packageName, members) {
  ...
  for (const member of members) {
    try {
      ...
      const version = (0, import_semver6.maxSatisfying)(versions, member.version, { includePrerelease: true });
      if (!version) continue;
...
function setupIdleProbe(ctx, config) {
  ...
  const getDelay = () => Math.max(0, config.idleProbeDelay ?? import_koishi10.Time.minute * 5);
  const getBootDelay = () => Math.max(0, config.idleProbeBootDelay ?? import_koishi10.Time.minute);
  const getInterval = () => Math.max(0, config.idleProbeInterval ?? import_koishi10.Time.hour * 6);
  const runProbe = async () => {
    clearIdleTimer();
    if (!ctx.scope.isActive) return;
    if (getClientCount()) return;
    if (ctx.installer.isInstalling) {
      logger3.debug("skip idle background probe because dependency install is active");
      schedule(getDelay());
      return;
    }
    ...
    const retryWait = lastFailure ? Math.min(import_koishi10.Time.minute * 5, getInterval()) - (Date.now() - lastFailure) : 0;
    if (!lastProbe && retryWait > 0) {
      logger3.debug(`skip idle background probe because retry gate is active: remaining=${retryWait}ms`);
      schedule(retryWait);
      return;
    }
    const intervalWait = lastProbe ? getInterval() - (Date.now() - lastProbe) : 0;
    if (intervalWait > 0) {
      logger3.debug(`skip idle background probe because interval gate is active: remaining=${intervalWait}ms`);
    ...
    logger3.info(`idle background probe started: clients=0, delay=${getDelay()}ms, interval=${getInterval()}ms`);
    try {
      ...
      if (succeeded) {
        lastProbe = Date.now();
        lastFailure = 0;
        logger3.info(`idle background probe completed: elapsed=${Date.now() - probeStartedAt}ms`);
      } else {
        ...
        logger3.warn(`idle background probe failed: ${reason instanceof Error ? reason.message : reason}`);
      }
    } catch (error) {
      lastFailure = Date.now();
      logger3.warn(`idle background probe failed: ${error instanceof Error ? error.message : error}`);
    } finally {
      running = false;
      if (!getClientCount()) schedule(lastProbe ? getInterval() : Math.min(import_koishi10.Time.minute * 5, getInterval()));
    }
  };
  const schedule = (delay = getDelay()) => {
    ...
    logger3.debug(`idle background probe scheduled: delay=${Math.max(0, delay)}ms`);
  };
  ctx.on("console/connection", () => {
    if (getClientCount()) {
      clearIdleTimer();
      logger3.debug(`idle background probe cancelled: clients=${getClientCount()}`);
    } else {
      schedule();
    }
  });
...
function apply(ctx, config = {}) {
  ...
  ctx.inject(["installer"], (ctx2) => {
    ...
    ctx2.command("plugin.install <name>", { authority: 4 }).alias(".i").action(async ({ session }, name2) => {
      ...
      ctx2.loader.envData.message = {
        ...(0, import_koishi10.pick)(session, ["sid", "channelId", "guildId", "isDirect"]),
  ...
  ctx.inject(["console", "installer", "server"], (ctx2) => {
    ...
    ctx2.console.addEntry({
      dev: (0, import_path8.resolve)(__dirname, "../../client/index.ts"),
      prod: (0, import_path8.resolve)(__dirname, "../../dist")
    });
