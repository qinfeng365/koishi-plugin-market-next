...
var REGISTRY_FAST_ROUTE_THRESHOLD = import_koishi.Time.second * 1.5;
var levelMap = {
  "info": "info",
  "warning": "debug",
  "error": "warn"
...
var Installer = class extends import_koishi.Service {
  ...
  async probeMetadataEndpoint(name2, endpoints, serial) {
    ...
    logger.info(`npm registry route probe started: probe=${name2}, primary=${endpoints[0]}, fallbackCount=${Math.max(0, endpoints.length - 1)}, slowThreshold=${fallbackDelay}ms`);
    if (endpoints.length === 1 || this.config.autoRoute === false) {
      try {
        const result = await this.fetchRegistryEndpoint(name2, endpoints[0], serial);
    ...
    try {
      const result = await new Promise((resolve4, reject) => {
        ...
        const settle = (result2, index) => {
          if (settled) {
            logger.debug(`ignore slower npm registry endpoint: probe=${name2}, endpoint=${result2.endpoint}, elapsed=${result2.elapsed}ms`);
            return;
          }
          settled = true;
          clearTimeout(timer);
          ...
          if (result2.endpoint !== endpoints[0]) result2.fallbackReason = fallbackReason;
          this.recordRegistryRouteSuccess(result2);
          resolve4(result2);
        };
        ...
        const startEndpoint = (endpoint, index, waitIndex = 0) => {
          ...
          }).then((result2) => settle(result2, index)).catch((error) => fail(endpoint, index, error));
        };
        const startFallback = (reason) => {
          if (settled || fallbackStarted) return;
          fallbackStarted = true;
          fallbackReason = reason;
          logger.info(`npm registry fallback race started: probe=${name2}, reason=${reason}, count=${endpoints.length - 1}, stagger=${REGISTRY_ROUTE_STAGGER}ms`);
  ...
  async fetchRegistryByRoute(name2, endpoints, serial, onAttempt) {
    ...
    const fallbackDelay = this.getFallbackDelay(endpoints[0]);
    logger.debug(`fetch npm registry by route: package=${name2}, primary=${endpoints[0]}, fallbackCount=${endpoints.length - 1}, slowThreshold=${fallbackDelay}ms, endpoints=${endpoints.join(", ")}`);
    return new Promise((resolve4, reject) => {
      let settled = false;
      let failed = 0;
      let lastError;
      ...
      const settle = (result, index) => {
        if (settled) {
          logger.debug(`ignore slower npm registry endpoint: package=${name2}, endpoint=${result.endpoint}, elapsed=${result.elapsed}ms`);
          return;
        }
        settled = true;
        clearTimeout(timer);
        ...
        this.recordRegistryRouteSuccess(result);
        resolve4({ ...result, attempts, lastEndpoint });
      };
      const fail = (endpoint, index, error) => {
        if (settled) return;
        const detail = this.formatRegistryError(error);
        ...
        settled = true;
        clearTimeout(timer);
        logger.debug(`all routed npm registry endpoints failed: package=${name2}, count=${endpoints.length}, elapsed=${Date.now() - start}ms`);
        reject(lastError);
      };
      const startEndpoint = (endpoint, index, waitIndex = 0) => {
        const signal = controllers[index].signal;
      ...
      const startFallback = (reason) => {
        ...
        fallbackStarted = true;
        fallbackReason = reason;
        logger.debug(`npm registry fallback race started: package=${name2}, reason=${reason}, count=${endpoints.length - 1}, stagger=${REGISTRY_ROUTE_STAGGER}ms`);
        endpoints.slice(1).forEach((endpoint, fallbackIndex) => startEndpoint(endpoint, fallbackIndex + 1, fallbackIndex));
      };
      startEndpoint(endpoints[0], 0);
    });
