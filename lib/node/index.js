...
var Installer = class extends import_koishi.Service {
  constructor(ctx, config = {}) {
    ...
    this.statsFile = (0, import_path.resolve)(ctx.baseDir, "cache", "market-next-registry-stats.json");
    ctx.effect(() => () => {
  ...
  async loadRouteStats() {
    try {
      const content = await import_fs.promises.readFile(this.statsFile, "utf8");
  ...
  scheduleStatsWrite() {
    ...
    this.statsWriteTimer = setTimeout(async () => {
      ...
      try {
        await import_fs.promises.mkdir((0, import_path.resolve)(this.statsFile, ".."), { recursive: true });
        await import_fs.promises.writeFile(this.statsFile, JSON.stringify({ version: 1, stats, savedAt: Date.now() }));
      } catch (error) {
  ...
  waitRouteTurn(index, signal) {
    ...
    return new Promise((resolve4, reject) => {
      ...
      const timer = setTimeout(resolve4, index * REGISTRY_ROUTE_STAGGER);
  ...
  getRegistryRouteScore(endpoint) {
    ...
    if (stats.lastSuccess && Date.now() - stats.lastSuccess <= import_koishi.Time.minute * 10) score += 0.5;
    return score;
  }
  ...
  async snapshotPackageManifest() {
    const filename = (0, import_path.resolve)(this.cwd, "package.json");
    const content = await import_fs.promises.readFile(filename, "utf8");
  ...
  async restorePackageManifest(snapshot, deps, reason) {
    ...
    let manifest;
    try {
      manifest = JSON.parse(await import_fs.promises.readFile(filename, "utf8"));
    } catch {
    ...
    await import_fs.promises.writeFile(filename, JSON.stringify(manifest, null, 2) + "\n");
  ...
  async install(deps, forced, beforeReload) {
    ...
    let release;
    this.installTask = new Promise((resolve4) => {
      release = resolve4;
    });
...
var FAST_ROUTE_THRESHOLD = import_koishi3.Time.second * 0.8;
...
var MarketProvider2 = class extends MarketProvider {
  ...
  recordRouteSuccess(result) {
    ...
    stats.score = clamp2(stats.score + (result.elapsed <= FAST_ROUTE_THRESHOLD ? 0.5 : -0.5), -6, 3);
    stats.lastSuccess = Date.now();
  ...
  recordRouteFailure(endpoint) {
    ...
    stats.score = clamp2(stats.score - 2, -10, 3);
  ...
  pruneCacheEntries(lastUsed) {
    const entries = Object.values(this.cacheEntries).filter((cache2) => !!cache2 && Array.isArray(cache2.result?.objects) && Date.now() - cache2.fetchedAt <= CACHE_ENTRY_TTL).sort((a, b) => {
...
var Config = import_koishi5.Schema.object({
  ...
  updateIgnored: import_koishi5.Schema.dict(import_koishi5.Schema.any()).hidden(),
  collapsedGroups: import_koishi5.Schema.dict(Boolean).hidden(),
  bundleRecords: import_koishi5.Schema.dict(import_koishi5.Schema.any()).hidden(),
...
async function installBundle(ctx, request, forced) {
  const start = Date.now();
...
function setupIdleProbe(ctx, config) {
  ...
  const runProbe = async () => {
    ...
    try {
      const market = ctx.get("console.services.market");
      await Promise.all([
        ctx.installer.probeDependenciesInBackground("idle"),
        market?.probeInBackground?.("idle probe") ?? Promise.resolve(false)
      ]);
      logger3.info(`idle background probe completed: elapsed=${Date.now() - lastProbe}ms`);
    } catch (error) {
...
function apply(ctx, config = {}) {
  ...
  ctx.inject(["console", "installer"], (ctx2) => {
    ...
    ctx2.console.addListener("market/install-bundle", async (request, forced) => {
      return installBundle(ctx2, request, forced);
    }, { authority: 4 });
    ...
    ctx2.on("ready", () => {
      ...
      ctx2.effect(() => () => clearTimeout(timer));
    });
  });
}
function sleep2(ms) {
