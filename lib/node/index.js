...
var AVATAR_CACHE_TTL = import_koishi13.Time.day * 7;
var AVATAR_CACHE_SWEEP_INTERVAL = import_koishi13.Time.hour;
...
var MarketSilentRuleType = import_koishi14.Schema.union([
  import_koishi14.Schema.const("preview").description("\u72B6\u6001\uFF1A\u9884\u89C8\u7248\u63D2\u4EF6"),
  import_koishi14.Schema.const("insecure").description("\u72B6\u6001\uFF1A\u4E0D\u5B89\u5168\u63D2\u4EF6"),
  import_koishi14.Schema.const("bundle").description("\u72B6\u6001\uFF1A\u63D2\u4EF6\u5305"),
  import_koishi14.Schema.const("created-before").description("\u521B\u5EFA\u65F6\u95F4\uFF1A\u65E9\u4E8E\u6307\u5B9A\u65E5\u671F"),
  import_koishi14.Schema.const("created-after").description("\u521B\u5EFA\u65F6\u95F4\uFF1A\u665A\u4E8E\u6307\u5B9A\u65E5\u671F"),
  import_koishi14.Schema.const("updated-before").description("\u66F4\u65B0\u65F6\u95F4\uFF1A\u65E9\u4E8E\u6307\u5B9A\u65E5\u671F"),
  import_koishi14.Schema.const("updated-after").description("\u66F4\u65B0\u65F6\u95F4\uFF1A\u665A\u4E8E\u6307\u5B9A\u65E5\u671F"),
  import_koishi14.Schema.const("created-within").description("\u521B\u5EFA\u65F6\u95F4\uFF1A\u6700\u8FD1 N \u5929\u5185"),
  import_koishi14.Schema.const("updated-within").description("\u66F4\u65B0\u65F6\u95F4\uFF1A\u6700\u8FD1 N \u5929\u5185"),
  import_koishi14.Schema.const("custom").description("\u81EA\u5B9A\u4E49\u9AD8\u7EA7\u6761\u4EF6")
]);
var MarketSilentRules = import_koishi14.Schema.array(import_koishi14.Schema.object({
  type: MarketSilentRuleType.default("preview").description("\u89C4\u5219\u7C7B\u578B"),
  value: import_koishi14.Schema.string().default("").description("\u89C4\u5219\u503C\u3002\u72B6\u6001\u7C7B\u7559\u7A7A\uFF1B\u65E5\u671F\u7C7B\u586B\u5199 YYYY-MM-DD\uFF0C\u4F8B\u5982 2024-01-01\uFF1B\u6700\u8FD1 N \u5929\u586B\u5199\u6570\u5B57\uFF0C\u4F8B\u5982 30\uFF1B\u81EA\u5B9A\u4E49\u89C4\u5219\u586B\u5199\u641C\u7D22\u6761\u4EF6\uFF0C\u4F8B\u5982 category:adapter\u3002"),
  note: import_koishi14.Schema.string().default("").description("\u5907\u6CE8"),
  enabled: import_koishi14.Schema.boolean().default(true).description("\u662F\u5426\u542F\u7528")
...
var Config = import_koishi14.Schema.object({
  frontendMode: import_koishi14.Schema.union([
    import_koishi14.Schema.const("performance").description("\u6027\u80FD\u6A21\u5F0F"),
    import_koishi14.Schema.const("polished").description("\u7CBE\u81F4\u6A21\u5F0F")
  ]).role("radio").default("performance").description("Frontend display mode."),
  depsLayout: import_koishi14.Schema.union([
    import_koishi14.Schema.const("grid").description("\u7F51\u683C"),
    import_koishi14.Schema.const("list").description("\u5217\u8868")
  ]).role("radio").default("grid").description("Dependencies page layout."),
  idleProbe: import_koishi14.Schema.boolean().default(true).description("Run dependency and market metadata probes while Console is idle."),
  idleProbeDelay: import_koishi14.Schema.number().role("time").default(import_koishi14.Time.minute * 5).description("How long Console must stay idle before the background probe starts."),
  idleProbeBootDelay: import_koishi14.Schema.number().role("time").default(import_koishi14.Time.minute).description("Minimum delay after startup before idle probing is allowed."),
  idleProbeInterval: import_koishi14.Schema.number().role("time").default(import_koishi14.Time.hour * 6).description("Minimum interval between idle background probes."),
  bulkMode: import_koishi14.Schema.boolean().default(false).hidden().description("Batch operation mode for dependency changes."),
  removeConfig: import_koishi14.Schema.union([
    import_koishi14.Schema.const(void 0).description("Ask every time"),
    import_koishi14.Schema.const(true).description("Always remove plugin config"),
    import_koishi14.Schema.const(false).description("Never remove plugin config")
  ]).hidden().description("Whether to remove existing plugin config when uninstalling a plugin."),
  updateIgnoredPackages: import_koishi14.Schema.string().role("textarea").hidden().description("Dependency package names that should not be checked for updates. One package per line, or separated by commas."),
  updateIgnoreDuration: import_koishi14.Schema.number().role("time").default(0).hidden().description("Default duration for ignoring one update. 0 means no time-based expiry."),
  updateIgnoreVersions: import_koishi14.Schema.number().min(1).max(20).step(1).default(1).hidden().description("How many consecutive newer versions should be ignored after ignoring one update."),
  updateIgnorePrerelease: import_koishi14.Schema.boolean().default(false).hidden().description("Ignore alpha, beta, rc and other prerelease versions when checking updates."),
  collapsedGroups: import_koishi14.Schema.dict(Boolean).hidden(),
  registry: installer_default.Config,
  search: market_default.Config,
  chatlunaTool: import_koishi14.Schema.boolean().default(false).description("Enable ChatLuna plugin market query tool."),
  marketSilentFilters: import_koishi14.Schema.string().role("textarea").hidden().description("Legacy permanent silent filters."),
  marketSilentStatusRules: import_koishi14.Schema.array(import_koishi14.Schema.any()).hidden(),
  marketSilentDateRules: import_koishi14.Schema.array(import_koishi14.Schema.any()).hidden(),
  marketSilentRecentRules: import_koishi14.Schema.array(import_koishi14.Schema.any()).hidden(),
  marketSilentCustomRules: import_koishi14.Schema.array(import_koishi14.Schema.any()).hidden(),
  ...
  ctx.inject(["installer"], (ctx2) => {
    ...
    ctx2.command("plugin.install <name>", { authority: 4 }).alias(".i").action(async ({ session }, name2) => {
      ...
      ctx2.loader.envData.message = {
        ...(0, import_koishi15.pick)(session, ["sid", "channelId", "guildId", "isDirect"]),
