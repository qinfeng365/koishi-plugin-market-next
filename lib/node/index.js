...
var FIRST_PAYLOAD_TIMEOUT = import_koishi.Time.second * 1.5;
var FAST_ROUTE_THRESHOLD = import_koishi.Time.second * 0.5;
var MARKET_GENERATION_TOLERANCE = import_koishi.Time.minute * 2;
var MAX_CACHE_ENTRIES = 3;
var CACHE_ENTRY_TTL = import_koishi.Time.day * 30;
var logLevels = ["silent", "error", "warn", "info", "debug"];
function isValidMarketIndex(result) {
  return !!result && Array.isArray(result.objects) && result.objects.every((object) => {
    return !!object && typeof object === "object" && !!object.package && typeof object.package === "object" && typeof object.package.name === "string" && !!object.package.name.trim();
  });
}
...
function formatAge(age) {
  ...
  if (age < import_koishi.Time.second) return `${Math.max(0, Math.round(age))}ms`;
  if (age < import_koishi.Time.minute) return `${Math.round(age / import_koishi.Time.second)}s`;
  if (age < import_koishi.Time.hour) return `${Math.round(age / import_koishi.Time.minute)}m`;
  if (age < import_koishi.Time.day) return `${Math.round(age / import_koishi.Time.hour)}h`;
  return `${Math.round(age / import_koishi.Time.day)}d`;
}
...
function getRouteCooldown(failures = 0) {
  ...
  if (failures === 1) return import_koishi.Time.minute;
  if (failures === 2) return import_koishi.Time.minute * 5;
  if (failures === 3) return import_koishi.Time.minute * 30;
  if (failures === 4) return import_koishi.Time.hour * 4;
  return import_koishi.Time.hour * 12;
}
...
function shouldLog(level, configuredLevel = currentLogLevel) {
  if (configuredLevel === "silent") return false;
  return logLevels.indexOf(configuredLevel) >= logLevels.indexOf(level);
}
var MarketLogger = class {
  constructor(name2 = "market", getContext) {
    this.name = name2;
    this.getContext = getContext;
    this.baseLogger = new import_koishi2.Logger(name2);
  }
  name;
  getContext;
  baseLogger;
  ...
  log(level, message, ...args) {
    ...
    const logger3 = this.ctxLogger;
    if (level === "debug") {
      logger3.info(`[debug] ${message}`, ...args);
    } else {
      logger3[level](message, ...args);
    }
  }
  debug(message, ...args) {
...
var MarketDataStore = class extends import_console2.DataService {
  ...
  async write() {
    const tempFile = `${this.file}.${process.pid}.${(0, import_crypto.randomUUID)()}.tmp`;
    try {
      ...
      await import_fs.promises.rm(tempFile, { force: true }).catch(() => {
      });
...
var import_which_pm_runs2 = __toESM(require("which-pm-runs"));
...
var FULL_RELOAD_DELAY = import_koishi4.Time.second;
...
function loadManifest(name2, baseDir) {
  ...
  meta.dependencies ||= {};
  (0, import_koishi4.defineProperty)(meta, "$workspace", !filename.includes("node_modules"));
  return meta;
}
...
function getVersions(versions) {
  return Object.fromEntries(versions.map((item) => [item.version, (0, import_koishi4.pick)(item, ["peerDependencies", "peerDependenciesMeta", "deprecated"])]).sort(([a], [b]) => (0, import_semver3.compare)(b, a)));
}
function sleep(ms) {
...
async function prepareLocalBindingPackage(baseDir, name2, dependency, dependencies, timeout) {
  ...
  try {
    const child = await (0, import_execa.default)("npm", [
      ...
      cwd: baseDir,
      timeout: Math.max(import_koishi5.Time.minute, timeout ?? 0)
    });
    ...
    const hash = (0, import_crypto2.createHash)("sha256").update(content).digest("hex");
    ...
    const validateExistingTarget = async () => {
      ...
      const targetHash = (0, import_crypto2.createHash)("sha256").update(await import_fs3.promises.readFile(target)).digest("hex");
      if (targetHash !== hash) {
    ...
    logger.info(`local plugin source prepared: ${name2}@${dependency.resolved}, file=${filename}, size=${pack.size}`);
    return {
...
var LocalPackageUploadStore = class {
  ...
  async start(request) {
    ...
    if (!Number.isSafeInteger(size) || size <= 0 || size > MAX_LOCAL_BINDING_PACK_SIZE) {
      throw new Error(`\u672C\u5730\u63D2\u4EF6\u5F52\u6863\u5927\u5C0F\u5FC5\u987B\u5728 1 B \u5230 ${formatBytes2(MAX_LOCAL_BINDING_PACK_SIZE)} \u4E4B\u95F4\u3002`);
    }
    await import_fs4.promises.mkdir(this.temporaryRoot, { recursive: true });
    const uploadId = (0, import_crypto3.randomUUID)();
    ...
    this.sessions.set(uploadId, {
      ...
      touchedAt: Date.now(),
      handle,
      hash: (0, import_crypto3.createHash)("sha256"),
  ...
  async finish(request) {
    ...
    return this.runSession(session, async () => {
      ...
      if (session.received !== session.size) {
        throw new Error(`\u672C\u5730\u63D2\u4EF6\u5F52\u6863\u5C1A\u672A\u4E0A\u4F20\u5B8C\u6210\uFF08${formatBytes2(session.received)} / ${formatBytes2(session.size)}\uFF09\u3002`);
      }
...
async function readFileHash(path) {
  try {
    ...
    return (0, import_crypto3.createHash)("sha256").update(await import_fs4.promises.readFile(path)).digest("hex");
  } catch (error) {
...
function getEnvironmentSnapshotId(dependencies) {
  return `env-${(0, import_crypto4.createHash)("sha256").update(canonicalDependencies(dependencies)).digest("hex").slice(0, 20)}`;
}
...
var REGISTRY_FAST_ROUTE_THRESHOLD = import_koishi6.Time.second * 0.8;
var REGISTRY_STATS_TTL = import_koishi6.Time.day * 30;
var REGISTRY_DETECTION_TIMEOUT = import_koishi6.Time.second * 3;
var DEFAULT_REGISTRY_ENDPOINT = "https://registry.npmjs.org";
function isRegistryEndpoint(value) {
...
async function resolveRegistryEndpoint(configured, cwd, readRegistry = readPackageManagerRegistry) {
  ...
  const environment = [process.env.npm_config_registry, process.env.NPM_CONFIG_REGISTRY].find(isRegistryEndpoint);
...
async function readPackageManagerRegistry(options, run = (name2, args, commandOptions) => (0, import_execa2.default)(name2, args, {
  ...commandOptions,
  reject: false
})) {
  ...
  let name2 = agent?.name || "npm";
  if (name2 === "deno") name2 = "npm";
  const result = await run(name2, ["config", "get", key], {
    cwd: options.cwd,
...
var RegistryRouter = class {
  ...
  async loadRouteStats() {
    try {
      ...
      for (const [endpoint, stats] of Object.entries(store.stats)) {
        ...
        const hasRecentSuccess = Number(stats.lastSuccess) && Date.now() - Number(stats.lastSuccess) < import_koishi6.Time.day;
      ...
      if (error?.code !== "ENOENT") logger.debug(`failed to load registry route stats: ${error instanceof Error ? error.message : error}`);
    }
  }
  scheduleStatsWrite() {
    clearTimeout(this.statsWriteTimer);
  ...
  raceEndpoints(name2, endpoints, serial, onAttempt) {
    ...
    return new Promise((resolve11, reject) => {
      ...
      const startFallback = (reason) => {
        ...
        logger.info(`npm registry fallback race started: probe=${name2}, reason=${reason}, count=${endpoints.length - 1}`);
  ...
  async probeMetadataEndpoint(name2, endpoints, serial) {
    ...
    logger.info(`npm registry route probe started: probe=${name2}, primary=${endpoints[0]}, fallbackCount=${Math.max(0, endpoints.length - 1)}, slowThreshold=${this.getFallbackDelay(endpoints[0])}ms`);
    try {
  ...
  async fetchRegistryEndpoint(name2, endpoint, serial, signal) {
    ...
    try {
      logger.debug(`fetch npm registry endpoint: package=${name2}, endpoint=${endpoint}`);
      ...
      if (!registry?.versions || typeof registry.versions !== "object" || Array.isArray(registry.versions) || !Object.keys(registry.versions).length || Object.entries(registry.versions).some(([version, entry]) => !(0, import_semver6.valid)(version) || !entry || typeof entry !== "object" || entry.version !== version)) {
        throw new Error(`invalid registry metadata for ${name2}`);
      }
      const elapsed = Date.now() - attemptStart;
      logger.debug(`fetch npm registry endpoint succeeded: package=${name2}, endpoint=${endpoint}, elapsed=${elapsed}ms, versions=${Object.keys(registry.versions).length}`);
      return { endpoint, registry, elapsed };
    } catch (error) {
      const detail = this.formatError(error);
      logger.debug(`fetch npm registry endpoint failed: package=${name2}, endpoint=${endpoint}, elapsed=${Date.now() - attemptStart}ms, reason=${detail.reason}, error=${detail.error}`);
      throw error;
    }
  }
  applyRouteProbeResult(name2, result, serial, start) {
    ...
    if (result.endpoint === this.endpointValue) {
      logger.info(`npm registry primary selected: probe=${name2}, endpoint=${result.endpoint}, elapsed=${result.elapsed}ms, total=${Date.now() - start}ms`);
    } else {
      logger.info(`npm registry fallback selected: probe=${name2}, endpoint=${result.endpoint}, previous=${previous}, reason=${result.fallbackReason ?? "unknown"}, elapsed=${result.elapsed}ms, total=${Date.now() - start}ms`);
    }
    logger.debug(`npm registry route scores after probe: ${formatRouteScores2(this.getRouteScores())}`);
  }
  ...
  recordRouteSuccess(result) {
    ...
    stats.score = clamp2(stats.score + (result.elapsed <= REGISTRY_FAST_ROUTE_THRESHOLD ? 0.4 : 0.1), -6, 3);
  ...
  recordRouteFailure(endpoint, reason) {
    ...
    stats.score = clamp2(stats.score - Math.min(1.5, getFailurePenalty(stats.lastFailureReason)), -8, 3);
    this.scheduleStatsWrite();
  }
  getFallbackDelay(endpoint) {
    const stats = this.routeStats[endpoint];
    if (!stats) return REGISTRY_FAST_ROUTE_THRESHOLD;
    const recentSuccess = stats.lastSuccess && Date.now() - stats.lastSuccess <= import_koishi6.Time.minute * 10;
...
// src/node/registry-metadata.ts
var NOT_FOUND_CACHE_TTL = import_koishi7.Time.minute * 5;
var RegistryMetadata = class {
  ...
  async getRegistry(name2, serial = this.serial) {
    ...
    for (let retry = 0; retry <= maxRetry; retry++) {
      ...
      logger.debug(`registry metadata candidates for ${name2}: endpoints=${endpoints.join(", ")}, retry=${retry + 1}/${maxRetry + 1}, concurrency=${this.config.concurrency ?? 4}`);
      try {
        ...
        if (result.endpoint !== this.selectedEndpoint) {
          logger.debug(`routed npm registry endpoint for ${name2}: ${result.endpoint}`);
          logger.info(`npm registry route selected for ${name2}: endpoint=${result.endpoint}, previous=${this.selectedEndpoint}, reason=${result.fallbackReason ?? "same-priority"}, elapsed=${result.elapsed}ms`);
        ...
        logger.debug(`loaded registry metadata for ${name2} from ${result.endpoint} in ${result.elapsed}ms, attempts=${attempts}, versions=${Object.keys(result.registry.versions).length}`);
        return result.registry;
      } catch (error) {
        lastError = error;
        const detail2 = this.formatError(error);
        failureReasons.push(...getRegistryAttemptReasons(error, detail2.reason));
        logger.debug(`failed routed registry metadata for ${name2}, attempt=${retry + 1}/${maxRetry + 1}, endpoint=${lastEndpoint}, attempts=${attempts}: ${detail2.error}`);
  ...
  setPackage(name2, versions) {
    delete this.notFoundCache[name2];
  ...
  async fetchPackage(name2, serial = this.serial) {
    try {
      ...
      this.fullCache[name2] = this.tempCache[name2] = getVersions(Object.values(registry.versions).filter((remote) => {
        if (name2 === "koishi") return (0, import_semver7.satisfies)(remote.version, "4");
...
var DEFAULT_INSTALL_LOG_RETENTION = import_koishi8.Time.day * 3;
...
var InstallHistoryStore = class {
  ...
  async getHistory(limit = 20) {
    await this.cleanup();
    const count = clamp3(Math.floor(Number(limit) || 20), 1, 50);
    const dir = this.getDirectory();
    let files = [];
    try {
      const entries = await import_fs7.promises.readdir(dir, { withFileTypes: true });
  ...
  async getPackageHistory(name2, limit = 24) {
    const normalizedName = typeof name2 === "string" ? name2.trim() : "";
    if (!normalizedName) return { name: "", entries: [] };
    const entries = (await this.getHistory(50)).filter((entry) => entry.status === "success").flatMap((entry) => {
      const change = entry.changes.find((change2) => change2.name === normalizedName);
      if (!change) return [];
    ...
    const count = clamp3(Math.floor(Number(limit) || 24), 1, 50);
    return {
      name: normalizedName,
  ...
  getRetention() {
    ...
    if (Number.isFinite(hours) && hours > 0) return Math.max(1, hours) * import_koishi8.Time.hour;
    const legacyRetention = Number(this.config.installLogRetention);
    return Number.isFinite(legacyRetention) && legacyRetention > 0 ? Math.max(import_koishi8.Time.hour, legacyRetention) : DEFAULT_INSTALL_LOG_RETENTION;
  }
  async writeMetadata() {
...
function applyDependencyOverrides(manifest, changes) {
  const dependencies = { ...manifest.dependencies ?? {} };
  for (const [name2, request] of Object.entries(changes)) {
    if (request) dependencies[name2] = request;
    else delete dependencies[name2];
  }
  return {
    ...manifest,
...
async function writePackageManifest(filename, manifest, io = import_fs8.promises) {
  const tempFile = `${filename}.${process.pid}.${(0, import_crypto5.randomUUID)()}.tmp`;
  try {
    await io.writeFile(tempFile, JSON.stringify(manifest, null, 2) + "\n");
    await io.rename(tempFile, filename);
  } finally {
    await io.rm(tempFile, { force: true }).catch(() => void 0);
  }
}
function requiresPackageManager(input) {
  ...
  for (const name2 in changes) {
    ...
    if (local || resolved && (0, import_semver8.satisfies)(resolved, nextRequest, { includePrerelease: true })) continue;
    return true;
  }
  return false;
}
...
var PackageManagerRunner = class {
  constructor(cwd, agent, emit, spawnProcess = import_execa3.default) {
    this.cwd = cwd;
    this.agent = agent;
    this.emit = emit;
  ...
  async exec(args) {
    ...
    const version = this.agent?.version ? (0, import_semver9.coerce)(this.agent.version) : null;
    const useJson = name2 === "yarn" && !!version && (0, import_semver9.gte)(version, "2.0.0");
    const finalArgs = [...args];
    if (name2 !== "yarn") finalArgs.unshift("install");
    const start = Date.now();
    logger.info(`run package manager: agent=${name2}${this.agent?.version ? "@" + this.agent.version : ""}, args=${finalArgs.join(" ") || "(none)"}, cwd=${this.cwd}, json=${useJson}`);
...
// src/node/installer.ts
var Installer = class extends import_koishi9.Service {
  constructor(ctx, config = {}) {
    super(ctx, "installer");
    this.ctx = ctx;
    this.config = config;
    ...
    this.packageManager = new PackageManagerRunner(this.cwd, (0, import_which_pm_runs2.default)(), (type, line) => {
      this.emitInstallLog(type, line);
    });
    ctx.setInterval(() => {
      void this.localPackageUploads.pruneExpired();
    }, import_koishi9.Time.minute * 5);
    ctx.effect(() => () => {
  ...
  markRegistryNotFoundDependency(name2, dependency = this.depCache[name2]) {
    ...
    logger.info(`dependency classified as unbound local plugin: ${name2}@${dependency.resolved}`);
    return true;
  }
  getLocalDepsSnapshot() {
    const start = Date.now();
    const result = (0, import_koishi9.valueMap)(this.manifest.dependencies, (request) => {
    ...
    for (const name2 of names) {
      ...
      if (!result[name2].local && !(0, import_semver10.valid)(result[name2].request)) {
    ...
    await (0, import_p_map.default)(targets, async (name2) => {
      ...
      try {
        ...
        if (versions) {
          ...
          logger.debug(`dependency npm not-found result reused from cache: ${name2}`);
        } else {
        ...
        if (detail.reason === "not-found" && this.markRegistryNotFoundDependency(name2, result[name2])) {
          ...
          logger.debug(`dependency metadata refresh skipped after error: ${name2}, reason=${detail.reason}, error=${detail.error}`);
        }
      }
  ...
  getPackageHistory(name2, limit = 24) {
    return this.installHistory.getPackageHistory(name2, limit);
  }
  ...
  async snapshotPackageManifest() {
    const filename = (0, import_path8.resolve)(this.cwd, "package.json");
    const content = await import_fs9.promises.readFile(filename, "utf8");
  ...
  async restorePackageManifest(snapshot, deps, reason) {
    ...
    let manifest;
    try {
      manifest = JSON.parse(await import_fs9.promises.readFile(filename, "utf8"));
    } catch {
  ...
  _getLocalDeps(override) {
    return (0, import_koishi9.valueMap)(override, (request, name2) => {
      const dep = { request };
      try {
        const meta = loadManifest(name2, this.cwd);
  ...
  async captureCurrentEnvironmentSnapshot(source, operationId) {
    ...
    for (const [name2, request] of Object.entries(manifest.dependencies)) {
      ...
      dependencies[name2] = {
        ...
        invalid: !local[name2]?.local && !(0, import_semver10.valid)(normalizedRequest)
      };
    }
  ...
  async _installLocked(deps, forced, beforeReload, options = {}) {
    ...
    try {
      ...
      for (const name2 in localDeps) {
        ...
        shouldReload = true;
        logger.debug(`dependency changed may require full reload: ${name2}, previous=${localDeps[name2]?.resolved ?? "-"}, current=${newDeps[name2]?.resolved ?? "-"}`);
      }
      if (beforeReload) {
        ...
        try {
          ...
          await this.recordCurrentEnvironmentSnapshot("operation", this.installHistory.currentId).catch((snapshotError2) => {
            logger.warn(`failed to record dependency environment after hook error: ${snapshotError2}`);
          });
  ...
  async withInstallLock(description2, callback) {
    ...
    if (this.installActive) logger.info(`dependency install queued: ${description2}`);
    await previous;
...
((Installer2) => {
  Installer2.Config = import_koishi9.Schema.object({
    endpoint: import_koishi9.Schema.string().role("link"),
    timeout: import_koishi9.Schema.number().role("time").default(import_koishi9.Time.second * 5),
    autoRoute: import_koishi9.Schema.boolean().default(true),
    retry: import_koishi9.Schema.number().min(0).max(5).step(1).default(1),
    concurrency: import_koishi9.Schema.number().min(1).max(16).step(1).default(4),
    installLogRetentionHours: import_koishi9.Schema.number().min(1).max(24 * 365).step(1).default(72)
  });
})(Installer || (Installer = {}));
...
var MarketDiskCache = class {
  ...
  async load(serial) {
    ...
    try {
      ...
      const content = await import_fs10.promises.readFile(this.file, "utf8");
  ...
  async loadEntry(entry) {
    if (isValidMarketIndex(entry.result)) return entry;
    if (!entry.file) return;
    try {
      const content = await import_fs10.promises.readFile((0, import_path9.resolve)(this.directory, entry.file), "utf8");
  ...
  scheduleWrite(result, meta = this.conditionMeta) {
    ...
    this.options.log("debug", `schedule market disk cache write: endpoint=${entry.endpoint}, objects=${result.objects.length}, entries=${Object.keys(cache2.entries).length}, file=${this.file}, hash=${shortHash(entry.hash) ?? "-"}, size=${formatBytes(entry.size)}, wireSize=${formatBytes(entry.wireSize)}, encoding=${entry.contentEncoding ?? "identity"}`);
    this.writeTimer = setTimeout(() => {
      this.writeTimer = void 0;
  ...
  getEntryFilename(endpoint) {
    return `${(0, import_crypto6.createHash)("sha1").update(endpoint).digest("hex").slice(0, 16)}.json`;
  }
  ...
  async writeEntryFile(entry) {
    await import_fs10.promises.mkdir(this.directory, { recursive: true });
    const file = (0, import_path9.resolve)(this.directory, this.getEntryFilename(entry.endpoint));
    const tempFile = `${file}.${process.pid}.${Date.now()}.tmp`;
    await import_fs10.promises.writeFile(tempFile, JSON.stringify(entry.result));
  ...
  async pruneSplitFiles(entries) {
    try {
      ...
      const files = await import_fs10.promises.readdir(this.directory).catch(() => []);
      await Promise.all(files.filter((file) => file.endsWith(".json") && !keep.has(file)).map((file) => import_fs10.promises.unlink((0, import_path9.resolve)(this.directory, file)).catch(() => {
      })));
    } catch (error) {
  ...
  async writeStore(cache2) {
    if (!this.options.isActive()) return;
    try {
      await import_fs10.promises.mkdir((0, import_path9.dirname)(this.file), { recursive: true });
      const entries = {};
      ...
      await import_fs10.promises.writeFile(tempFile, nextStore);
      await import_fs10.promises.rename(tempFile, this.file);
...
var MarketEndpointFetcher = class {
  ...
  decodeResponse(endpoint, response, cached) {
    ...
    const hash = (0, import_crypto7.createHash)("sha256").update(text).digest("hex");
  ...
  ctx.inject(["chatluna"], (ctx2) => {
    ...
    ctx2.effect(() => {
      ...
      try {
        ...
        logger.warn(`Failed to register ChatLuna market search tool: ${formatError3(error)}`);
        return () => {
        };
      }
    });
...
function normalizeAvatarCacheKey(key) {
  return key.replace(/[^0-9A-Za-z:@._-]/g, "-").slice(0, 128) || `url:${(0, import_crypto8.createHash)("sha1").update(key).digest("hex")}`;
}
function getAvatarCacheFile(ctx, key) {
  return (0, import_path10.resolve)(getAvatarCacheDir(ctx), `${(0, import_crypto8.createHash)("sha1").update(normalizeAvatarCacheKey(key)).digest("hex")}.json`);
}
...
async function readAvatarDiskCache(ctx, key) {
  try {
    ...
    const entry = normalizeAvatarDiskCache(JSON.parse(await import_fs11.promises.readFile(file, "utf8")), key);
    if (!entry) {
      void import_fs11.promises.unlink(file).catch(() => {
      });
      return;
    }
...
async function writeAvatarDiskCache(ctx, key, url, result) {
  try {
    await import_fs11.promises.mkdir(getAvatarCacheDir(ctx), { recursive: true });
    const file = getAvatarCacheFile(ctx, key);
    const tempFile = `${file}.${process.pid}.${Date.now()}.tmp`;
    const entry = {
      key,
    ...
    await import_fs11.promises.writeFile(tempFile, JSON.stringify(entry));
    await import_fs11.promises.rename(tempFile, file);
  } catch (error) {
...
async function cleanupAvatarDiskCache(ctx) {
  ...
  avatarDiskCleanupTask = (async () => {
    try {
      ...
      const files = await import_fs11.promises.readdir(dir).catch(() => []);
      const entries = await Promise.all(files.filter((file) => file.endsWith(".json")).map(async (file) => {
        ...
        try {
          const stat = await import_fs11.promises.stat(path);
          let cachedAt = stat.mtimeMs;
          try {
            const value = JSON.parse(await import_fs11.promises.readFile(path, "utf8"));
            cachedAt = Number(value.cachedAt) || cachedAt;
            if (!value?.key || !value?.url || typeof value?.data !== "string" || typeof value?.type !== "string") {
              await import_fs11.promises.unlink(path).catch(() => {
              });
              return;
            }
      ...
      await Promise.all(alive.slice(AVATAR_MAX_ENTRIES).map((entry) => import_fs11.promises.unlink(entry.path).catch(() => {
      })));
    } finally {
...
async function clearAvatarCacheStorage(ctx) {
  ...
  const dir = getAvatarCacheDir(ctx);
  const files = await import_fs11.promises.readdir(dir).catch((error) => {
    if (error?.code === "ENOENT") return [];
    throw error;
  });
  const disk = files.filter((file) => file.endsWith(".json")).length;
  await import_fs11.promises.rm(dir, { recursive: true, force: true });
  return { memory, disk };
}
...
async function ensurePluginConfig(ctx, name2, write = true) {
  ...
  if (isPluginBundleDependency(name2)) {
    logger.debug(`skip default config entry for plugin bundle: ${name2}`);
    return false;
  }
  const shortname = getPluginShortname(name2);
  if (hasPluginConfig(ctx.loader.config?.plugins, shortname)) return false;
  await requestPluginRuntime(ctx, name2).catch((error) => logger.warn(error));
  if (hasPluginConfig(ctx.loader.config?.plugins, shortname)) return false;
  const key = createDisabledPluginConfig(ctx, shortname);
  if (!key) return false;
  if (write) await ctx.loader.writeConfig();
  logger.info("created disabled default config entry %c for %c", key, name2);
  return true;
}
...
var Config = import_koishi20.Schema.object({
  ...
  logLevel: import_koishi20.Schema.union(logLevels.map((level) => import_koishi20.Schema.const(level))).default("warn").description("Log level for market and package operations. silent disables logs; debug enables detailed logs."),
...
function findMarketNextConfigNode(plugins, currentConfig) {
  let best;
  const visit = (nodes, parentDisabled = false) => {
    for (const key of Object.keys(nodes || {})) {
      if (key.startsWith("$")) continue;
      const value = nodes[key];
      if (!value || typeof value !== "object") continue;
      const disabled = parentDisabled || key.startsWith("~");
      const [name2] = (key.startsWith("~") ? key.slice(1) : key).split(":", 1);
      const named = name2 === "market-next" || name2 === "koishi-plugin-market-next";
      ...
      if (name2 === "group") visit(value, disabled);
    }
  };
  visit(plugins);
  return best;
...
var import_semver11 = require("semver");
async function assertNoDirectBundleCycles(ctx, packageName, members) {
  ...
  for (const member of members) {
    try {
      ...
      const version = (0, import_semver11.maxSatisfying)(versions, member.version, { includePrerelease: true });
      if (!version) continue;
...
var MarketSnapshotTransport = class {
  ...
  async prepare(data) {
    ...
    const id = (0, import_crypto9.createHash)("sha256").update(json).digest("hex");
...
function setupConsoleListeners(ctx, config, dataStore, marketSnapshotTransport) {
  ...
  const addListener = (event, callback, options) => {
  ...
  addListener("market/package-history", async (name2, limit) => {
    return ctx.installer.getPackageHistory(name2, limit);
  }, { authority: 4 });
  ...
  addListener("market/prepare-local-binding", async (name2) => {
  ...
  addListener("market/package", async (name2) => {
  ...
  addListener("market/registry", async (names) => {
    const entries = await (0, import_p_map2.default)(names, async (name2) => {
      try {
        ...
        logger.debug(`skip registry metadata for ${name2}: ${error instanceof Error ? error.message : error}`);
      }
    }, { concurrency: ctx.installer.config.concurrency ?? 4 });
    return Object.fromEntries(entries.filter(Boolean));
  }, { authority: 4 });
  addListener("market/ensure-config", async (name2) => {
...
function apply(ctx, config = {}) {
  ...
  if (ensureMarketNextConfigDefaults(ctx, config)) {
    ...
    void ctx.loader.writeConfig(true).then(() => ctx.get("console")?.refresh("config")).catch((error) => logger.warn(error));
  }
  ...
  ctx.inject(["console", "installer", "server"], (ctx2) => {
    ...
    ctx2.on("ready", () => {
      ...
      const timer = setTimeout(() => {
        ...
        void ensureInstalledPluginConfigs(ctx2).catch((error) => logger.warn(error));
      }, 1e3);
