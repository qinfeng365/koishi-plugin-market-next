...
var Installer = class extends import_koishi6.Service {
  ...
  getLocalDepsSnapshot() {
    ...
    for (const name2 of names) {
      ...
      if (!result[name2].local && !(0, import_semver7.valid)(result[name2].request)) {
  ...
  requiresPackageManager(deps, localDeps, forced) {
    if (forced) return true;
    for (const name2 in deps) {
      const nextRequest = deps[name2];
      const currentRequest = this.manifest.dependencies?.[name2];
      const currentSource = classifyDependencySource(currentRequest ?? "", {
        workspace: this.depCache[name2]?.workspace,
        installed: !!this.depCache[name2]?.resolved
      });
      const nextSource = classifyDependencySource(nextRequest ?? "", {
        workspace: localDeps[name2]?.workspace,
        installed: !!localDeps[name2]?.resolved
      });
      if (!nextRequest) return true;
      if (currentRequest !== nextRequest && (currentSource.local || nextSource.local)) return true;
      const { resolved, local } = localDeps[name2] || {};
  ...
  async captureCurrentEnvironmentSnapshot(source, operationId) {
    ...
    for (const [name2, request] of Object.entries(manifest.dependencies)) {
      ...
      dependencies[name2] = {
        ...
        invalid: !local[name2]?.local && !(0, import_semver7.valid)(normalizedRequest)
      };
    }
  ...
  async _installLocked(deps, forced, beforeReload, options = {}) {
    ...
    try {
      ...
      for (const name2 in localDeps) {
        const { resolved } = localDeps[name2];
        if (!newDeps[name2]) continue;
        const requestChanged = snapshot.dependencies[name2] !== deps[name2];
        const localRequestChanged = requestChanged && classifyDependencySource(deps[name2] ?? "", {
          workspace: newDeps[name2].workspace,
          installed: !!newDeps[name2].resolved
        }).local;
        if (newDeps[name2].resolved === resolved && !localRequestChanged) continue;
        try {
        ...
        shouldReload = true;
        logger5.debug(`dependency changed may require full reload: ${name2}, previous=${resolved ?? "-"}, current=${newDeps[name2]?.resolved ?? "-"}`);
      }
      if (beforeReload) {
...
async function assertNoDirectBundleCycles(ctx, packageName, members) {
  ...
  for (const member of members) {
    try {
      ...
      const version = (0, import_semver8.maxSatisfying)(versions, member.version, { includePrerelease: true });
      if (!version) continue;
