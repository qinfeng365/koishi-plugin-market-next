...
function sameDependency(left, right) {
  ...
  return (left.resolved || left.request) === (right.resolved || right.request) && !!left.workspace === !!right.workspace;
}
...
function getEnvironmentDiff(current, target) {
  ...
  return [...names].sort((a, b) => a.localeCompare(b)).map((name2) => {
    ...
    if (currentVersion && targetVersion && (0, import_semver.valid)(currentVersion) && (0, import_semver.valid)(targetVersion)) {
      const direction = (0, import_semver.compare)(targetVersion, currentVersion);
...
function getEnvironmentInstallChanges(diff, target) {
  ...
  for (const change of diff) {
    ...
    changes[change.name] = dependency ? dependency.workspace ? dependency.request : dependency.resolved || dependency.request : "";
  }
  return changes;
}
...
var EnvironmentSnapshotStore = class {
  ...
  async persist() {
    await import_fs2.promises.mkdir((0, import_path2.dirname)(this.filename), { recursive: true });
...
var REGISTRY_FAST_ROUTE_THRESHOLD = import_koishi.Time.second * 0.8;
var REGISTRY_STATS_TTL = import_koishi.Time.day * 30;
var NOT_FOUND_CACHE_TTL = import_koishi.Time.minute * 5;
var FULL_RELOAD_DELAY = import_koishi.Time.second;
var SELF_PACKAGE = "koishi-plugin-market-next";
var DEFAULT_INSTALL_LOG_RETENTION = import_koishi.Time.day * 3;
...
function getVersions(versions) {
  return Object.fromEntries(versions.map((item) => [item.version, (0, import_koishi.pick)(item, ["peerDependencies", "peerDependenciesMeta", "deprecated"])]).sort(([a], [b]) => (0, import_semver2.compare)(b, a)));
}
var Installer = class extends import_koishi.Service {
  constructor(ctx, config = {}) {
    ...
    this.statsFile = (0, import_path3.resolve)(ctx.baseDir, "cache", "market-next-registry-stats.json");
    this.environmentSnapshots = new EnvironmentSnapshotStore(
      (0, import_path3.resolve)(ctx.baseDir, "data", "market-next-environment-snapshots.json"),
  ...
  async loadRouteStats() {
    try {
      ...
      for (const [endpoint, stats] of Object.entries(store.stats)) {
        ...
        const hasRecentSuccess = Number(stats.lastSuccess) && Date.now() - Number(stats.lastSuccess) < import_koishi.Time.day;
  ...
  scheduleStatsWrite() {
    ...
    this.statsWriteTimer = setTimeout(async () => {
      ...
      try {
        await import_fs3.promises.mkdir((0, import_path3.resolve)(this.statsFile, ".."), { recursive: true });
  ...
  getRegistryRouteScore(endpoint) {
    ...
    if (stats.lastSuccess && Date.now() - stats.lastSuccess <= import_koishi.Time.minute * 10) score += 1.5;
  ...
  getFallbackDelay(endpoint) {
    ...
    const recentSuccess = stats.lastSuccess && Date.now() - stats.lastSuccess <= import_koishi.Time.minute * 10;
    ...
    for (let retry = 0; retry <= maxRetry; retry++) {
      ...
      try {
        ...
        if (result.endpoint !== this.metadataEndpoint) {
          ...
          logger.info(`npm registry route selected for ${name2}: endpoint=${result.endpoint}, previous=${this.metadataEndpoint}, reason=${result.fallbackReason ?? "same-priority"}, elapsed=${result.elapsed}ms`);
          this.metadataEndpoint = result.endpoint;
        }
        this.setRegistryStatus(name2, {
          loading: false,
  ...
  async _getPackage(name2, serial = this.serial) {
    try {
      ...
      this.fullCache[name2] = this.tempCache[name2] = getVersions(Object.values(registry.versions).filter((remote) => {
        if (name2 === "koishi") return (0, import_semver2.satisfies)(remote.version, "4");
  ...
  getLocalDepsSnapshot() {
    ...
    for (const name2 of names) {
      ...
      if (!(0, import_semver2.valid)(result[name2].request)) {
  ...
  async _refreshDependencyMetadata(result = this.depCache, serial = this.serial) {
    const start = Date.now();
    const names = Object.keys(result);
    const targets = names.filter((name2) => !result[name2].workspace && !result[name2].invalid);
  ...
  getInstallLogDir() {
    return (0, import_path3.resolve)(this.cwd, "data", INSTALL_LOG_DIR);
  }
  getInstallLogRetention() {
    const hours = Number(this.config.installLogRetentionHours);
    if (Number.isFinite(hours) && hours > 0) return Math.max(1, hours) * import_koishi.Time.hour;
    const legacyRetention = Number(this.config.installLogRetention);
    return Number.isFinite(legacyRetention) && legacyRetention > 0 ? Math.max(import_koishi.Time.hour, legacyRetention) : DEFAULT_INSTALL_LOG_RETENTION;
  }
    ...
    this.installLogCleanupTask = (async () => {
      ...
      try {
        ...
        await Promise.all(entries.filter((entry) => entry.isFile() && (entry.name.endsWith(".log") || entry.name.endsWith(".log.json"))).map(async (entry) => {
          const path = (0, import_path3.resolve)(dir, entry.name);
  ...
  async startInstallLog(deps, forced, options = {}, changes = []) {
    ...
    const file = (0, import_path3.resolve)(dir, `${formatLogTimestamp(now)}-${suffix}.log`);
    const id = (0, import_path3.basename)(file);
...
`;
  ...
  getInstallLogPath(id) {
    if (!id || (0, import_path3.basename)(id) !== id || !id.endsWith(".log")) return;
    return (0, import_path3.resolve)(this.getInstallLogDir(), id);
  }
...
${tail.toString("utf8")}`,
  ...
  parseLegacyInstallLog(id, content, size) {
    ...
    const active = (0, import_path3.basename)(this.installLogFile || "") === id;
  ...
  createInstallHistoryEntry(metadata, size) {
    const status = metadata.status === "running" && (0, import_path3.basename)(this.installLogFile || "") !== metadata.id ? "unknown" : metadata.status;
    return {
      id: metadata.id,
  ...
  async getInstallHistory(limit = 20) {
    ...
    try {
      ...
      files = (await Promise.all(entries.filter((entry) => entry.isFile() && entry.name.endsWith(".log")).map(async (entry) => ({
        id: entry.name,
        mtime: (await import_fs3.promises.stat((0, import_path3.resolve)(dir, entry.name))).mtimeMs
  ...
  async override(deps) {
    const filename = (0, import_path3.resolve)(this.cwd, "package.json");
  ...
  async captureCurrentEnvironmentSnapshot(source, operationId) {
    ...
    for (const [name2, request] of Object.entries(manifest.dependencies)) {
      ...
      dependencies[name2] = {
        ...
        invalid: !(0, import_semver2.valid)(normalizedRequest)
      };
    }
  ...
  async _installLocked(deps, forced, beforeReload, options = {}) {
    ...
    try {
      ...
      for (const name2 in deps) {
        ...
        if (workspace || deps[name2] && resolved && (0, import_semver2.satisfies)(resolved, deps[name2], { includePrerelease: true })) continue;
        forced = true;
...
((Installer2) => {
  Installer2.Config = import_koishi.Schema.object({
    ...
    installLogRetentionHours: import_koishi.Schema.number().min(1).max(24 * 365).step(1).default(72)
  });
})(Installer || (Installer = {}));
function sleep(ms) {
...
function validateBundleManifest(packageName, bundle, options = {}) {
  ...
  for (const [index, member] of bundle.members.entries()) {
    ...
    else if (!(0, import_semver3.validRange)(member.version.trim())) errors.push(`${prefix}.version is not a valid semver range`);
...
function getUpdateCandidates(versions, resolved, policy) {
  if (!resolved || !(0, import_semver4.valid)(resolved)) return [];
  const result = Array.from(new Set(versions)).filter((version) => {
    if (!(0, import_semver4.valid)(version)) return false;
    if (policy?.updateIgnorePrerelease && (0, import_semver4.prerelease)(version)?.length) return false;
    try {
      return (0, import_semver4.gt)(version, resolved);
    } catch {
      return false;
    }
  });
  return result.sort((a, b) => (0, import_semver4.compare)(b, a));
}
...
var MarketProvider2 = class extends MarketProvider {
  constructor(ctx, config = {}) {
    ...
    this.cacheFile = (0, import_path4.resolve)(ctx.baseDir, "cache", "market-next-index.json");
    this.cacheDir = (0, import_path4.resolve)(ctx.baseDir, "cache", "market-next-index");
  ...
  async fetchEndpoint(endpoint, index, total, serial, warnFailure = true, signal) {
    ...
    try {
      ...
      const hash = (0, import_crypto2.createHash)("sha256").update(text).digest("hex");
  ...
  async loadCacheEntryResult(entry) {
    ...
    try {
      const content = await import_fs4.promises.readFile((0, import_path4.resolve)(this.cacheDir, entry.file), "utf8");
  ...
  getCacheEntryFilename(endpoint) {
    return `${(0, import_crypto2.createHash)("sha1").update(endpoint).digest("hex").slice(0, 16)}.json`;
  }
  ...
  async writeCacheEntryFile(entry) {
    ...
    const file = (0, import_path4.resolve)(this.cacheDir, this.getCacheEntryFilename(entry.endpoint));
  ...
  async pruneSplitCacheFiles(entries) {
    try {
      ...
      await Promise.all(files.filter((file) => file.endsWith(".json") && !keep.has(file)).map((file) => import_fs4.promises.unlink((0, import_path4.resolve)(this.cacheDir, file)).catch(() => {
      })));
    } catch (error) {
  ...
  async writeDiskCache(cache2) {
    if (this.disposed || !this.ctx.scope.isActive) return;
    try {
      await import_fs4.promises.mkdir((0, import_path4.dirname)(this.cacheFile), { recursive: true });
      const entries = {};
...
function getAvatarCacheDir(ctx) {
  return (0, import_path5.resolve)(ctx.baseDir, "cache", "market-next-avatars");
}
function normalizeAvatarCacheKey(key) {
  return key.replace(/[^0-9A-Za-z:@._-]/g, "-").slice(0, 128) || `url:${(0, import_crypto3.createHash)("sha1").update(key).digest("hex")}`;
}
function getAvatarCacheFile(ctx, key) {
  return (0, import_path5.resolve)(getAvatarCacheDir(ctx), `${(0, import_crypto3.createHash)("sha1").update(normalizeAvatarCacheKey(key)).digest("hex")}.json`);
}
  ...
  avatarDiskCleanupTask = (async () => {
    try {
      ...
      const entries = await Promise.all(files.filter((file) => file.endsWith(".json")).map(async (file) => {
        const path = (0, import_path5.resolve)(dir, file);
        try {
...
function apply(ctx, config = {}) {
  ...
  ctx.inject(["console", "installer"], (ctx2) => {
    ...
    ctx2.console.addEntry({
      dev: (0, import_path5.resolve)(__dirname, "../../client/index.ts"),
      prod: (0, import_path5.resolve)(__dirname, "../../dist")
    });
