...
function getEnvironmentSnapshotId(dependencies) {
  return `env-${(0, import_crypto.createHash)("sha256").update(canonicalDependencies(dependencies)).digest("hex").slice(0, 20)}`;
}
...
function getEnvironmentDiff(current, target) {
  ...
  return [...names].sort((a, b) => a.localeCompare(b)).map((name2) => {
    ...
    if (currentVersion && targetVersion && (0, import_semver3.valid)(currentVersion) && (0, import_semver3.valid)(targetVersion)) {
      const direction = (0, import_semver3.compare)(targetVersion, currentVersion);
...
var EnvironmentSnapshotStore = class {
  ...
  async load() {
    if (!this.loaded) {
      this.loaded = (async () => {
        try {
          const parsed = JSON.parse(await import_fs2.promises.readFile(this.filename, "utf8"));
  ...
  async persist() {
    await import_fs2.promises.mkdir((0, import_path3.dirname)(this.filename), { recursive: true });
    const temporary = `${this.filename}.${process.pid}.${Date.now()}.tmp`;
    await import_fs2.promises.writeFile(temporary, JSON.stringify(this.value, null, 2) + "\n");
    try {
      await import_fs2.promises.rename(temporary, this.filename);
    } catch (error) {
      if (!["EEXIST", "EPERM"].includes(error?.code)) throw error;
      await import_fs2.promises.rm(this.filename, { force: true });
      await import_fs2.promises.rename(temporary, this.filename);
    }
  }
...
function loadManifest(name2, baseDir) {
  const resolver = baseDir ? (0, import_module.createRequire)((0, import_path4.resolve)(baseDir, "package.json")) : require;
  const filename = resolver.resolve(name2 + "/package.json");
  const meta = JSON.parse((0, import_fs3.readFileSync)(filename, "utf8"));
  meta.dependencies ||= {};
...
function resolvePackageManifest(name2, baseDir) {
  const resolver = (0, import_module.createRequire)((0, import_path4.resolve)(baseDir, "package.json"));
  return resolver.resolve(name2 + "/package.json");
}
function getVersions(versions) {
  return Object.fromEntries(versions.map((item) => [item.version, (0, import_koishi2.pick)(item, ["peerDependencies", "peerDependenciesMeta", "deprecated"])]).sort(([a], [b]) => (0, import_semver4.compare)(b, a)));
}
var Installer = class extends import_koishi2.Service {
  constructor(ctx, config = {}) {
    ...
    this.statsFile = (0, import_path4.resolve)(ctx.baseDir, "cache", "market-next-registry-stats.json");
    this.environmentSnapshots = new EnvironmentSnapshotStore(
      (0, import_path4.resolve)(ctx.baseDir, "data", "market-next-environment-snapshots.json"),
  ...
  async loadRouteStats() {
    try {
      const content = await import_fs3.promises.readFile(this.statsFile, "utf8");
  ...
  scheduleStatsWrite() {
    ...
    this.statsWriteTimer = setTimeout(async () => {
      ...
      try {
        await import_fs3.promises.mkdir((0, import_path4.resolve)(this.statsFile, ".."), { recursive: true });
        await import_fs3.promises.writeFile(this.statsFile, JSON.stringify({ version: 1, stats, savedAt: Date.now() }));
      } catch (error) {
  ...
  waitRouteTurn(index, signal) {
    ...
    return new Promise((resolve5, reject) => {
      ...
      const timer = setTimeout(resolve5, index * REGISTRY_ROUTE_STAGGER);
  ...
  async _getPackage(name2, serial = this.serial) {
    try {
      ...
      this.fullCache[name2] = this.tempCache[name2] = getVersions(Object.values(registry.versions).filter((remote) => {
        if (name2 === "koishi") return (0, import_semver4.satisfies)(remote.version, "4");
  ...
  markRegistryNotFoundDependency(name2, dependency = this.depCache[name2]) {
    const source = classifyRegistryNotFoundDependency(dependency, import_registry.default.isPlugin(name2));
  ...
  getLocalDepsSnapshot() {
    ...
    for (const name2 of names) {
      ...
      if (!result[name2].local && !(0, import_semver4.valid)(result[name2].request)) {
  ...
  getInstallLogDir() {
    return (0, import_path4.resolve)(this.cwd, "data", INSTALL_LOG_DIR);
  }
    ...
    this.installLogCleanupTask = (async () => {
      ...
      try {
        ...
        await Promise.all(entries.filter((entry) => entry.isFile() && (entry.name.endsWith(".log") || entry.name.endsWith(".log.json"))).map(async (entry) => {
          const path = (0, import_path4.resolve)(dir, entry.name);
          if (path === this.installLogFile || path === this.installLogMetadataFile) return;
          try {
            const stat = await import_fs3.promises.stat(path);
            if (now - stat.mtimeMs <= this.getInstallLogRetention()) return;
            await import_fs3.promises.rm(path, { force: true });
          } catch (error) {
  ...
  async writeInstallLogMetadata() {
    ...
    await import_fs3.promises.writeFile(this.installLogMetadataFile, JSON.stringify(this.installLogMetadata, null, 2) + "\n");
  }
  async startInstallLog(deps, forced, options = {}, changes = []) {
    ...
    const file = (0, import_path4.resolve)(dir, `${formatLogTimestamp(now)}-${suffix}.log`);
    const id = (0, import_path4.basename)(file);
    await import_fs3.promises.writeFile(file, [
...
`;
    this.installLogWriteTask = this.installLogWriteTask.then(() => import_fs3.promises.appendFile(file, text)).catch((error) => {
  ...
  getInstallLogPath(id) {
    if (!id || (0, import_path4.basename)(id) !== id || !id.endsWith(".log")) return;
    return (0, import_path4.resolve)(this.getInstallLogDir(), id);
  }
  async readInstallLogMetadata(id) {
    const file = this.getInstallLogPath(id);
    if (!file) return;
    try {
      const metadata = JSON.parse(await import_fs3.promises.readFile(file + ".json", "utf8"));
  ...
  async readInstallLog(file, limit, headLimit, tailLimit) {
    const stat = await import_fs3.promises.stat(file);
    if (stat.size <= limit) {
      return {
        content: await import_fs3.promises.readFile(file, "utf8"),
        truncated: false,
        size: stat.size
      };
    }
    const handle = await import_fs3.promises.open(file, "r");
    try {
...
${tail.toString("utf8")}`,
  ...
  parseLegacyInstallLog(id, content, size) {
    ...
    const active = (0, import_path4.basename)(this.installLogFile || "") === id;
  ...
  createInstallHistoryEntry(metadata, size) {
    const status = metadata.status === "running" && (0, import_path4.basename)(this.installLogFile || "") !== metadata.id ? "unknown" : metadata.status;
    return {
      id: metadata.id,
  ...
  async getInstallHistoryEntry(id) {
    ...
    let stat;
    try {
      stat = await import_fs3.promises.stat(file);
    } catch (error) {
  ...
  async getInstallHistory(limit = 20) {
    ...
    try {
      ...
      files = (await Promise.all(entries.filter((entry) => entry.isFile() && entry.name.endsWith(".log")).map(async (entry) => ({
        id: entry.name,
        mtime: (await import_fs3.promises.stat((0, import_path4.resolve)(dir, entry.name))).mtimeMs
  ...
  async override(deps) {
    const filename = (0, import_path4.resolve)(this.cwd, "package.json");
    ...
    await import_fs3.promises.writeFile(filename, JSON.stringify(this.manifest, null, 2) + "\n");
  ...
  async snapshotPackageManifest() {
    const filename = (0, import_path4.resolve)(this.cwd, "package.json");
    const content = await import_fs3.promises.readFile(filename, "utf8");
  ...
  async restorePackageManifest(snapshot, deps, reason) {
    ...
    let manifest;
    try {
      manifest = JSON.parse(await import_fs3.promises.readFile(filename, "utf8"));
    } catch {
    ...
    await import_fs3.promises.writeFile(filename, JSON.stringify(manifest, null, 2) + "\n");
  ...
  requiresPackageManager(deps, localDeps, forced) {
    ...
    for (const name2 in deps) {
      ...
      if (local || resolved && (0, import_semver4.satisfies)(resolved, nextRequest, { includePrerelease: true })) continue;
      return true;
    }
    return false;
  }
  async captureCurrentEnvironmentSnapshot(source, operationId) {
    ...
    for (const [name2, request] of Object.entries(manifest.dependencies)) {
      ...
      dependencies[name2] = {
        ...
        invalid: !local[name2]?.local && !(0, import_semver4.valid)(normalizedRequest)
      };
    }
  ...
  async _installLocked(deps, forced, beforeReload, options = {}) {
    ...
    try {
      ...
      for (const name2 in localDeps) {
        const { resolved, local } = localDeps[name2];
        if (local || !newDeps[name2]) continue;
  ...
  async withInstallLock(description2, callback) {
    ...
    let release;
    this.installTask = new Promise((resolve5) => {
      release = resolve5;
    });
  ...
  async prepareLocalBinding(name2) {
    ...
    if (!import_registry.default.isPlugin(name2) || !Object.prototype.hasOwnProperty.call(packageSnapshot.dependencies, name2)) {
    ...
    const sourceDir = (0, import_path4.dirname)(manifestFile);
    ...
    const destination = (0, import_path4.resolve)(this.cwd, ".yarn", "local");
    await import_fs3.promises.mkdir(destination, { recursive: true });
    const temporary = await import_fs3.promises.mkdtemp((0, import_path4.resolve)(destination, ".market-next-pack-"));
    try {
      ...
      const packedFile = (0, import_path4.resolve)(temporary, pack.filename);
      if ((0, import_path4.dirname)(packedFile) !== temporary || (0, import_path4.relative)(temporary, packedFile).startsWith("..")) {
      ...
      const target = (0, import_path4.resolve)(destination, filename);
      if ((0, import_path4.dirname)(target) !== destination || (0, import_path4.relative)(destination, target).startsWith("..")) {
...
var MarketProvider2 = class extends MarketProvider {
  constructor(ctx, config = {}) {
    ...
    this.cacheFile = (0, import_path5.resolve)(ctx.baseDir, "cache", "market-next-index.json");
    this.cacheDir = (0, import_path5.resolve)(ctx.baseDir, "cache", "market-next-index");
  ...
  waitRouteTurn(index, signal) {
    ...
    return new Promise((resolve5, reject) => {
      if (signal?.aborted) return reject(signal.reason);
      const timer = setTimeout(resolve5, index * ROUTE_STAGGER);
      signal?.addEventListener("abort", () => {
        clearTimeout(timer);
        reject(signal.reason);
      }, { once: true });
  ...
  async fetchEndpoint(endpoint, index, total, serial, warnFailure = true, signal) {
    ...
    try {
      ...
      const hash = (0, import_crypto3.createHash)("sha256").update(text).digest("hex");
  ...
  async loadDiskCache(serial) {
    ...
    try {
      ...
      const content = await import_fs4.promises.readFile(this.cacheFile, "utf8");
  ...
  async loadCacheEntryResult(entry) {
    ...
    try {
      const content = await import_fs4.promises.readFile((0, import_path5.resolve)(this.cacheDir, entry.file), "utf8");
  ...
  getCacheEntryFilename(endpoint) {
    return `${(0, import_crypto3.createHash)("sha1").update(endpoint).digest("hex").slice(0, 16)}.json`;
  }
  ...
  async writeCacheEntryFile(entry) {
    await import_fs4.promises.mkdir(this.cacheDir, { recursive: true });
    const file = (0, import_path5.resolve)(this.cacheDir, this.getCacheEntryFilename(entry.endpoint));
    const tempFile = `${file}.${process.pid}.${Date.now()}.tmp`;
    await import_fs4.promises.writeFile(tempFile, JSON.stringify(entry.result));
    await import_fs4.promises.rename(tempFile, file);
  }
  async pruneSplitCacheFiles(entries) {
    try {
      const keep = new Set(Object.values(entries).map((entry) => entry?.file).filter(Boolean));
      const files = await import_fs4.promises.readdir(this.cacheDir).catch(() => []);
      await Promise.all(files.filter((file) => file.endsWith(".json") && !keep.has(file)).map((file) => import_fs4.promises.unlink((0, import_path5.resolve)(this.cacheDir, file)).catch(() => {
      })));
    } catch (error) {
  ...
  async writeDiskCache(cache2) {
    if (this.disposed || !this.ctx.scope.isActive) return;
    try {
      await import_fs4.promises.mkdir((0, import_path5.dirname)(this.cacheFile), { recursive: true });
      const entries = {};
      ...
      await import_fs4.promises.writeFile(tempFile, nextStore);
      await import_fs4.promises.rename(tempFile, this.cacheFile);
  ...
  try {
    return await Promise.race([
      ...
      new Promise((resolve5) => {
        timer = setTimeout(() => resolve5(false), timeout);
      })
    ]);
  } finally {
    clearTimeout(timer);
...
function getAvatarCacheDir(ctx) {
  return (0, import_path6.resolve)(ctx.baseDir, "cache", "market-next-avatars");
}
function normalizeAvatarCacheKey(key) {
  return key.replace(/[^0-9A-Za-z:@._-]/g, "-").slice(0, 128) || `url:${(0, import_crypto4.createHash)("sha1").update(key).digest("hex")}`;
}
function getAvatarCacheFile(ctx, key) {
  return (0, import_path6.resolve)(getAvatarCacheDir(ctx), `${(0, import_crypto4.createHash)("sha1").update(normalizeAvatarCacheKey(key)).digest("hex")}.json`);
}
...
async function readAvatarDiskCache(ctx, key) {
  try {
    ...
    const entry = normalizeAvatarDiskCache(JSON.parse(await import_fs5.promises.readFile(file, "utf8")), key);
    if (!entry) {
      void import_fs5.promises.unlink(file).catch(() => {
      });
      return;
    }
...
async function writeAvatarDiskCache(ctx, key, url, result) {
  try {
    await import_fs5.promises.mkdir(getAvatarCacheDir(ctx), { recursive: true });
    const file = getAvatarCacheFile(ctx, key);
    const tempFile = `${file}.${process.pid}.${Date.now()}.tmp`;
    const entry = {
      key,
    ...
    await import_fs5.promises.writeFile(tempFile, JSON.stringify(entry));
...
async function cleanupAvatarDiskCache(ctx) {
  ...
  avatarDiskCleanupTask = (async () => {
    try {
      ...
      const files = await import_fs5.promises.readdir(dir).catch(() => []);
      const entries = await Promise.all(files.filter((file) => file.endsWith(".json")).map(async (file) => {
        const path = (0, import_path6.resolve)(dir, file);
        try {
          const stat = await import_fs5.promises.stat(path);
          let cachedAt = stat.mtimeMs;
          try {
            const value = JSON.parse(await import_fs5.promises.readFile(path, "utf8"));
            cachedAt = Number(value.cachedAt) || cachedAt;
            if (!value?.key || !value?.url || typeof value?.data !== "string" || typeof value?.type !== "string") {
              await import_fs5.promises.unlink(path).catch(() => {
              });
              return;
            }
      ...
      await Promise.all(alive.slice(AVATAR_MAX_ENTRIES).map((entry) => import_fs5.promises.unlink(entry.path).catch(() => {
      })));
    } finally {
...
async function clearAvatarCacheStorage(ctx) {
  ...
  const dir = getAvatarCacheDir(ctx);
  const files = await import_fs5.promises.readdir(dir).catch((error) => {
    if (error?.code === "ENOENT") return [];
    throw error;
  });
  const disk = files.filter((file) => file.endsWith(".json")).length;
  await import_fs5.promises.rm(dir, { recursive: true, force: true });
  return { memory, disk };
}
...
async function isBlockedAvatarTarget(url) {
  ...
  try {
    const records = await (0, import_promises.lookup)(hostname, { all: true, verbatim: false });
...
async function ensurePluginConfigs(ctx, names) {
  ...
  let changed = false;
  let checked = 0;
  for (const name2 of names.filter((name3) => import_registry3.default.isPlugin(name3))) {
...
async function ensureInstalledPluginConfigs(ctx) {
  ...
  const names = Object.keys(manifest.dependencies ?? {}).filter((name2) => import_registry3.default.isPlugin(name2)).filter((name2) => !isPluginBundleDependency(name2));
...
async function assertNoDirectBundleCycles(ctx, packageName, members) {
  ...
  for (const member of members) {
    try {
      ...
      const version = (0, import_semver5.maxSatisfying)(versions, member.version, { includePrerelease: true });
      if (!version) continue;
...
function apply(ctx, config = {}) {
  ...
  ctx.inject(["console", "installer"], (ctx2) => {
    ...
    ctx2.console.addEntry({
      dev: (0, import_path6.resolve)(__dirname, "../../client/index.ts"),
      prod: (0, import_path6.resolve)(__dirname, "../../dist")
    });
