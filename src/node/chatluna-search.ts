...
export const CHATLUNA_TOOL_NAME = 'koishi_plugin_market_search'
const DAY = Time.day
...
interface SearchPayload {
  tool: typeof CHATLUNA_TOOL_NAME
  registry: string
...
export function formatSearchResult(result: LoadResult, input: NormalizedSearchInput) {
  const { index } = result
...
export function normalizeInput(input: SearchInput): NormalizedSearchInput {
...
export function formatLoadError(endpoint: string, input: NormalizedSearchInput, error: unknown) {
  const payload: SearchPayload = {
    tool: CHATLUNA_TOOL_NAME,
    registry: endpoint,
    fetchedAt: null,
